...
/// ROADMAP §2 "Index sealing" (I4): is `e`'s TOP an OPERATOR that [`fold_bits_at`]
/// folds — unary `~ - +`, a binary with a wide-domain arm, or a ternary?
///
/// [`fold_bits_at`]: crate::const_wide::fold_bits_at
///
/// Only the arms that exist: `& | ^ ~^` (bitwise), `+ - *` (limbs), `/ %`
/// (`wide_divmod`), `<< <<< >>` (logical shift), `>>>` (arithmetic shift), `**`
/// (`wide_pow`), and the conditional. Comparisons and `&& ||` are not listed: their
/// top is self-determined and `wide_top_is_self_determined` already admits it. An
/// operator the domain has no arm for stays out rather than reaching a decline.
///
/// This is a SHAPE test only. `Elaborator::override_bits` pairs it with "no fill",
/// [`wide_operator_tree_is_plain`] and "self-determined width past 64 bits", so an
/// operator top at or below 64 bits never binds through the wide fold.
pub(crate) fn wide_operator_top(e: &ast::Expr) -> bool {
    match &e.kind {
        ast::ExprKind::Paren { inner } => wide_operator_top(inner),
        ast::ExprKind::Unary {
            op: ast::UnOp::BitNot | ast::UnOp::Minus | ast::UnOp::Plus,
            ..
        } => true,
...
/// A node's own `(width, signed, top bit set)` in the wide domain; `None` = declines.
pub(crate) type NodeShape<'a> = &'a dyn Fn(&ast::Expr) -> Option<(u32, bool, bool)>;

/// ROADMAP §2 "Index sealing" I4: is the operator-topped override tree `e` one the wide
/// fold computes the way §11.6.1 and §11.8.2 do? A STRUCTURAL rule: no values, and no
/// per-operator mirroring of the fold.
///
/// Holds iff
/// * (a) the top is [`wide_operator_top`];
/// * (b) every SELF-DETERMINED position in the tree holds a PLAIN LEAF — a sized
///   literal, a name or a package-qualified name. The positions: size-cast width and
///   operand, signing-cast operand, concatenation parts, replication count and parts,
///   comparison / case-equality / logical operands, a shift's right operand, `**`'s
///   exponent, the ternary condition, reduction and `!` operands, select base and
///   indices, system-function arguments. An operator, cast, concatenation, select or
///   call inside a position fails;
/// * (c) the tree is SIGN-HOMOGENEOUS: every node reached through the
///   context-determined arms (a position's own node — the comparison, the cast, the
///   concatenation — included) has the top's sign. A position's LEAF is exempt: it is
///   folded self-determined and keeps its own type (§11.8.1), so the tree's sign never
///   extends it — `128'hFF << 64` (a signed count) and `1 ? ~128'd0 : 128'd1` (a
///   signed condition) are plain.
///
/// WHY. The shared walk (`fold_bits_at`) has two defects this arm cannot repair: it
/// folds a self-determined position with no context, so an operator inside one is
/// computed at its own width (`{~8'd1 + 120'd0} + 128'd0` gives `0…0fe` where both
/// oracles give `00ff…fe`); and it does not push §11.8.2's expression sign down into
/// the operands, so a signed operand of an unsigned tree extends with its own sign
/// (`~S8 + 128'd0` gives `0…02` where both give `ff…f02`). Both are pre-existing on the
/// localparam lane. (b) keeps every position a leaf, where no context is needed; (c)
/// keeps every extension in the one sign the whole tree has, where the push-down is a
/// no-op. Repairing the walk is a prerequisite slice; until then a tree outside the
/// rule keeps the pre-slice route. `shape` answering `None` fails the rule.
pub(crate) fn wide_operator_tree_is_plain(e: &ast::Expr, shape: NodeShape) -> bool {
    if !wide_operator_top(e) {
        return false;
    }
    let Some((_, top_sign, _)) = shape(e) else {
        return false;
    };
    tree_is_plain(e, top_sign, shape)
}

fn same_sign(x: &ast::Expr, sign: bool, shape: NodeShape) -> bool {
    shape(x).is_some_and(|(_, s, _)| s == sign)
}

/// A self-determined position: a plain leaf (any sign — see (c)) the fold answers.
fn position_is_plain(x: &ast::Expr, shape: NodeShape) -> bool {
    match &x.kind {
        ast::ExprKind::Paren { inner } => position_is_plain(inner, shape),
        ast::ExprKind::IntLit { kind, raw } => {
            !crate::literal::is_fill_literal(raw, *kind) && shape(x).is_some()
        }
        ast::ExprKind::Ident(_) | ast::ExprKind::PkgScoped { .. } => shape(x).is_some(),
        _ => false,
    }
}

/// A node reached through the context-determined arms.
fn tree_is_plain(e: &ast::Expr, sign: bool, shape: NodeShape) -> bool {
    use ast::BinOp as B;
    use ast::ExprKind as K;
    use ast::UnOp as U;
    let pos = |x: &ast::Expr| position_is_plain(x, shape);
    let sub = |x: &ast::Expr| tree_is_plain(x, sign, shape);
    if let K::Paren { inner } = &e.kind {
        return sub(inner);
    }
    if !same_sign(e, sign, shape) {
        return false;
    }
    match &e.kind {
        K::Paren { inner } => sub(inner),
        // A leaf OPERAND of the tree: extended by the tree, so its sign was checked above.
        K::IntLit { .. } | K::Ident(_) | K::PkgScoped { .. } => pos(e),
        K::Unary { op, operand } => match op {
            U::Plus | U::Minus | U::BitNot => sub(operand),
        ...
        K::Binary { op, lhs, rhs } => match op {
            ...
            | B::BitXor
            | B::BitXnor
            | B::BitOr => sub(lhs) && sub(rhs),
            B::Shl | B::Shr | B::AShl | B::AShr | B::Pow => sub(lhs) && pos(rhs),
            B::Lt
            | B::Le
            | B::Gt
            | B::Ge
            ...
            | B::CaseNe
            | B::LogAnd
            | B::LogOr => pos(lhs) && pos(rhs),
            B::WildEq | B::WildNe => false,
        },
        K::Ternary {
            cond,
            then_e,
            else_e,
        } => pos(cond) && sub(then_e) && sub(else_e),
        K::Cast { target, expr } => match target {
            ast::CastTarget::Size(n) => pos(n) && pos(expr),
            ast::CastTarget::Signing { .. } => pos(expr),
            ast::CastTarget::Prim(_)
            | ast::CastTarget::Named(_)
            | ast::CastTarget::SigningParam { .. } => false,
        },
        K::Concat { parts } => parts.iter().all(pos),
        K::Replicate { count, value } => pos(count) && value.iter().all(pos),
        K::BitSelect { base, index } => pos(base) && pos(index),
        K::PartSelect { base, msb, lsb } => pos(base) && pos(msb) && pos(lsb),
        K::IndexedPart {
            base,
            offset,
            width,
            ..
        } => pos(base) && pos(offset) && pos(width),
        K::SysCall { args, .. } => args.iter().all(pos),
        K::RealLit { .. }
        | K::StrLit { .. }
        | K::Call { .. }
        | K::RandomizeWith(_)
