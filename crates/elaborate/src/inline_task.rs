...
impl Elaborator<'_> {
    ...
    pub(crate) fn hoist_inline_task_locals(
        ...
        for (chain, d) in decls {
            ...
            // `$blk$…` prefix the Logic-phase `Stmt::Block` arm lowers its block body
            // in, so two same-named sibling block-locals get two nets instead of
            // sharing one under `$itask$<name>$L`. `None` = the pre-existing path.
            let allocated = match self.block_local_scope_prefix(chain, d) {
                Some(seg) => self.with_scope(&seg, |s| s.hoist_one_inline_local(d)),
