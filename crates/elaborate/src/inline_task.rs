...
impl Elaborator<'_> {
    /// Inline a user-task call into the current process: the body statements join the
    /// caller's CFG via the normal `lower_stmt` machinery (so a task with if/case/delay
    /// just works). INPUT formals substitute a read ExprId; OUTPUT/INOUT formals bind to
    /// the caller's net (reads + writes hit it directly).
    pub(crate) fn inline_task(
        ...
        if name.segments.len() == 2 {
            // The statement twin of `inline_fn`'s modport-call refusal (§25.5).
        ...
        // §2 Scoping (subroutine block-locals): each local carries the CHAIN of blocks
        // that declare it (empty for a body-top local), so the reserver can reproduce
        // the `$blk$<lo>` segments the body lowering wraps the block in.
        let mut tlocals: Vec<(Vec<ast::Span>, ast::NetVarDecl)> = task
            .body_decls
            .iter()
            .map(|d| (Vec::new(), d.clone()))
            .collect();
        crate::block_local::collect_block_local_decls_spanned(
            &task.body,
            &mut Vec::new(),
            &mut tlocals,
        );
