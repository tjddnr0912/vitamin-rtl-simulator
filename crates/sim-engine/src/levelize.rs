...
/// One step of a fusion prelude.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PreludeStep {
    /// Run this process template's body.
    Body(u32),
    /// Perform this continuous assign inline. It is a whole-net copy (`assign y = r`),
    /// the shape a module port connection lowers to, and it is REMOVED from the settle —
    /// the fused body is now its only evaluator.
    Copy(u32),
}

/// A fusion CHAIN, resolved into the form the engine executes.
///
/// `consumer` runs `prelude` — ancestor bodies and the copy assigns between them, in
/// dependency order — before its own body, all inside ONE activation. Every `Body` in
/// the prelude is fused away (never armed, never seeded) and every `Copy` is dropped
/// from the settle, because the only thing that could observe either running separately
/// is `consumer`, which now runs immediately after them.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FusionChain {
    pub consumer: usize,
    pub prelude: Vec<PreludeStep>,
}

/// Producer → (consumer, copies crossed). Unifies adjacent fusion (zero copies) with
/// fusion across a port-connection copy chain, which is the shape real module-to-module
/// connections take: `always_comb r` → `assign y = r` → parent wire → child input →
/// next stage. Adjacent-only fusion was measured to fire on NOTHING real; crossing the
/// copy chain finds 23 of 24 stages on the instance-chained benchmark.
fn fusion_pairs_with_copies(ir: &SimIr) -> Vec<(usize, usize, Vec<u32>)> {
    let n = ir.processes.len();
    let comb = |p: usize| matches!(ir.processes[p].sensitivity.kind, SensKind::Comb);
    let lreads: Vec<BTreeSet<u32>> = (0..n).map(|p| level_reads(ir, p)).collect();
    let ereads: Vec<BTreeSet<u32>> = (0..n).map(|p| edge_reads(ir, p)).collect();
    ...
    // Every net any NON-copy continuous assign touches, on either side. A net in here
    // cannot be crossed: the settle between the two bodies would have seen it move.
    let mut other_ca: BTreeSet<u32> = BTreeSet::new();
    for (ci, c) in ir.cont_assigns.iter().enumerate() {
        if copy_cis.contains(&ci) {
            continue;
    ...
    for (p, bw) in bwrites.iter().enumerate() {
        ...
        let mut hops = 0usize;
        let consumer = loop {
            // Nothing outside the chain may touch the connecting net.
            if other_ca.contains(&cur)
                || ereads.iter().any(|s| s.contains(&cur))
                || nwrites.iter().any(|s| s.contains(&cur))
                || (0..n).any(|q| q != p && bwrites[q].contains(&cur))
            {
                break None;
            }
            // A net written by more than one copy has no single producer to fuse into.
            if copies.iter().filter(|(_, y, _)| *y == cur).count() > 1 {
                break None;
            }
            let readers: Vec<usize> = (0..n).filter(|&q| lreads[q].contains(&cur)).collect();
            let outs: Vec<&(u32, u32, usize)> =
                copies.iter().filter(|(rn, _, _)| *rn == cur).collect();
            match (readers.as_slice(), outs.as_slice()) {
                // Exactly one process reads it and no copy forwards it: chain end.
                ([q], []) => break Some(*q),
                // No process reads it and exactly one copy forwards it: keep walking.
                ([], [one]) => {
                    crossed.push(one.2 as u32);
                    cur = one.1;
                    hops += 1;
                    if hops > copies.len() + 1 {
                        break None; // a copy cycle: refuse rather than spin
        ...
        let Some(q) = consumer else { continue };
        if comb(q) && q != p {
            out.push((p, q, crossed));
        }
    }
    out
}

/// Resolve fusable pairs into maximal chains the engine can execute.
///
/// Chaining transitively is the point: a depth-D ladder becomes ONE activation of D
/// bodies. Measured (`perf_fusion_effect`, depth 48): 72.7 ms unfused vs 42.6 ms fused
/// on the interpreter, and 50.1 ms vs 20.0 ms on the bytecode VM.
///
/// A template appears in at most one chain, so prelude lists partition cleanly and no
/// body can run twice.
pub fn fusion_chains(ir: &SimIr) -> Vec<FusionChain> {
    let pairs = fusion_pairs_with_copies(ir);
    let mut succ: BTreeMap<usize, (usize, Vec<u32>)> = BTreeMap::new();
    let mut pred: BTreeMap<usize, usize> = BTreeMap::new();
    for (p, q, copies) in pairs.iter().cloned() {
        // Refuse to build on a non-function mapping rather than silently dropping an
        // edge — a body that ran twice, or not at all, is a wrong value at exit 0.
        if succ.contains_key(&p) || pred.contains_key(&q) {
            return Vec::new();
        }
        succ.insert(p, (q, copies));
        pred.insert(q, p);
    }
    let mut out = Vec::new();
    for (_, &(tail, _)) in succ.iter() {
        if succ.contains_key(&tail) {
            continue; // not the end of its chain
        }
        // Walk back from the tail collecting (body, copies-to-next) in reverse.
        let mut rev: Vec<(u32, Vec<u32>)> = Vec::new();
        let mut cur = tail;
        let mut guard = 0usize;
        while let Some(&anc) = pred.get(&cur) {
            let copies = succ.get(&anc).map(|(_, c)| c.clone()).unwrap_or_default();
            rev.push((anc as u32, copies));
            cur = anc;
            guard += 1;
            if guard > pairs.len() {
                return Vec::new(); // cycle: refuse the whole transform
            }
        }
        rev.reverse();
        let mut prelude = Vec::new();
        for (body, copies) in rev {
            prelude.push(PreludeStep::Body(body));
            prelude.extend(copies.into_iter().map(PreludeStep::Copy));
        }
...
/// Continuous assigns consumed by a fusion prelude — the settle must NOT evaluate them,
/// because the fused body is now their only evaluator.
pub fn fused_copy_set(chains: &[FusionChain]) -> BTreeSet<u32> {
    chains
        .iter()
        .flat_map(|c| &c.prelude)
        .filter_map(|s| match s {
            PreludeStep::Copy(ci) => Some(*ci),
            PreludeStep::Body(_) => None,
        })
        .collect()
}
