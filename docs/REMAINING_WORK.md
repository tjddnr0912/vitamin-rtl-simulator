...
7915 tests passing with 15 skipped, artifact `format_version` 31, 70 `MsgCode` diagnostic codes; the
...
| # | track | item |
|---|---|---|
| 1 | §2 silent-wrong | A package routine's formal DEFAULT-VALUE expression is lowered outside the package scope (`input [15:0] a = x` reads the module's `x`: `D=ef` against both oracles' `124`); the default is handed to the caller-scope actual lowering ahead of the §4.5.493 scope push |
| 2 | §2 silent-wrong | A call ACTUAL's region is not a §11.6.1 context of the formal's width (`idw($signed(u8) * q8)` into `input [31:0]` is `20` against both oracles' `120`, module and inline twins alike); the actual lowering takes the context walk only for a fill |
| 3 | §3 loud → correct-support | `always_comb a1 = $signed(u8) * q8;` beside a declaration initializer on `u8` is a false-loud E3001 MULTIDRIVER (both oracles print `120`); the multidriver walk counts a `$signed` argument as a write |
...
| §2 recorded defects by mechanism | 110 bullets | inline / frame binds 16 · size cast / signedness 16 · constant domain (i64) 14 · scoping / imports / block-locals 17 · oracle splits 10 · performance 7 · real 7 · index sealing 6 · delays / events 6 · ranges / bounds / selects 4 · diagnostics / artifacts 4 · class fields 3 |
...
| §3 small residues | 86 rows | subroutine / frame 23 · constants / parameters 16 · parser accept 12 · system tasks & file I/O 9 · nets / timing 6 · strings / heap 6 · loud shapes surfaced by §4.5.493–495 6 · diagnostics quality 5 · VCD / real conversion 3 |
