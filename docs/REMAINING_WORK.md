...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-04 = **format_version 19 · 3076 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(MODULE sibling-block string[read-gated]·packed-WIDTH sibling·leading-NUL frame string 등 소형 — 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-04 `integer unsigned` PARAM/VAR ✅**(§4.5.96·§4.5.91 companion·파서 trailing `opt_signed`·oracle-backed ②·적대 R1 CONVERGE CLEAN·2499 snippet byte-identity) · INLINE sibling-block string 충돌 loud(§4.5.95) · FRAME string FORMAL `s[i]`(§4.5.94) · INLINE string `s[i]`(§4.5.93). **string `s[i]` 완결**·**inline sibling string loud화**·**unsigned-param family 완결**(int/byte/shortint/longint/integer). **EXT2-D**(block-local `automatic`)=**deep+no-oracle**(iverilog "sorry"=파싱O 시맨틱X·per-entry storage 필요)로 재분류. **G2 OBS 트랙**: OBS-1a ✅. **follow-on(pre-existing loud)**: function RETURN-type integer signing(E2002 별 parser)·`bit[N] unsigned`/`time/real` trailing-signing(no-oracle).
