...
>
> - **기준(2026-09-01)**: format_version **29** · **6,418 tests green** · 워크로드 코퍼스 **9/10** · CI **3-OS + `build-no-oracle`** green · MsgCode **68** · **MSRV 1.85** · **기본 백엔드 = `native`**.
...
| **2** | **§3 loud→supported** — ⭐⭐ 착수 순서는 **워크로드 코퍼스**가 정한다(§4.5.369). 남은 거절 하나 = **⑧ verilog-ethernet**(frame-call subset 밖) · ⚠️ **verilog-axi 는 `Expect::Split`**(돈다 · 다이제스트는 오라클과 다르고 그 잔여는 **오라클 자기모순** = §2-N 판정)이라 제2 오라클 없이 착수 금지 | 2-오라클 · hand-IEEE · ⚠️ **재그라운딩 2026-09-01: ⑧ 은 `ca_always` 성능 고침 뒤에** — 게이트 ~10줄로 elaborate 는 1.03 s 에 통과하지만 시뮬이 **~38시간 대 iverilog 7.6 s**(런타임의 **100.0%** 가 `Expr::Call` RHS 연속대입 80줄) ⇒ 거절이 닫히는 게 아니라 옮겨간다 |
| **2a** | ⭐ **워크로드 코퍼스 확장** — 지금 **10 중 1 거절 · 1 ruled-split**. **ibex** 가 열리면 코퍼스 첫 SystemVerilog 워크로드 | `corpus-runner list` · [study/03](study/03-workload-corpus.md) |
