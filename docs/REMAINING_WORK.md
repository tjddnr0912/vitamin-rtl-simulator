...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-04 = **format_version 19 · 3012 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(INLINE func/task md-packed subst-bound local·CLASS-METHOD string-formal literal·string local `s[i]`/sibling-block — 전부 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-04 TASK string formal 2종+static loud-reject ✅**(§4.5.88·`str_params`+`formal_str` task 미러·`run_task` copy-in 재료화·static task string formal은 loud-reject=divergence 방지·적대 R1 CONVERGE CLEAN) · FRAME 함수 string LITERAL(§4.5.87) · 2-state class FIELD X→0(§4.5.86) · class-method 2-state coercion(§4.5.85). **G2 OBS 트랙**: OBS-1a ✅ · OBS-1b는 EXT2 체인 다음.
