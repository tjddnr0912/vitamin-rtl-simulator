...
7890 tests passing with 15 skipped, artifact `format_version` 31, 70 `MsgCode` diagnostic codes; the
...
| # | track | item |
|---|---|---|
| 1 | §2 silent-wrong | An imported package function's body reads a package VARIABLE through the MODULE scope (`Z=ee` against both oracles' `Z=123`, and E3010 where the module declares no such net); a package routine body is lowered against the caller's nets and has no package-scope name resolution |
| 2 | §2 silent-wrong | The FRAME route takes a §11.6.1 width context on a REAL target where there is none (`function automatic real f; f = a8*b8;` is `65025.000000` against both oracles' `1.000000`) — the mirror image of §4.5.491, which closed the inline route |
| 3 | §2 silent-wrong | A `$signed(...)` / `$unsigned(...)` leaf in an inline function body is not widened to its §11.6.1 region (`$unsigned(s8) * q8` is `00000020` against both oracles' `0000d820`); one leaf arm in `widen_inline_leaf` |
...
| §2 recorded defects by mechanism | 107 bullets | inline / frame binds 17 · size cast / signedness 16 · constant domain (i64) 14 · scoping / imports / block-locals 13 · oracle splits 10 · performance 7 · real 7 · index sealing 6 · delays / events 6 · ranges / bounds / selects 4 · diagnostics / artifacts 4 · class fields 3 |
...
| §3 small residues | 80 rows | subroutine / frame 23 · constants / parameters 16 · parser accept 12 · system tasks & file I/O 9 · nets / timing 6 · strings / heap 6 · diagnostics quality 5 · VCD / real conversion 3 |
