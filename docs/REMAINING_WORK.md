...
>
> - **기준(2026-07-20)**: format_version **22** · **3726 tests green** · 3-OS CI green · MsgCode 58 · **MSRV 1.85** · 최신 완료 §4.5.168(**V3/V4 suspendable tasks** — task 内 `@`/`#`/wait/NBA/$systask/재귀/nested가 scheduler call-stack + window stash/restore로 동작·round-14 gating ①·format 22 불변[recompute]·적대 2-lens core SOUND+5 over-lift correct-or-loud화·잔여 loud=frame-local array/wait-frame-local/repeat-timing/fork/disable-fork) · 직전 §4.5.167(round-14 loud→supported: **V9** 64-bit MSB-set scalar-localparam 리터럴 fold[`const_eval_i64_lit`의 signed-only width-64 bit-reinterpret arm을 `cv.width==64`로 확장·§4.5.151 defer 해소·magnitude sink 全 loud 확인] + **V1** function/task/block/class-method-local `string` 변수[`frame_local_net_kind`·4 사이트 heap NetKind::String slot] + **V1-twin** 엔진 str_bytes frame-aware[심층검증서 loud→silent 회귀 발굴: NetKind만 고치면 `.len()` 등 method가 frame slot 못 읽어 조용히 0→`read_net` 미러로 수정]; round-14=§4.5.166 ★V10의 confirmation 리포트; 적대 2-lens differential CLEAN[~40 probe·zero silent-wrong·오히려 iverilog가 magnitude-misuse silent] + soundness SOUND[4 change 全]; elaborate value-only+engine eval-only·format 불변). 미착수 후보=**V3/V4 task suspend(구조적 DEEP·TB gate)·V2A/V5 frame dyn-array(§3)·V2B/V6/V8 oracle-reject** · 함수/태스크 body-내부 read→caller comb 감도(§2 DEEP·§4.5.166) · size-cast `N'(expr)` context width(§2 DEEP).
