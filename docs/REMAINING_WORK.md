...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-04 = **format_version 19 · 3055 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(🔴FRAME(2-state-return/automatic) string formal/local `s[i]`=0/X[idiomatic `function int(string)`]·sibling-block 등 — 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-04 INLINE(4-state-return) string local/formal `s[i]` ✅**(§4.5.93·`string_index_read`→`.getc` 라우팅[`handle_is_str_readable` 게이트=Signal-to-string-net/Const만·frame 1-bit wire fall-through]+엔진 `handle_str_bytes` Const read·적대 R1 CONVERGE CLEAN) · INLINE md-packed local(§4.5.92) · `int unsigned` PARAM(§4.5.91) · class method/ctor DEFAULT-ARG(§4.5.90). **G2 OBS 트랙**: OBS-1a ✅ · OBS-1b는 EXT2 체인 다음.
