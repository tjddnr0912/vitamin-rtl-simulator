...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-04 = **format_version 19 · 3040 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(INLINE func/task md-packed subst-bound local·string local `s[i]`/sibling-block — 전부 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-04 `int/byte/shortint/longint unsigned` PARAM 부호 drop ✅**(§4.5.91·파서 explicit-signing-wins+elaborate `p.signed` 사용·적대 R1 CONVERGE CLEAN) · class method/ctor DEFAULT-ARG(§4.5.90·bulletproof allow-list) · CLASS-METHOD string formal(§4.5.89·3부작 완결) · TASK string formal(§4.5.88). **G2 OBS 트랙**: OBS-1a ✅ · OBS-1b는 EXT2 체인 다음.
