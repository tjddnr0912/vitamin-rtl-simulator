...
| **2** | **§3 loud→supported** — ⭐⭐ 착수 순서는 **워크로드 코퍼스**가 정한다(§4.5.369). ✅ **남은 거절 ⑧ 이 §4.5.404 로 해소** — verilog-ethernet 이 두 오라클 다이제스트로 **2.2 s**(iverilog 7.7 s)에 돈다 ⇒ **코퍼스에 거절 0** · ⚠️ **verilog-axi 는 `Expect::Split`**(돈다 · 다이제스트는 오라클과 다르고 그 잔여는 **오라클 자기모순** = §2-N 판정)이라 제2 오라클 없이 착수 금지 ⇒ **다음 §3 착수는 코퍼스가 더 이상 정해 주지 않는다**(코퍼스 확장 = 2a) | 2-오라클 · hand-IEEE |
