...
>
> - **기준(2026-07-29)**: format_version **26** · **4909 tests green** · 3-OS CI green · MsgCode **59**(W3057) · **MSRV 1.85**.
> - **최신 완료(외부 round-19 리포트 응답 — 진단 34건 전부 RESOLVED + silent-wrong 2건)**: **§4.5.274** ①리포트 34건 중 **33건이 하나의 뿌리** — output actual 의 쓰기를 인정하는 자리가 *문장 모양*뿐이라 **호출이 값을 반환하는 순간**(`go = nxt(5,r)` · `if (nxt(5,r)==1)` · `while (n<lim && rsp_next(fd,r)==1)`) 보이지 않았다. 표현식 단위 효과 워크(`ExprDa::{Clean,Writes,Reads}`) + **분기는 조건의 값을 안다**(`a && f(r)` 이 참이면 둘 다 평가됐다 — iverilog 로 전제 4가지 실측). ②`void'(f(out r))` = §3.1 의 우회로인데 그 자체가 막혀 있었고, 문장 위치는 사실 **가장 쉬운 경우**였다. ③named arg 는 매핑이 안 쓰여 있었을 뿐 — 그걸 만들자 `emit_frame_func_out_call` 이 G10 재정렬을 **아예 안 했다**는 것이 드러났다. **silent-wrong 2건**: **R19-X1** 기본 인자 값을 caller 스코프에서 낮춰 caller 의 그림자를 집었다(vita `91` / iverilog `6`) — 같은 위험을 **클래스 메서드에서는 이미 닫아 뒀는데** 평범한 함수/태스크 쌍둥이만 안 닫혀 있었다 · **R19-X2** `$fgets`/`$fscanf`/`$sscanf`/`$fread` 가 frame body 안에서 **조용히 0**(vita `rc=0 loc=[]` / iverilog `rc=9 loc=[Len = 16]`) — §3.1 이 방금 연 `.rsp` 워커의 모양. 직전 = **§4.5.273**(외부 round-18 — 깊은 참조 워커에 타이밍 arm 이 없었다 11/12 · struct 멤버는 비트 커버리지 · `automatic` unpacked struct 의 lifetime 을 **파서가** 떨어뜨렸다 · 그 밑의 공유 넷+suspend silent-wrong).
