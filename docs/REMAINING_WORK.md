...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-03 = **format_version 19 · 2989 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(INLINE func/task md-packed subst-bound local·FRAME string-literal·string local `s[i]`/sibling-block — 전부 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-03 2-state class FIELD X→0 coercion ✅**(§4.5.86·object-heap `class_field_write`·§4.5.85 sibling·적대 R1 CLEAN) · class-method 2-state coercion(§4.5.85) · string LOCAL 관계비교(§4.5.84) · class-method md-packed(§4.5.83) · ㊂ frame-local md-packed(§4.5.82). **G2 OBS 트랙**: OBS-1a ✅ · OBS-1b는 EXT2 체인 다음.
