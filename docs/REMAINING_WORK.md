...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-05 = **format_version 19 · 3090 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(MODULE sibling-block string[read-gated]·packed-WIDTH sibling·leading-NUL frame string 등 소형 — 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-05 EXT2-H FRAME body part-select 대입 `r[7:0]=x` ✅**(§4.5.97·validate relax+engine RMW·적대 2-round[R1 3+R2 1=4 silent-wrong 발굴→수정: concat/collapsed-array/X-index/coalesce-shadow]·md-packed element-write 보너스·CONVERGE CLEAN) · `integer unsigned` PARAM/VAR(§4.5.96) · INLINE sibling-block string 충돌 loud(§4.5.95) · FRAME string FORMAL `s[i]`(§4.5.94). **string `s[i]` 완결**·**unsigned-param family 완결**·**EXT2 체인 6/11 완료**(0/A/E/C/F/H·잔여 D/B/A2c/NAP=대부분 no-oracle). **EXT2-B/D**(fn-body typedef enum·block-local automatic)=**no-oracle 재분류**(iverilog "sorry"/segfault·2026-07-05 그라운딩 정정). **G2 OBS 트랙**: OBS-1a ✅. **follow-on**: EXT2 잔여 no-oracle=hand-IEEE·function RETURN-type integer signing(E2002).
