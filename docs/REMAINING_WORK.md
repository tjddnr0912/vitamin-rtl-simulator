...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-04 = **format_version 19 · 3034 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(INLINE func/task md-packed subst-bound local·string local `s[i]`/sibling-block — 전부 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-04 class method/ctor DEFAULT-ARG drop ✅**(§4.5.90·`build_class_call`/`lower_ctor_call`이 `fill_default_args` 호출·NAME/CALL default는 scope-ambiguous→bulletproof allow-list loud-reject·적대 3-round[6 agent] CONVERGE CLEAN) · CLASS-METHOD string formal(§4.5.89·3부작 완결) · TASK string formal(§4.5.88) · FRAME 함수 string LITERAL(§4.5.87). **G2 OBS 트랙**: OBS-1a ✅ · OBS-1b는 EXT2 체인 다음.
