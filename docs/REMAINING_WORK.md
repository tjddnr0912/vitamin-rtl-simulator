...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-04 = **format_version 19 · 3046 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(string local `s[i]`/sibling-block — 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-04 INLINE 함수 md-packed local ✅**(§4.5.92·whole-truncate[local_dims full-width]+element-select bit-select[frame 라우팅/loud-reject]·적대 R1 CONVERGE CLEAN) · `int unsigned` PARAM 부호(§4.5.91) · class method/ctor DEFAULT-ARG(§4.5.90·allow-list) · CLASS-METHOD string formal(§4.5.89·3부작 완결). **G2 OBS 트랙**: OBS-1a ✅ · OBS-1b는 EXT2 체인 다음.
