...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-04 = **format_version 19 · 3070 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(MODULE sibling-block string[read-gated]·packed-WIDTH sibling·leading-NUL frame string 등 소형 — 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-04 INLINE sibling-block same-name string/packed ✅**(§4.5.95·detection-only loud-reject §S⑤·`reduce_function_body` conflicting string-ness 검출·적대 R1: soundness SOUND+differential inline CLEAN[module clause FP 발견→revert]) · FRAME string FORMAL `s[i]`(§4.5.94) · INLINE string `s[i]`(§4.5.93) · INLINE md-packed local(§4.5.92). **string `s[i]` 완결**(inline+frame formal)·**inline sibling string 충돌 loud화**. **G2 OBS 트랙**: OBS-1a ✅ · OBS-1b는 EXT2 체인 다음.
