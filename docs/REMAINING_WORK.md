...
| **2** | **§3 loud→supported** — ⭐⭐ **2026-08-23 부로 착수 순서를 워크로드 코퍼스가 정한다**(§4.5.369). 서드파티 여덟 중 **셋**이 거절되고 셋이 **상수 도메인 한 축**이다. **①(문자열 삼항)이 첫 표적** — 혼자 verilog-ethernet + serv **둘**을 열고 좁히기가 이미 끝났다(`param_str_literal` 이 `StrLit`/`Paren` 두 팔뿐 · 막힌 건 삼항 하나) → ②(`{N{…}}` = verilog-axi) → ③(`$fgetc`/`$value$plusargs` 의 blocking-only 술어). 그다음이 3판 라운드 잔여(§3.1(c)·§3.3·§3.7·§3.11) | ① 은 **2-오라클(축 자체에서)** · ②③ iverilog ✓ |
