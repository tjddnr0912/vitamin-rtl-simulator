...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-04 = **format_version 19 · 3021 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(method/ctor DEFAULT-ARG drop·INLINE func/task md-packed subst-bound local·string local `s[i]`/sibling-block — 전부 별개 경로·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-04 CLASS-METHOD string formal 2종 ✅**(§4.5.89·string-formal-in-callee 3부작 완결[func/task/method]·`str_params` slot `1+i`=this-offset·`formal_str` push·eval.rs 재료화 재사용·elaborate-only·적대 R1 CONVERGE CLEAN) · TASK string formal(§4.5.88) · FRAME 함수 string LITERAL(§4.5.87) · 2-state class FIELD X→0(§4.5.86). **G2 OBS 트랙**: OBS-1a ✅ · OBS-1b는 EXT2 체인 다음.
