...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-03 = **format_version 19 · 2823 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong 0**(잔여는 전부 honest-loud=안전 또는 신규 기능).
...
| A2b | package-level array parameter | A2a desugar를 package로 확장(+package 내 array `'{…}` init 저장) · acceptance=sha3_pkg `RC_TABLE[0:23]` repro → **§6 리포트 CLOSE** | hand-IEEE + 내부 차분 |
...
| **A2b-prereq 발굴 pre-existing**(§4.5.70 ㉶~㉺) | `always @(*)` decl-init wake(iverilog 미발화·vita-wide) · param override 비상수=W3056 warn+default(iverilog=error) · `@(p::x)` event control(iverilog ✓) · longint MIN fold 갭(package만) · 비상수 package init(iverilog ✓·v1 loud) |
