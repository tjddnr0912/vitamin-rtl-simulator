...
> **리뉴얼: 2026-07-02** (사용자 지시 재계획) · 기준 갱신 2026-07-03 = **format_version 19 · 2983 tests green · 3-OS CI green · MsgCode 58** · known **silent-wrong = §C 잔여(INLINE func/task md-packed subst-bound local·FRAME string-literal·string local `s[i]`/sibling-block·class 2-state FIELD — 전부 별개 경로/축·전용 슬라이스)**, 나머지 잔여는 honest-loud=안전. 최신 = **2026-07-03 class-method 2-state formal/return/local X→0 coercion ✅**(§4.5.85·frame `intro_kind` parity·적대 R1 CLEAN) · string LOCAL 관계비교(§4.5.84) · class-method md-packed(§4.5.83) · ㊂ frame-local md-packed(§4.5.82·3 site) · string-formal 관계비교(§4.5.81). **G2 OBS 트랙**: OBS-1a ✅ · OBS-1b는 EXT2 체인 다음.
...
| 항목 | 잔여 내용 |
|---|---|
| **🔴 silent-wrong 잔여**(EXT2-C 적대리뷰 발굴·pre-existing·main 재현·**다음 우선순위 ①**) | ~~㊀ narrow-actual INLINE 함수 인자 width-extend~~ **✅ 수정(§4.5.78)** · ~~㊁ INLINE(static) 함수 body context-width propagation 부재~~ **✅ 수정(§4.5.80, 2946 green, 적대 4-round CLEAN)**: frame-routing(`body_needs_context_width`=IEEE §11.6 per-node CW 모델·`ctx_widening_below`)+always_comb 감도 수정(`collect_expr_reads` Call args 하강)+`body_reads_only_locals` 게이트(global-reader는 inline 유지=회귀 0). **잔여 follow-on(별개·pre-existing)**: (a) global-reading widening 함수는 width fix 미수혜(inline 유지=main·frame 감도 완전수정=callee body reads 기여 시 해소) (b) size-cast `16'(a*b)` context 미확립(module scope도 wrong) (c) signed `>>>` unsigned-context=logical shift(inline shift lowering signedness) (d) inline-call return 미truncate(`[7:0] g`→0x1FE) · ~~string 함수 formal 관계비교(`a<b`) packed 오답~~ **✅ 수정(§4.5.81, 2952 green, 적대 R1 CLEAN)**: `formal_str` 스택으로 formal 선언-string 추적·`expr_is_string_ast` 최우선 조회(FORMAL 선언타입 keyed=over-trigger 방지)→INLINE+literal·FRAME+var·bit-return-frame 수정. **잔여 follow-on(별개·pre-existing·branch==main=무회귀)**: (a) **FRAME+string LITERAL**=엔진 string-lit→width-1 String frame-slot materialization 버그(detection은 맞으나 slot 값 손상) (b) ~~함수 body 내 두 string LOCAL 비교~~ **✅ 수정(§4.5.84·inline subst-bound resize 손상=handle-guard+formal_str)** (c) TASK string formal(inline=`__taskarg_` local 갭·frame=clean mirror 미적용) (d) string RETURN 타입=E2002 parse-loud·string method on formal=E3009 loud · ~~㊂ md-packed frame-LOCAL element-select~~ **✅ 수정(§4.5.82, 2964 green, 적대 R2 CLEAN)**: frame func/task/block-local 3 site를 full packed width+`packed_dims`+`dim_desc`로 등록(module-scope 미러·`frame_packed_width`/`register_frame_packed` 헬퍼·`compute_dim_desc` 분리로 4-state 비-coerce). **잔여 follow-on(별개 경로·pre-existing·branch==main·각 `range_to_dims`-truncate)**: ~~class method(`reserve_class_method`)~~ **✅ 수정(§4.5.83)** · INLINE func/task body-local **md-packed** element-select(`fold_straight_line`/`hoist_inline_task_locals`·subst=net 아님→dims 미보유·string local은 §4.5.84로 수정·md-packed는 deep 잔여) · packed+UNPACKED frame local($bits가 unpacked dim 무시) · ~~class-method 2-state(`bit`) local X→0 coercion 부재~~ **✅ 수정(§4.5.85·frame intro_kind parity·formal/return/local)** · inline string local **element-select `s[i]`**·sibling-block same-name string/packed(§4.5.84 발굴) · **2-state class FIELD X coercion 부재**(§4.5.85 발굴·field=object heap·method slot 아님·별개 경로) |
