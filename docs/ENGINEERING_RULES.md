...
- **테스트 하네스가 사이드카를 설치하는지 확인하라 — 미설치면 그 규칙을 시험하는 테스트가 다른 것을 재고 있다.** §4.5.299~302 에서 **세 번** 나왔다: `final_procs` 없으면 `final` 거부 행이 도달 불가 · `wired_and_nets` 없으면 `wand` 가 평범한 multi-driver 로 읽힌다 · `ca_delays` 없으면 rise/fall 테스트가 **균일 지연을 재고 있다**. 규칙이 사이드카에 실려 있으면(elaborate 가 `Display`+표, `assign #d`+표 식으로 낮추는 모든 것) 테스트 빌더에 그 표를 넣기 전까지는 **그 규칙을 시험할 수 없다**.
