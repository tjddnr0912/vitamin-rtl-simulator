...
| Treat a self-determined position (cast operand, concat part, comparison operand, shift count, exponent, select index, system-function argument) as its own context: fold its inner at the position's self width, never at ctx 0, and push §11.8.2's expression sign into every context-determined operand. A routing arm that reaches a shared walk with neither property excludes by STRUCTURE — leaf-only positions, sign-homogeneous trees — never by a value-aware walk that mirrors the shared walk's arms | A ctx-0 fold computing a narrower context-determined operator at its own width, and a mirror walk that re-derives the shared walk's cases missing one axis per round (§4.5.527: three same-axis BLOCKINGs in three rounds — nested operands, operators inside positions, the sign inside positions — until both value-aware exclusions were deleted for one structural rule) |
