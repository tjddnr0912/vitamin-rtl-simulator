...
| **picorv32** (YosysHQ) | cpu | ISC | 6.682 s | **4.512 s** | **1.48×** |
...
> 기하평균 **1.30×** (도는 일곱) · **1.61×** (서드파티 다섯만). 측정 = 릴리즈 바이너리,
...
⭐ **first-party 둘을 빼면 1.30 → 1.61 로 올라간다.** 우리가 쓴 두 설계가 평균을 *끌어내리고*
...
- **10 중 3 이 거절**이다. 이 숫자가 이 코퍼스의 존재 이유이고, §3 ①②가 닫히면 **10/10** 이 된다.
...
- **모양이 넷**(cpu·crypto·stream·fabric)인데 stream 과 fabric 이 **각각 하나**뿐이고 둘 다
  거절 상태다. 즉 지금 도는 일곱은 전부 cpu 아니면 crypto 다 — 이 편향은 §3 ①② 가 닫히기
  전까지 남는다.
