...
| **M1** 조합 깊이 | **cyc**(순환) — 진짜 깊이 산출 불가 | ❌ **FAIL** |
...
⚠️ **M1 의 FAIL 은 설계가 아니라 내 분석의 한계일 수 있다.** 랭크 그래프가 **넷 단위**라, 비트 단위로는 비순환인 의존이 넷 단위에서 순환으로 보인다(레지스터 파일·넓은 버스가 있는 CPU 에서 흔하다). 실제 조합 루프인지 분석 조도의 문제인지는 **미확인**.

⇒ **3b 는 이 설계에서 근거를 얻지 못했다**(융합할 게 4개). 3a 또는 ②(계약 파기) 재검토 대상.
