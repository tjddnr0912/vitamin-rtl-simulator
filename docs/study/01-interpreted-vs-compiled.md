...
## 8장 · ⚠️ 완성본에 반드시 들어가야 할 절 (독자 요청)

> **`[TBD-P2]` — 이 절은 완성본에서 반드시 자세히 쓴다. 아래는 그 요구사항 메모다.**

### 8.1 "고르게 퍼져 있다 · 단일 레버가 없다 · 전부 4-state 비트 조작이다"를 처음부터 설명할 것

본 초안 §4~§6 이 이 문장을 결론으로 던지고 지나갔는데, **배경지식 없이는 이해할 수 없는 문장**이다.
완성본에서는 다음을 순서대로, 하나도 건너뛰지 말고 설명한다.

1. **4-state 가 무엇이고 왜 두 개의 평면(`val`/`unk`)으로 저장하는가**
   — `0/1/x/z` 를 비트당 2비트로 인코딩하는 방식, 왜 `x` 와 `z` 가 둘 다 필요한가.
2. **그 인코딩이 모든 연산을 어떻게 바꾸는가** — 2-state 라면 AND 가 기계어 `&` 하나인데,
   4-state 에서는 known-0/known-1 을 따로 계산해야 하므로 **연산 하나가 여러 개**가 된다.
   실제 공식(`and_w` 등)을 그대로 놓고 연산 수를 세어 보인다.
3. **`mask_top` / `resize` / `from_packed` / net 읽기가 각각 무슨 일을 하며 왜 두 배인가**
   — 평면이 둘이니 마스킹도 둘, 복사도 둘, 저장도 둘.
4. **프로파일을 어떻게 읽는가** — "함수 X 가 16%" 가 "함수 X 를 고치면 16% 빨라진다"가
   **아닌** 이유. 심볼 귀속이 인라인된 코드까지 모은다는 함정(§4.5.278 의 교훈)과,
   실제로 두 번 시도해 두 번 다 ~0/~1% 가 나온 기록.
5. **"단일 레버가 없다"의 정확한 뜻** — 비용이 한 함수에 몰려 있으면 그것을 고치면 되지만,
   `eval 26.7 / resize 16.6 / netread 13.1 / mask_top 13.0 / eval_binary 12.5` 처럼
   퍼져 있으면 **어느 하나를 없애도 상한이 작다**(Amdahl). 숫자로 계산해 보인다.
6. **그래서 왜 `unk` 평면 제거(2-state)가 유일한 구조적 레버인가** — 위 다섯 항목이
   *전부 같은 원인을 공유*하므로, 그 원인을 없애면 다섯이 동시에 줄어든다.
   이것이 "레버"의 정의다.
7. **그 대가로 무엇을 잃는가** — X-optimism, 초기화 버그 은폐, 사인오프 부적합.
   Verilator 가 왜 그 거래를 했고 왜 아무도 버그라 부르지 않는가.

### 8.2 그 외 완성본 항목

- Phase 2 구현 결과 · 실측
- 이 세션의 성능 작업 총정산표
- Phase 1 이 실패했을 때의 대안 기록(3a / ②)

## 9장 · Phase 2 결과 `[TBD-P2]`
