...
- RESOLVED §4.5.458: 평범한 `[N:0]` 벡터의 비트/부분선택이 좁은 SIGNED 인덱스를 무부호로 읽었다. 뿌리는 `norm_offset_for_net` 의 `lsb == 0` arm — 뺄 것이 없어 인덱스 식을 **그대로** 넘겼고, 엔진은 인덱스를 `to_u64` 로 읽는다. 상수만이 아니라 signed 넷·상수식도 같았고, 배열 원소 비트선택(`mem[0][-2'sd1]`)도 같은 퍼널이라 같이 풀렸다. 224칸(8 넷모양 × 28 인덱스 × read/part/write) PRE/POST/iverilog: **42 fixed · 0 regressed · 224/224 iverilog 일치**. ⚠️ 보이는 대역은 "무부호 읽기가 넷 안에 우연히 떨어지는 곳" — 8비트 넷이면 폭 2·3 뿐, 64비트면 2~6. 한 폭만 쓸면 0건이 나온다.
- RESOLVED §4.5.459: **0-LSB 파라미터**와 **packed 원소의 residual dim** 도 같은 봉인을 받는다 — `localparam logic [7:0] K = 8'b1010_0101; K[-2'sd1]` vita `0` / iverilog `x`. 자리는 둘: `norm_offset_for_range` 의 descending `lo == 0` arm(넷 쪽과 같은 verbatim 반환 · `param_range` 그룹 1·2 와 packed residual 이 전부 여기로 온다) + `norm_offset_if_net` 의 최종 fall-through(`param_range` 항목이 없는 untyped-EXPRESSION 파라미터). 72칸 band(8 컨테이너 × 9 폭) **20 fixed · 0 regressed · 72/72 iverilog 일치** + 14 shape/form 칸. ⚠️ 줄이 주장한 "elaborate fold 소비자까지 blast radius" 는 **반박됨** — fold 레인은 이미 정직-loud 다(`localparam L = K[-2'sd1]` = E3009, 64비트에서 무부호 읽기가 범위 안인 칸도 decline). 범위는 런타임 lowering 뿐이었다.
...
- NEW(pre-existing · §4.5.459 grounding · 다른 클래스): untyped-EXPRESSION 파라미터가 자기 폭을 32 로 넓힌다 — `localparam E = ~8'h5A;` 가 vita `ffffffa5` / 두 오라클 `a5`. 봉인과 무관(POST 는 인덱스를 옳게 답한다)이지만 E 의 가시 대역을 폭 2~3 에서 2~5 로 넓힌다.
...
- RESOLVED §4.5.457: 사이즈드 음수 리터럴 `#(-4'd1)`·`#(-(4'd1))`·`#(-8'sd1)` 이 자기 폭에서 접힌다(15·15·255, 두 오라클). >64비트·x/z 리터럴은 이전 fold 로 FALL BACK — decline 하면 `#(-128'd1)` 이 지연 없이 발화한다(iverilog 는 아예 발화 안 함, 측정).
...
- `TimeLit` RESOLVED §4.5.458 — 식 안의 타임 리터럴(`#(2.5ns + 1ns)` = 3.5 ns, `#((2.5ns))`, `#(2500ps + 1000ps)`, `#(2.5ns/2)`, `#(3ns/2)`, ternary, gate prim, net delay). 215칸(43 철자 × 5 timescale) PRE/POST/iverilog 측정: **81 fixed · 0 regressed**. ⚠️ 클래스 이름이 틀렸었다 — "REAL 타임 리터럴" 이 아니라 **모듈단위 값이 분수**인 것이다(`2500ps + 1000ps` 엔 real 이 없고 `1ns + 1ns` 도 `10ns/1ns` 에선 0 이었다). ⚠️ 반올림은 **모듈 시간단위가 아니라 정밀도**에서 한 번, 완성된 값에. §4.5.457 의 산문이 반대로 적혀 있었다(`$time` 프로브가 모듈단위로 반올림해 자기 반올림을 오라클 답으로 읽었다 — `#(25ns)`@`10ns/1ns` 는 30 이 아니라 25 ns). PROCEDURAL `#(2.5ns) stmt;` 는 정직한 loud(E3009 · §3) 로 유지.
- ⓑ RESOLVED §4.5.459: 리터럴의 **단위**가 설계 정밀도보다 미세하지만 **값**은 아닌 것 — `assign #(2500ps)`@`1ns/1ns` 두 오라클 3 ns / vita 무지연. 뿌리는 `e < 0` decline 이고 **복사본이 셋**이었다(`delay_ticks_in_scope` 의 bare arm · `delay_units_in_scope` 의 식 leaf · `const_fn.rs::const_eval_in_scope`). 앞의 둘을 고쳤다: sub-precision-UNIT leaf 는 **잎에서** 정밀도로 반올림(`subprec_unit_ticks`)하고 나머지는 그대로. 102칸(6 timescale × 17 철자) PRE/POST/두 오라클: **26 fixed · 0 regressed** · rise/fall 사이드카 · net-decl · gate prim 네 자리 전부 두 오라클 일치. 진짜 sub-precision(`#(2.5ps)`·`#(0.4ns)`@`1ns/1ns`)은 같은 산술로 0 틱이 되고 `fold_ca_delay` 의 zero-rise 필터가 `None` 을 유지해 byte-identical.
...
- ⓒ 음수 지연 `#(1ns - 5ns)`: 두 오라클 발화 안 함 / PRE·POST 둘 다 즉시 발화(`real_delay_ticks` 가 0 으로 clamp). 안 발화를 표현하려면 tick 필드가 필요 ⇒ frozen 타입.
- ⓓ NEW(§3 · §4.5.459 grounding · 9칸): PROCEDURAL/intra-assign/`always` 레인은 세 번째 복사본(`const_fn.rs` 의 `e < 0`)에 걸려 **false-LOUD** 다 — `#(2500ps);`@`1ns/1ns` 가 E3009, 두 오라클 3 ns. 그중 3칸(`#(2.5ps)`)은 두 오라클이 0 이라 구조 레인이 이미 맞는 값을 낸다. 자리는 SHARED evaluator 이므로 opt-in 파라미터가 선행조건(ER "shared machinery").
...
|---|---|---|---|---|
| 1 | 1 | §3 ⑤ⓕ residue after §4.5.459: a `localparam` OF an unpacked-array type (1-oracle, cheap — route `typedef_param_shape` into the existing `ConstArrayVar` channel), then the dim-COUNT-changing override (needs a per-instance declarator arity, which the parse-time stamp cannot give — measure whether `monomorph` can before pricing). The function RETURN type stays do-not-start (1-oracle + frozen `FunctionDef`) | §3 ⑤ⓕ | ② |
| 2 | 2 | §2 "Delays / events" ⓓ, opened by §4.5.459's own grounding: the PROCEDURAL / intra-assign / `always` delay lane is FALSE-LOUD on the class §4.5.459 just fixed for the structural one — `#(2500ps);` under `1ns/1ns` is E3009 where both oracles delay 3 ns. **9 measured cells, 2-oracle**, and 3 of them are cells the structural lane already answers correctly. Root = the THIRD copy of the `e < 0` decline, in `const_fn.rs::const_eval_in_scope`'s `TimeLit` arm. ⚠️ That is a SHARED evaluator (indices, bounds, widths all reach it), so the fix is an opt-in delay parameter, never a widening of the arm itself — ENGINEERING_RULES "shared machinery". No format bump | §2 Delays | ① |
| 3 | 3 | §2 "Index sealing" residue, found by §4.5.459's own grounding in a DIFFERENT class: an untyped-EXPRESSION parameter takes a 32-bit self width where both oracles take the RHS's — `localparam E = ~8'h5A;` prints `ffffffa5` in vita, `a5` in both. It is why `E`'s visible index band is widths 2..5 instead of 2..3. Site = `param_decl_width_opt`'s value inference (the group-3 provenance `param_range` deliberately declines). 2-oracle; measure the `$bits(E)` and concat twins before sizing | §2 | ① |
