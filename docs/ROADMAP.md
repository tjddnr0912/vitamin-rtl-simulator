...
- **🔴 사이즈 캐스트 `N'(expr)` 가 `%`·`/` 위에서 조용히 갈린다**(pre-existing·PRE==POST·세 백엔드 동일·**오라클 ✓**·§4.5.315 soundness 부수 발굴). `integer k = 7;` 에서 `2'(k%4)` 가 vita **`xx`** / iverilog `11`, `3'(k%4)` 가 **`111`** / `011`, `2'(k/2)` 가 **`00`** / `11` — **exit 0·진단 없음**. `2'(k)`·`2'(k&3)`·`2'(k+1)`·`4'(k%4)` 와 상수 피연산자 캐스트는 전부 일치하므로 갭은 **캐스트 폭이 `%`/`/` 안으로 문맥 폭처럼 밀려드는 것**으로 읽힌다(4 를 2비트로 자르면 0 → mod-by-zero → x). IEEE 해석부터 핀해야 하는 전용 슬라이스.
