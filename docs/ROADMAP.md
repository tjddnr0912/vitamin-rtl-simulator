...
| **4** | **성능 — 스케줄러 축**(⚠️ **2026-08-20 재측정으로 축소**) | ⚠️ **"29%" 의 절반은 스케줄러가 아니다** — 그 버킷의 `dispatch_body` 8.31% 는 **문장 실행기**, `settle_cont_assigns` 8.29% 는 **cont-assign 평가+쓰기가 인라인된 것**이다. 진짜 스케줄러 오버헤드 ≈ **12%**. settle 의 빈 패스(패스의 61%)는 **공짜다**(등가 early-out 실측 0%) | ⭐ 첫 표적으로 적혀 있던 `propagate` 의 델타당 `Vec` 셋은 **여전히 유효**하나 상한이 12% 안이다 |
