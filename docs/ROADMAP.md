...
> | 25 | 🆕 **An override does not retype an UNTYPED parameter whose default is a SIZED LITERAL** (§2 row 14 review · **2-oracle** · silent-wrong · pre-existing, PRE == POST) | `module leaf #(parameter P = 8'sd1);` instantiated `#(.P(32'hDEADBEEF))` is `P=deadbeef` with `$bits(P)` **32** in iverilog AND verilator; vita says `P=ef`, `$bits` **8**, everywhere — the override is truncated to the DECLARED DEFAULT's literal width. §6.20.2 gives an untyped parameter the type of its final override, so the sized literal in the default is not a fact about the parameter once one arrives. ⭐ **This is not row 17.** That row is an oracle SPLIT about how a >64-bit target extends a narrow override; here both oracles agree and the value is truncated before any extension question arises. ⚠️ It is also the reason `param_decl_width_declared`'s *"sized literal"* group is safe for a `localparam` (no override channel) and not for a `parameter` — the two callers of that width should be asking different questions, and `param_decl_width_declared_overridden` already exists for the second. ⚠️ §2 row 14's fix made the two spellings of one expression AGREE on the truncated value (they used to answer `ffffffffffffffef` and `00000000000000ef` in one design), which is a rung, not a resolution.
...
> | 29 | 🔄 **RE-GROUNDED 2026-09-02 — THE HEADLINE WAS BACKWARDS. A size cast fails to apply ITS OWN width DOWN into a context-determined operand** (**2-oracle** · **174/266 silent-wrong · 0 loud · 0 oracle-split**) | `logic [63:0] A = 64'(~(W32 + 32'd1));` → vita `000000002152410f`, both oracles `ffffffff2152410f`, exit 0. ⭐ **One-token discriminator**: `64'({W32} + 32'd1)` is CORRECT — a pair of braces makes `ast_ctx_signed` answer `Some(false)` instead of `None`. ⚠️⚠️ **The recorded cause is refuted and inverted.** The row said the outer context passes THROUGH the cast; measured with a discriminating shape (`64'((~w) >> 4)` into a 64-bit and a 128-bit destination) vita gives the same correct answer in both — **the seal works** — and if the outer width did leak, truncation would land on the CORRECT value, so the stated mechanism cannot produce the stated symptom. ⭐ **The trigger is the LEAF, not the context**: with a runtime NET leaf 0/56 cells are wrong; it fires only where a leaf's sign is unresolvable at AST level (15 of 19 leaf kinds). The row was un-searchable by its own symptom. ⭐ Site = `expr_size_ctx.rs:55 ast_ctx_signed`, private, **two** non-recursive call sites (both in `expr_cast.rs`, neither needs an edit), ~30–50 product lines. Three reference implementations in tree (`expr_self_signed` resolves 9 of 11 leaf kinds on lowered IR; `arm_coercion_info`; `const_signed_env`), and `size_cast_context_width.rs:350` already pins the gap in its own docstring. ⭐ Corpus demand measured in vendored RTL: `Dw'(` **256**, plus `DepthW'(` 23 · `PtrW'(` 19 · `MuBi4Width'(` 21 · `Width'(` 14 — and the `NAME'(…)` arm is equally broken. ⚠️ Traps: the `None` fallback is load-bearing in BOTH directions (the same file records 170/1,560 cells regressed by answering `None` where `Some` was possible, and a WRONG `Some` is §4.5.393's blocking shape) · **exclude `real` params** (needs the tree-wide AST self-width pass) · **Call/hier is a SEPARATE slice** — `expr_self_signed`'s `_ => false` is its own oracle-free silent-wrong with **21 callers**, so folding it in trades a 2-site blast radius for a 21-caller one. |
...
| 게이트 | **6,438 tests green** · 워크로드 코퍼스 **10/10 · 거절 0** · no-oracle 축 green · clippy 0 · fmt 0 · format_version **29** · MsgCode **68** (2026-09-02 · §4.5.404) |

### 다음 후보 — 우선순위 순

| 순위 | 트랙 | 왜 여기 | 착수 조건 / 첫 걸음 |
|---|---|---|---|
| **1** | **정확성 큐 — §2 silent-wrong 잔여** | 이 저장소의 **최상위 원칙**이 정확성이고 성능 축은 수확 체감에 도달했다 | ⚠️ **§2 를 위에서부터 읽지 마라** — 그 절은 주제별 묶음이지 착수 순서가 아니다. **2026-09-02 재그라운딩(8 에이전트 · 전 항목 HEAD 3-툴)이 정한 순서**: **row 29**(size cast 가 제 폭을 못 내린다 · **174/266 silent** · 코퍼스 수요 `Dw'(` 256회) → **row 30**(unsized fill 이 32비트에서 접힌다 · **70/140** · ~5줄) → **§3 ⑦**(선언 바운드 리덕션 · **12 silent** · 3~4줄 · ⚠️ RESOLVED 로 적혀 있었다) → **row 33**(whole-net copy 가 한 델타 stale · **41/63** · 신규 클래스). ⚠️⚠️ **여덟 줄을 재측정했더니 여덟 줄 다 모양이 바뀌었고 셋은 severity 등급이 바뀌었다** — 착수 첫 행동은 구현이 아니라 census 다 |
