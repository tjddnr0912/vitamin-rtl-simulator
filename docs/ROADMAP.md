...
- **🔴🔴 선언 범위 정규화가 무부호 32비트로 낮춰져 두 갈래로 조용히 틀린다**(pre-existing·세 백엔드 동일·오라클 ✓·§4.5.307 differential 발굴·**우선순위 ①**). 뿌리는 한 곳 — `elaborate/src/packed.rs::norm_offset_for_range` 가 정규화를 IR 의 **무부호 32비트 `Sub`** 로 emit 한다: ⓐ **하강 넷의 비-0 LSB**(`reg [33:2] bus`)에서 `Sub(idx, lsb)` 가 LSB 아래 인덱스에 대해 **랩**한다 — `bus[1 +: 2]` 가 iverilog `00000001`(범위 안 비트만 부분 쓰기 = 그 함수 주석이 인용하는 P0-IPU 규칙) 인데 vita 는 **쓰기를 통째로 버린다**(`00000000`·exit 0·진단 없음). 0-LSB 넷의 같은 형태(`a[-1 +: 4]`)는 **정상**이라, 경계가 정확히 "base < 선언 LSB" 다. ⓑ **상승 넷**(`reg [0:31] dn`)에서 `Sub(hi_const32, idx)` 가 **사용자 인덱스를 32비트로 넓혀** 문맥 결정 연산자의 값을 바꾼다 — `dn[~r5]`(r5 5비트)가 iverilog `00040000` / vita `00000000`, `dn[r5-5'd1]`·`dn[r5+5'd2]` 도 같은 이유로 갈리고 **읽기도 같이 틀린다**(`dn[~r5]` 가 `x`). 폭 민감 연산자가 없으면(`dn[r5]`) 정상. **fix 형태는 기록된 규칙 그대로** — 사용자 인덱스를 **자기 폭에서 봉인**한 뒤(concat 등) 정규화 산술을 **부호 있는 도메인**에서 해야 한다("변환은 leaf 가 아니라 문맥 경계에서"). ⚠️ 이 함수는 세 백엔드가 공유하는 **elaborate 낮추기**라 읽기·쓰기·방향·폭 전 축의 3-way 스윕과 골든 재확인이 필요한 **전용 슬라이스**다(§4.5.307 은 그 위에 특수화를 얹었을 뿐 이 값을 만들지 않았다 — PRE 동일 실측).
