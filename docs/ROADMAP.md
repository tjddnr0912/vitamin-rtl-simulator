...
- ~~🔴 inner NET 이 outer PARAM 을 shadow 못 함~~ **RESOLVED**(§4.5.246) — 근인은 `lower_expr` 의 fall-through 가 `lookup_scoped`(params 전용 walk)를 불러 방금 도출한 innermost 키를 무시한 것. **잔여**: enum-label 및 package 변수 clobber(형제 항목·아래 별도 줄).
