...
- **접히지 않은 상수식 인덱스는 진단을 잃는다**(§4.5.310 이 경계를 의도적으로 얕게 뒀다·오라클 갈림): `index_is_compile_time_constant` 는 `Const`·`$signed`/`$unsigned`·단항 부호까지만 본다. 그보다 복잡한 상수식(`$unsigned(-32'sd3 + 1)` 등)은 런타임 인덱스로 분류돼 **32비트로 랩**하므로 값은 verilator 와 같지만 iverilog 가 내는 out-of-range 경고가 사라진다. 깊이를 늘리는 대신 **elaborate 의 정본 상수 접기를 IR 표현식에 대해 부를 수 있게** 만드는 것이 옳은 해소다(오늘은 AST 전용).
