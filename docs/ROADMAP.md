...
> | 8b | 🆕 **The clocking-input read-only rule has ≥8 write funnels and 3 are covered** (§2 row 8 의 적대 리뷰 census · **no oracle** — hand-IEEE §14.3 · **pre-existing, PRE == POST**) | 측정된 잔여: `$fgets`·`$fread`·`$fscanf`·`$sscanf`·`$value$plusargs`·`$swrite`/`$sformat` 이 전부 `cb.sig` 를 인자로 받아 **홀딩 넷을 exit 0 으로 쓴다**(초기 55 → 68/07/09/39 등, 진단 없음). 그리고 `$readmem*` 자신의 **계층 철자**(`dut.cb.mem`)도 그렇다 — `deny_clocking_input_arg` 의 `lookup_net_scoped` 가 그 경로를 풀지 못하는데 **lvalue 쌍둥이는 잡는다**(`u.cb.s = 8'h55;` 는 E3009). ⇒ 두 resolver 가 같은 질문에 다르게 답한다. ⭐ verilator 는 이 축에서 **accept/reject 오라클**이다(자기 `Cannot write to input clockvar` 를 낸다) — x/z 가 아니라 합법성 질문이므로. ⚠️ 곁수확: `$readmem` 이 clocking **OUTPUT** 을 쓰면 **native 와 vm 이 다른 답**을 낸다(`00 bb cc dd` vs `00 00 00 00`, PRE 도 동일) — 1,572 설계 스윕에서 유일한 백엔드 분열.
...
> | 11 | 🆕 **A module-scope ENUM LABEL is invisible to a body `localparam`** (§4.5.384 census · **2-oracle** · loud, not silent · **재측정 2026-09-01: 산다**) | `typedef enum logic [31:0] { EA = 32'hAB34 } e_t; localparam logic [31:0] Q = EA;` 은 `E3009` *"undefined name `EA`"* 이고 두 오라클은 `Q=43828`, 그 라벨로 폭을 잡은 net 은 `52` 다. ⚠️ 프로브 주의: `localparam int Q = EA;` 로 쓰면 **iverilog 가 assertion 으로 죽는다**(`netlist.cc:2327`) — unsigned 타깃으로 물어야 오라클이 답한다. ⭐ 원인은 select 가 아니라 **PHASE ORDER**: body 파라미터는 (3b), 모듈 enum 라벨은 (3c) 에서 바인딩된다. ⚠️⚠️ **착수 함정이 구체적이다** — 라벨을 (3b) 의 decl-order walk 로 옮기면 *지금은 접히는* `typedef enum { X = LATER_PARAM }`(나중 파라미터를 참조하는 라벨)이 loud 가 되고, 반대로 (3b) 시작에 전부 pre-bind 하면 **두 오라클이 거부하는 전방 참조를 조용히 받는다**(§2 표 12 가 방금 고친 바로 그 실수). ⇒ 선행조건 = **(3b) 안에서 라벨과 파라미터를 하나의 decl-order 목록으로 병합**하는 것이지, 어느 한쪽을 먼저 도는 것이 아니다.
