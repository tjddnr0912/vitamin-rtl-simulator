...
- **테스트 0건 스펙 잔여 후보**(§4.5.237 스캔): `$sscanf` · 파일 위치 함수군(`$fseek`/`$ftell`/`$rewind`/`$feof`/`$ferror`/`$ungetc`) · `$typename` · 포맷 `%u`/`%z`/`%l`. 오라클 미지원 영역은 차분이 침묵하므로 **결함이 있어도 안 보인다**(§4.5.236 선례).
