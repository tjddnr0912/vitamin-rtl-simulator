...
> | 1e | 🔽 **§4.5.357 로 20/24 축소 — 남은 것은 "RHS 가 net 을 읽는" 자리뿐**(PRE==POST) | §4.5.355 는 **bare fill** 만 지연했는데, 진짜 제약은 모양이 아니라 **스코프**였다 ⇒ `scope_free_fill_expr`(리터럴과 그 위의 연산자만)로 넓혀 `u.w64='1+1`·`-'1`·`~'1`·`'1<<0`·`'1&8'hF0` 등 **20칸**을 닫았다(70칸 census · bad 0) · ⚠️ **두 번째 아키텍처 제약을 만났다**: expr arena 는 **post-order** 라(`sim-ir/selfwidth.rs` 가 패닉) 나중에 지은 부분식을 앞선 슬롯에 복사할 수 없다 ⇒ **문장의 rhs 참조**를 새 id 로 돌린다 · **남은 4칸 = `u.a = c ? '1 : 1'b0`**(RHS 가 net 을 읽어 재-lower 에 lowering 스코프가 필요) — **선행조건은 그대로**: resolve 시점에 쓸 수 있는 **스코프 스냅샷** |
...
> | 2b | 🆕 **`$itor(-<unsigned>)` 에서 두 오라클이 갈린다**(§4.5.358 실측 · **PRE==POST**) | iverilog 는 **−8**(부호 있는 해석), verilator 는 unsigned 값 — vita 는 verilator 편. 5칸(32비트·64비트 타입) · **2-오라클 합의가 없으므로 착수 불가**, 먼저 필요한 건 §20.5 hand-IEEE 판정 |
