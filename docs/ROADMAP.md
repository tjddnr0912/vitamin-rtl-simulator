...
> | 14 | 🔄 **REWRITTEN 2026-09-01 — the fold's sign rule is CORRECT; the operand's declared width is what is missing** (was: *"the constant domain sign-extends across a mixed-sign operand"*) | Minimal repro, four lines, **no enum and no package** — both of which the old wording made load-bearing: `localparam logic signed [7:0] NM = -8'sd2; localparam logic [63:0] X = NM ^ 64'h0;` is `00000000000000fe` in iverilog AND verilator, `fffffffffffffffe` in vita. ⭐⭐ **vita contradicts itself, so no oracle is needed to prove it**: the same expression in an `initial` block is `00…fe`, and the same expression over a FUNCTION LOCAL (`logic signed [7:0] NM;` inside an `automatic` function) folds to `00…fe` too. ⭐ That last cell is the diagnosis: `const_fn_width.rs`'s binary arm sizes the operand pair as `max(self_width(lhs), self_width(rhs))` and takes `cs = signed(lhs) && signed(rhs)` — which is §11.8.2, exactly right, and demonstrably works when the width is known. A function local's width is in `envw`; a module-scope `localparam`'s is not, so the pair sizes to **0** and the conversion never runs. ⚠️ Every width diverges the same way (16/32/33/64/65/66/97/128), which is the signature of "unsized", not of a boundary bug. ⭐ **The literal spelling is not the same question**: `(-8'sd2) ^ 64'h0` is `ff…fe` in ALL THREE tools — a self-determined signed literal really does keep its sign — so any fix must move the NAME and leave the literal alone. ⇒ **prerequisite = declared width/sign provenance for a parameter name in the constant domain**, the wall §4.5.371/373/382 already stopped at ([[param-values-not-canonical-at-their-claimed-width]]). **This is the fourth arrival; it is one infra item, not four queue lines.** Do not take it as a sign-rule fix.
...
> | 21 | 🆕 **A `**` (and any context-determined top) cannot be folded at a width WIDER than its operands** (round-34 R3 residue · **2-oracle** · loud · **재측정 2026-09-01: 산다**) | `localparam bit [127:0] C = 3 ** 41;` 은 loud 이고 두 오라클은 `…1fa2a1cf67b5fb863` 를, `localparam longint D = 3 ** 40;` 도 같이 답한다. §11.6.1 상 `**` 는 폭을 **문맥**에서 받으므로 밑수를 128/64 비트에서 평가해야 하는데 wide fold 는 **피연산자 제 폭에서 접고 나서 확장**한다 — size cast 를 loud 로 유지하는 그 규칙과 같다. ⚠️ 이것과 표 16 은 **같은 선행조건**(대상의 선언 폭에서 override/피연산자를 접기)이고, 그 폭은 §2 표 14 가 멈춘 provenance 벽 위에 있다 ⇒ 세 줄이 아니라 **하나의 인프라 항목**이다.
