...
> | 12 | 🔄 **RE-MEASURED 2026-09-01 — the "two answers" half is GONE; what is left is a missing check** (§4.5.384 · 2-oracle · **loud gap, no longer silent-wrong**) | `module top #(parameter W = 32); typedef enum logic [W-1:0] { EA = 32'hAB34 } e_t;` with `-GW=8` still exits 0 where iverilog (*"value that is too large"*) and verilator (`ENUMITEMWIDTH`) both REJECT. ⚠️ But the recorded symptom — *"the bare name reads the raw 0xAB34 while a select reads the truncated view, two answers to one label in one run, `$bits` 43828 vs 52"* — **no longer reproduces**: vita now prints `bare=52 bits=8 sel=4`, one consistent truncated view. §4.5.384's `enum_base_range` closed that half. ⇒ this dropped a rung on the ladder (silent-wrong → accepts an illegal design with a defensible answer) and should be ranked BELOW rows 13/14. Fix shape unchanged: the check is in the PARSER and needs a bare-literal bound; it belongs at ELABORATE, where the base range and the label value are both folded. ⚠️ Closing it makes row 13's `|| v < 0` clause reachable by a consumer that trusts the pair, so the two are ordered.
> | 13 | 🆕 **A negative label in an UNSIGNED enum base keeps its sign** (§4.5.384 Lens A · **2-oracle** · silent-wrong · ⭐ **re-censused 2026-09-01: ONE cell, and it is enum-specific**) | `typedef enum logic [7:0] { NM = -8'sd2 } e_t;` then `%0d` is **-2** in vita and **254** in both oracles. ⭐ A 64-cell census (8 declaration forms × 9 consumers, module scope AND package scope) moves ONLY this one: `%h`, every select, `$bits`, a comparison, a concat and all five wide binary operators agree everywhere, and the `localparam logic [7:0] NM = -8'sd2;` twin — same base, same value, same consumers — is **correct in vita**. So the earlier framing (*"the counter-example to a label's value being canonical at its declared width"*) is bigger than what reproduces: the width provenance is right and only `%0d`'s sign is wrong. ⭐ **Site named**: `net_util.rs:527` `param_meta.insert(key, (w, *signed || v < 0))` — the enum-label binder. ⚠️ `instance.rs:468` carries the SAME `|| v < 0` clause for parameters and the parameter twin is correct, so removing it here is **not** obviously symmetric — the first action is a census of what §4.5.154 added the clause to protect (*"a negative label on an illegal unsigned base"*), which row 12 makes reachable.
