...
- **body param/localparam comma-list**(`localparam A=1, B=2;`/`parameter int A=1, B=2;`)=vita E2002 loud(module-item 경로가 param 1개만 파싱 후 `;` 기대)·iverilog 수용(흔한 구문). 헤더 `#(…)` comma-list는 **§4.5.159 지원**(type-prefix 상속·`parse_param_prefix`+`finish_param_assignment` 기반 마련). body는 1 statement→多 ModuleItem multi-emit 필요(pending-queue at `parse_module_item`+`parse_gen_item` or 콜렉션 사이트 loop-aware·**gen_branch single-item**[`if(c) localparam A=1,B=2;`·iverilog 수용] 스코프 경계 주의)·package/generate 동일 경로. 별 슬라이스.
