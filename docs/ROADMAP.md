...
8. ~~`real` const-fold 전면 미지원~~ **RESOLVED**(§4.5.232) — 실수 산술·alias·체인·정수 승격 전부 correct-support. **잔여(의도적 loud)**: 실수를 정수 문맥에(폭/`$clog2`/replication/정수 localparam) — **i64 twin 등록은 시도했다가 철회**(twin 이 정수 도메인에 real 식을 열어 generate 분기 오선택 등 5건 silent-wrong; §11.8.1 순서를 아는 site 가 `param_real_value` 하나뿐) · ~~generate 스코프의 real~~ **RESOLVED**(§4.5.241) · generate **제어식**의 real(`generate if (R/2 > 2)`)은 잔여 — 제어식 6곳이 `const_eval_in_scope` 만 쓴다 · `1.0/0.0` · 실수 미정의 연산자 · 실수 override.
