...
- **`$value$plusargs` 가 expression 문맥서 loud**(§4.5.237 기록·iverilog 는 허용): blocking 대입의 직접 rhs 로만 지원. `$test$plusargs` 는 제약 없음. 소형 loud→supported 후보.
- **파일 위치 함수군 + `$sscanf` = honest-loud**(§4.5.238 실측): `$ftell`/`$sscanf` 등은 E3009 "unsupported system function in expression", `$fseek` 는 **W3056 warn+skip**. iverilog 는 전부 동작(`A=6 B=0 C=6 D=0`, `$sscanf`→`2 12 34`). **silent-wrong 아님** — ② loud→supported 후보. 스캔 잔여 후보: `$typename` · 포맷 `%u`/`%z`/`%l`.
