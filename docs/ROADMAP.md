...
> | ~~**1c**~~ | ✅ **RESOLVED §4.5.356 — case 의 §12.5 공통 max**(2026-08-21) | 큐엔 *"셀렉터만 문맥을 안 받는다"* 로 적혀 있었고 census 가 **아홉**을, 이어서 420칸 스윕이 **나머지 절반**(셀렉터에 fill 이 없어도 라벨끼리 공통 max 가 성립)을 냈다. ⭐ 고침은 **양끝이 다 움직인다** — 셀렉터를 넓히면 fill 라벨도 같이 넓혀야 한다. 상세=ARCHIVE §4.5.356 |
