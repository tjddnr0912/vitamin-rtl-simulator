...
- **🔴 사이즈 캐스트 `N'(expr)` 의 문맥 규칙이 틀렸다 — 규칙은 핀됐고 구현은 전제가 빠졌다**(pre-existing·**두 오라클 일치**·§4.5.315 발굴 → 2026-08-08 전용 슬라이스가 그라운딩하고 **되돌렸다**). §4.5.212 가 `N'(<문맥결정 연산>)` 을 **N 비트에서** 계산하게 했다 — 넓힐 때는 옳다(`8'(a*b)` 가 캐리를 지킨다). **좁힐 때도 그렇게 한다는 것을 아무도 안 쟀다**: `integer k=7` 에서 `2'(k%4)` 가 **`xx`**(2비트 문맥이 제수 `4` 를 `0` 으로 만든다) · `2'(k/2)` **`00`** · `2'(k>>1)` **`01`** · 중첩도 뚫린다(`2'((k/2)+1)` `01` · `2'(k?k/2:0)` `00`) — iverilog 13 과 verilator 5.050 은 각각 `11`·`11`·`11`·`00`·`11` 로 **일치**하고 vita 만 다르다. 4-state 도 같은 뿌리다(`logic [7:0] a=8'bxxxx_0011` 에서 `2'(a+one)` 가 vita `00` / iverilog `xx` — x 는 저비트 폐쇄를 깬다).
  **정본 규칙(핀 완료)** = IEEE 1800 §11.8.1: 문맥결정 피연산자는 **`max(self, N)`** 에서 평가되고(문맥은 **넓히기만** 한다) **부호는 무조건 전파**되며, 결과를 N 으로 절단한다.
  ⚠️ **`/ % >> >>>` 를 `lower_size_leaf` 로 보내는 수정은 하지 마라 — 시도했고 되돌렸다.** 그 함수는 문맥을 **통째로** 버려서 좁히기는 고치지만 **넓히기와 부호 전파를 같이 잃는다**: `8'((a/b)+u)`(a,b signed·u unsigned)가 62 → **255**, `5'(s4>>u3)` 가 6 → **2**. 실측 집계 = **silent→correct 397칸 vs 회귀 245칸** = 개선이 아니라 **맞바꾸기**(사다리 금지). ⚠️ 그리고 **폭을 필요 이상으로 넓혀도 안 된다** — `>>` 는 정확히 `max(self,N)` 에서만 맞다(32비트로 넓히면 `5'(s4>>u3)` 가 30 이 된다).
  ⭐⭐ **측정된 더 나은 후보가 있다(적대 리뷰가 뮤테이션으로 지어 쟀다)**: 같은 넷을 `lower_size_leaf` 가 아니라 **`lower_ctx_or_plain(e, n)`** 로 보내면 25,704칸 2-state 스윕에서 오답이 **PRE 1291 → 686**(내 수정은 1309)이고 혼합-부호 넷과 fill 칸(`8'('1>>1)`)까지 고친다. ⚠️ **단 그것도 넓힘 클래스는 안 고친다**(`8'(b>>1)`·`8'(b/c)` 오버플로·`9'(a/b)` 의 x 가 zero-extend). 그리고 **"오답이 줄었다" 는 "회귀 0" 이 아니다** — 착수하면 **fixed/regressed 를 따로 세어** PRE 대비 회귀 0 을 먼저 증명하라.
  **선행조건** = `lower_cast` 지점에서 **AST 피연산자의 self 폭**을 알아야 하는데
...
- **🔴 지연 CA 의 초기-X 창이 두 군데서 오라클과 갈린다**(pre-existing·양 백엔드 동일·오라클 ✓·§4.5.302 soundness 렌즈 발굴). `assign #d` 의 출력 레지스터는 첫 지연 쓰기가 착지하기 전까지 x 여야 한다(§11.2). vita 는 그 x 를 **t=0 의 변화로 몰아** 넣는다 — ⓐ `always @(y)` 가 iverilog 에 없는 **`event t=0 y=x` 를 한 번 더** 받는다(`assign #3 y = a;` 단일 드라이버로 재현), ⓑ 그리고 그 X-drive 는 `delayed_sole`(**넷 전체**의 유일 cont-assign 드라이버) 인 경우에만 걸려서, 한 넷의 **서로소 part-select 를 지연 구동하는 두 assign** 에서는 창 안이 x 가 아니라 **z** 다(`assign #3 y[3:2]=a; assign #3 y[1:0]=b;` → iverilog `t=1 y=xxxx` / vita `t=1 y=zzzz`; t=6 부터는 양쪽 `1001` 일치). ⓑ 의 올바른 술어는 "넷의" 가 아니라 **"이 lvalue 가 닿는 비트의"** 유일 드라이버다. **S1d-4d-4 는 이것을 짓지 않았다** — §4.5.303 의 그룹 분류는 whole-net 전용이고 부분 겹침은 E3001 거부라, 비트 단위 드라이버 맵은 여전히 미건설(§3 의 "E3001 일부는 iverilog 가 해상" 항목과 같은 인프라·같은 슬라이스가 소유해야 한다). ⓐ 는 넷 초기값을 x 로 **세우는** 문제(변화로 몰지 않기)라 VCD 바이트가 움직이므로 별도. **§4.5.303 differential 이 같은 가족의 두 구성원을 더 쟀다**: ⓒ CA 가 t0 에 **z 로 settle** 하면 `@(y)` 가 한 번 더 깬다(vita 넷은 x 로 태어나 x→z 가 변화 — 단일 드라이버 tri-state 로 재현·md 무관) ⓓ **keeper 사이클이 t0 x-창을 영구 래치** — `assign p=q; assign p=a?1'b1:1'bz; assign q=p;` 에서 구조 settle 이 initial 바디 **전에** 돌아 `a=x` 를 읽고 x 는 wire 해상의 흡수원이라 `a=1` 후에도 회복 불가(iverilog `p=1 q=1` / vita `x x`·exit 0·양 백엔드+PRE 동일). ⓓ 의 뿌리는 초기값이 아니라 **t0 이벤트 모델**(vita: settle 먼저·바디 나중 — §4.5.300 이 그 모델 위에 `arm_t0` 를 세웠다 / iverilog: t0 이벤트 인터리브)이라 fix = t0 아키텍처 재설계 = DEEP defer. `sched/mod.rs::delayed_sole` 주석의 "delta 예산을 태운다" 는 이유는 **무지연 드라이버와 섞인 경우**에만 성립하고 ⓑ 의 전지연 경우엔 성립하지 않는다(기록해 둔다 — 그 주석이 fix 를 막는 것처럼 읽힌다).
- **별칭 concat lvalue 의 쓰기 순서가 iverilog 와 반대**(pre-existing·세 백엔드 동일·§4.5.307 soundness 부수): `{mem[i], mem[i]} = 16'hBEEF;` 가 vita `ef` / iverilog `be`(상수 인덱스도 동일). IEEE 는 concat lvalue 조각이 **별칭일 때 어느 쪽이 이기는지 정의하지 않으므로** 둘 다 적법일 수 있다 — 판정하려면 퍼널이 조각을 MSB-first 로 적용해야 하는지부터 정해야 한다(기록만).
...
- **packed 비트 오프셋에서는 두 오라클이 갈리고 vita 는 iverilog 를 따른다**(§4.5.310 실측·기록만): 같은 비트 패턴을 packed 로 주면 `pk[u32]`(`reg [4:-3] pk`, u32=`0xFFFF_FFFD`)를 **iverilog 는 드롭**하고 **verilator 는 `pk[-3]` 에 쓴다**. §4.5.310 은 배열-워드 퍼널만 32비트 도메인으로 옮겼고 packed 퍼널(`sealed_signed_index`)은 값 의미 그대로 뒀다 — 합의가 없는 축에서 오라클을 바꿔 타면 근거 없이 움직이는 것이기 때문이다. 열려 있는 것은 "packed 도 32비트 주소 산술로 볼 것인가" 이고, 그것을 정하려면 vcs/xcelium 급 세 번째 오라클이 필요하다.
- **계층 참조를 담은 인덱스는 봉인이 물러난다**(§4.5.309 가 의도적으로 그렇게 뒀다·오라클 ✓·**조용하지만 PRE 와 동일**): `u.fs(0)` 같은 **계층 부호 함수**가 인덱스면 봉인 시점에 placeholder 라 질의가 `None` 이고, 그래서 §4.5.309 가 닫은 ⓓ(부호 있는 함수 반환)가 **로컬에서만** 닫혔다 — `reg [33:2] d2; d2[~u.fs(0)]` 가 iverilog `00000008` / vita `00000000`(로컬 `fs` 로 쓰면 정상). 같은 이유로 계층 `byte`/`shortint` 인덱스도 미해소. 물러남은 **회귀 방지책**이고(placeholder 위에서 봉인하면 correct→loud-wrong), 해소하려면 봉인 결정을 **해상 이후로** 미루거나 placeholder 에 선언 타입을 실어야 한다. ⚠️ **이 문장은 두 번 틀렸고, 세 번째가 실측이다**(§4.5.310 라운드 9·10). ① 처음엔 *"계층 신호·localparam 은 이미 정확하다"* 고 적었으나 `mg[~u.a]` 는 `x`+E4002 이고 로컬 쌍둥이·두 오라클은 42 다(근거로 든 핀은 32비트 `integer` 에 **bare** `mg[u.k]` — 봉인이 필요 없는 형태라 **거절만** 검증했다). ② 그래서 *"placeholder 를 담은 인덱스는 **전부** 거절(신호·localparam·함수반환·select 공통)"* 로 고쳤는데 그것도 틀렸다. ③ **실측한 축은 바깥 select 가 계층인가**다 — 8칸 매트릭스(두 오라클 전부 일치): `u.p[~la[1]]`·`u.p[~u.a[1]]`·`u.mg[~u.a[1]]` 는 **봉인된다**(계층 lvalue/rvalue 는 해상 시점에 다시 물으므로), `lp[~u.a[1]]`·`lp[~u.k]`·`u.p[~u.k]`·`lmg[~u.a[1]]` 는 거절한다. 즉 **로컬 select 안에 든 계층 참조**와 **whole-net 계층 참조**가 거절 대상이고, 계층 select 자체는 아니다. ⚠️ **쓰기 쌍둥이는 또 다르다** — `u.mg[~u.k] = …` 는 착지하는데 같은 식의 읽기는 `x`+E4002 다(PRE==POST).
- **`sim-ir::selfwidth::child()` 가 단언하는 post-order 불변식이 이미 깨져 있다**(pre-existing·latent·§4.5.309 soundness 실측): `hier_defer/read.rs` 의 지연 해상이 **상위 id 를 가진 노드의 복제를 하위 슬롯에 제자리 설치**하므로 그 노드의 자식 id 가 자기 id 보다 클 수 있다. 오늘 패닉이 안 나는 이유는 설치되는 두 형태(`Signal`·`Select`)가 `child()` 를 부르지 않는 팔이기 때문뿐이다(`Select` 의 폭은 `const_u32_of_expr_ctx` 로 아레나를 직접 인덱싱한다). 두 드라이버가 그 불변식을 **이름으로 인용**하므로, 세 번째 형태가 설치되는 순간 조용한 오답이 아니라 패닉이 난다 — 고칠 자리는 복제 설치이거나 `child()` 의 계약이다.
- **문자열 리터럴 인덱스가 조용히 버려진다**(pre-existing·§4.5.308 soundness 부수·오라클 ✓): `reg [33:2] d2; d2["\020"]` 가 iverilog `00004000` / vita `00000000`·exit 0. 문자열 상수는 `ConstRepr::StrUtf8` 이고 정본상 무부호인데 인덱스 경로가 값을 못 쓴다.
- **서브루틴 지역/포멀 배열의 좁은-부호 인덱스가 아직 loud 다**(§4.5.310 라운드-3 실측·**PRE 와 동일**): `index_is_repeatable` 이 **배열-원소 읽기**(`Signal{word:Some}`)를 복제 불가로 막아, `mp[~s8]` 은 75(오라클 일치)인데 **같은 식을 배열 원소로 준** `mp[~ix[0]]` 은 `x`+E4002 다(두 오라클 75). 300칸 스윕에서 14칸. **의도된 트레이드**다 — 복제를 허용하면 범위 밖 원소 읽기가 진단을 두 번 내고 `warn_run_range` 의 8/run 예산을 먹어 **무관한 사이트가 loud→silent** 가 된다(실측: 그 형태에서 PRE 7 / 허용 시 9). 값은 loud 이므로 사다리상 안전하고, 해소하려면 진단을 사이트 단위로 dedup 해야 한다. ⚠️ 리뷰어의 *"in-range 원소 인덱스 5개가 E4002 5개를 낸다"* 는 **재현되지 않았다**(PRE·POST 둘 다 0).
- **dyn/queue 원소 인덱스가 같은 설계 안에서 fixed 배열과 갈린다**(§4.5.310 라운드-4 실측·**loud**): 같은 64비트 인덱스로 `FIXED` 는 18(오라클 일치)인데 `DYN`·`QUE` 는 `x`+W4020 이다(두 오라클 모두 18). PRE 는 셋 다 틀렸으므로 uniform-wrong → divergence 로 바뀐 것이고, ENGINEERING_RULES 가 그 실패 모드를 이름으로 적어 뒀다. dyn/queue 원소 경로는 `seal_index_unsigned` 를 안 지난다 — 같은 규칙을 그 퍼널에도 태우는 것이 해소다(loud 라 사다리 위반은 아니다).
- **🔴 `byte'(…)`/`shortint'`/`int'` 캐스트가 피연산자를 결과 비트 수만큼 복제한다**(pre-existing·PRE==POST·§4.5.310 라운드-10 실측·오라클 ✓): `systask::coerce_two_state` 가 `Concat` 을 만들며 **비트마다 같은 `base: e`** 를 놓아, `r = byte'($urandom)` 이 **8번** 뽑는다(`shortint'` 16·`int'` 32·`8'(…)` 는 1). iverilog 는 1번이고 값도 다르다(`R 36` vs vita `R 78`). 인덱스 전용 문제가 아니다. ⚠️ 그리고 `packed.rs` 의 봉인 주석이 `m[byte'($urandom)]` 를 **복제 가드가 막는 예시로 세 번 인용**하는데, 그 예시는 이 두 번째 퍼널 때문에 **여전히 깨져 있다**(주석은 실측대로 정정했다). 해소 = `coerce_two_state` 에 같은 복제-가능성 가드를 걸거나 비복제 구성을 찾는 별도 슬라이스.
- **인덱스가 봉인되면 엔진 비용이 레벨당 6× 로 자란다**(§4.5.310 라운드-10 실측·**회귀 아님**): 중첩 packed 원소 select 로 만든 인덱스는 봉인의 `extend_to`(피연산자 2회)와 기하(3회)가 곱해져, 9레벨에서 vita 169 s / iverilog 0.01 s. elaboration 은 평평하고(0.00~0.02 s) 비용은 **엔진이 복제된 IR 을 트리로 도는 것**이다. PRE 가 맞히던 칸에는 회귀가 없다(무부호 쌍둥이는 PRE==POST) — 새로 맞히게 된 칸이 시간에서 무제한이다. 라운드 5~8 이 elaborate 쪽 절벽 넷을 dedup 으로 없앴지만 **인덱스를 두 번 이름 붙이는 비용 모델 자체는 그대로**다.
- **32비트 초과 인덱스의 READ 가 네 철자에서 두 오라클과 갈린다**(§4.5.310 라운드-7 실측·PRE==POST·840케이스 차분의 **유일한** 잔여): `reg [63:0] b = 64'h1_0000_0002` 를 런타임으로 주면 iverilog·verilator **둘 다 절단해서 원소를 읽는데** vita 는 `x` 다 — `reg [0:31]`·packed 워드·packed 비트·frame 원소 비트 넷. 쓰기 쪽은 오라클이 갈리고 vita 가 iverilog 를 따르므로(그래서 packed 은 절단을 안 받는다) **읽기/쓰기 비대칭을 오라클에서 먼저 설명해야** 결정할 수 있다. ⚠️ 이 carve-out 의 원래 근거 주석은 **리터럴**(`p[64'h1_0000_0002]`)을 인용했는데 iverilog 는 그걸 컴파일 에러로 막으므로 근거가 되지 못했다 — 실측으로 정정했다.
- **packed 원소 인덱스의 런타임 읽기가 갈린다**(pre-existing·PRE==POST·§4.5.310 라운드-3 실측): `reg [3:0][7:0] p; p[64'h1_0000_0002]` 읽기가 iverilog `22` / verilator `22` / vita `xx`. §4.5.310 은 **쓰기 쪽**을 근거로 packed 을 PRE 구성에 동결했는데(쓰기는 iverilog 가 드롭·verilator 가 착지 = 오라클 갈림), **읽기 쪽은 두 오라클이 일치**한다. 즉 packed 도 런타임 인덱스는 32비트로 절단하는 것이 맞을 가능성이 높고, 결정하려면 읽기/쓰기 비대칭을 iverilog 에서 먼저 설명해야 한다.
- **🔴 `timescale` 없는 모듈 안에서 `$time`/`%0t` 이 전역 정밀도로 나온다**(pre-existing·§4.5.305 그라운딩 부수 발굴·오라클 murky). 컴파일 순서상 어떤 ``timescale`` 디렉티브보다 **앞선** 모듈에서 `$time` 을 읽으면 iverilog 는 그 모듈의 시간 단위를 **1s** 로 보아 `0` 을 주고(1364 의 고전 기본값), vita 는 전역 정밀도 눈금 그대로 `4`/`%0t`=`4000` 을 준다(1800 §3.14.2.3 의 "지정 없으면 시뮬레이션 시간 단위" 해석). **두 해석 다 문헌 근거가 있어 오라클이 갈린다** — §4 룰대로 vita 는 자기일관성(전역 정밀도)을 지키되, 이 비대칭이 **실사용 혼합 파일 세트에서 조용한 출력 차이**라는 사실은 기록해 둔다(재현: 무-timescale 모듈 + `\`timescale 1ns/1ps` TB, `#3.5` 후 자식이 출력). 결정하려면 doc-15 의 W1017 정책과 함께 한 슬라이스로 다뤄야 한다.
...
- **같은 이름 plain 블록 로컬의 두 번째 초기화자가 조용히 사라진다**(pre-existing·1fe06e7==HEAD==§4.5.268·§4.5.265 마무리 리뷰 실측 — round-16 은 이 형태를 **건드리지 않았다**: §4.5.268 이 연 것은 `automatic`/dyn-storage 가 **두 중첩 레벨**에서 같은 이름을 쓰는 경우이고, plain 정수는 여전히 `$blk$` 스코핑 대상이 아니다): `initial begin int x = f("A"); end initial begin int x = f("B"); end` = iverilog `A B` / vita **`A` 만**. v1 의 bare-name flatten 모델(두 블록이 한 net 을 공유)에서 **두 번째 decl-init 이 첫 번째를 못 이기고 버려진다** — 읽기를 추가하면 definite-assignment 게이트가 loud 로 잡지만, 쓰기만 있으면 무성. `$blk$` 스코핑이 dyn-storage 에만 열려 있어 plain 정수는 대상이 아니다(→ §3 후보).
- **인스턴스화되지 않은 `interface` 가 root 가 아니다**(pre-existing·PRE(1fe06e7)==POST·§4.5.256~260 스윕 중 실측): `interface ifc; int iv = 7; initial $display(...); endinterface` 를 아무도 인스턴스화하지 않으면 iverilog 는 그것을 **root 로 elaborate 해서 initial 을 돌린다**. vita 의 root 선택(비인스턴스화 **모듈**)엔 인터페이스가 없어 조용히 아무것도 안 한다. 초기화 순서와 무관한 **root 선택** 갭이고, 순서 스윕에서 24개 오탐으로 나타나 격리했다.
