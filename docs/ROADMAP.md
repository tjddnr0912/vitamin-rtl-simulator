...
- NEW(pre-existing · §4.5.460 differential · 2-오라클): 같은 폭 규칙의 **OVERRIDE 쌍둥이**. `parameter HE = ~8'h5A;` + `#(.HE(~4'h5))` 가 `fffffffa` 32비트, verilator `a` 4비트(`4'h5 << 1` 도 같고 `8'd200 + 8'd100` 은 verilator `2c` 8 / iverilog `12c` 9). PRE·POST 동일 = 회귀 아님. 자리는 row 25 와 같은 것(`param_decl_width_opt` 가 `default_binds == false` 에서도 **기본값**을 읽는다) ⇒ **row 25 의 연산자 판**, WALL(provenance) 로 같이 둔다.
- NEW(pre-existing · §4.5.460 soundness · 2-오라클 · 3칸): `param_init_kept_loud` 의 전제가 이 슬라이스로 **거짓이 됐다**. 그 doc 이 "꼬리가 self 폭으로 사이즈하게 되면 이 술어도 같이 없앤다" 라고 쓰여 있고 꼬리는 이제 그렇게 한다 — `~(|4'b1010)`·`(|4'b1010) << 2`·`-(|4'b1010)` 는 여전히 E3009 인데 두 오라클은 `0`·`0`·`1` 을 1비트로 낸다. 삭제하면 값도 맞다(무제한 fold 뒤 새 폭으로 마스킹하면 셋 다 오라클 값). 호출부 5곳(`generate`·`instance`·`package`·꼬리·`params.rs:1683`) ⇒ 바인더 census 가 필요한 자기 슬라이스. §2 🆕 H ⓓ 가 이미 "그 클래스가 닫히면 삭제" 로 예고한 것.
- NEW(pre-existing · §4.5.460 differential · 2-오라클): 파라미터 VALUE fold 는 여전히 폭-무제한이다 — `localparam K = (8'd200 + 8'd100) >> 1` 은 폭 8(맞다)에 값 `96`, 두 오라클 `16`. 꼭대기 마스킹으로는 못 고친다(안쪽 합이 300 을 유지한 채 시프트가 8비트 합에 없는 비트를 읽는다). 폭-인지 쌍둥이는 `eval_const_env_self` 이고, 라우팅하면 **모든** untyped 파라미터의 값이 바뀐다 ⇒ 자기 census 를 가진 슬라이스.
