...
| 🆕 P | OPEN | a continuous assign inside a generate block whose RHS indexes a module-scope array PANICS: `logic [31:0] A[0:3]; generate if (1) begin : g assign top.o = A[1]; end endgenerate` aborts with `index out of bounds` at exit 101 where both oracles print the element. Reproduces with NO name shadowing (found beside §2 🆕 O, PRE == POST) | `sim-engine/src/state/init_diag.rs:869` indexes a per-net table with a net id the generate-scope continuous assign produced | a panic is below loud on the ladder ⇒ ①; census the table's producer against the generate-scope assign lane first | 2-oracle | S? |
| 🆕 Q | OPEN | a `localparam` declared in a PLAIN named `begin : g` block inside an `initial` is a parse error (`E-PARSE-UNEXPECTED-TOKEN: expected statement, found keyword 'localparam'`); both oracles accept it. The GENERATE-scope spelling of the same declaration parses | the block-local declaration region does not admit `localparam` (only the generate one does) | parser: admit it in the block decl region and bind it where a generate-scope one binds; §3 material (loud, additive) | 2-oracle | S |
