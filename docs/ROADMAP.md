...
| ⑤ⓕ | unpacked-array typedef residue (§4.5.445 supported the declaration, the ANSI port and every declaration binder; §4.5.447 the NON-ANSI port and `$bits` of the bare TYPE name, 14 cells): still loud — a tf-port FORMAL and a function RETURN type (**1-oracle**: iverilog 13.0 refuses `input logic [7:0] v [0:3]` itself — `sorry: Subroutine ports with unpacked dimensions are not yet supported` — so only the DYNAMIC `[]` formal is two-oracle; the row's old 2-oracle claim was measured false) · a `parameter` of that type · `parameter type T = a_t` (both oracles `$bits` 32) · `$bits(a_t)` when a dim names a PARAMETER (both oracles 64; the parse-time table declines a name it cannot fold before the override, which is the guard-rail that keeps it from baking a pre-override width) · dims on BOTH the typedef and the declarator (`a_t y [0:1]`, a live oracle SPLIT on dimension ORDER — iverilog `$size(y,1)=4 $size(y,2)=2`, verilator `2` / `4`, and iverilog contradicts its own answer for the identical explicit type) | each consumer reads `TypeInfo` and has no slot for unpacked dims; every one of them now DECLINES on `!info.unpacked.is_empty()` rather than binding the element type | per-consumer, each its own slice; the split row is do-not-start | 2-oracle except the function return type (1) and the declarator-dims split (0) | S each |
...
|---|---|---|---|---|
| 1 | 1 | §3 ⑤ⓕ residue, cheapest first: `$bits(a_t)` when a dim names a PARAMETER (the parse-time table declines a name it cannot fold before the override — that guard-rail is the design question) · then the tf-port FORMAL (1-oracle for the fixed shape, 2-oracle only for the dynamic `[]` one). `$bits(pkg::T)` and its scalar twin SHIPPED (§4.5.452) | §3 ⑤ⓕ | ② |
| 2 | 2 | §2 row 10: after `import pk::*;` a `K[31:24]` of a >64-bit package parameter reads `11` where both oracles read `dd` (the SCOPED spelling `pk::K[31:24]` and a `$bits`-sized net are both correct, so the two halves are different machinery). One root — a wide import binds into `wide_param_bits` and `param_sel_range`'s `walk_scopes_key` does not look there | §2 row 10 | ① |
| 3 | 3 | §2 🆕 H ⓐ: a reduction operator in a range BOUND clamps to 1 bit — `(&4'b110x)` / `(|4'b101x)` are 3 and 4 in both oracles, 8 cells (the `^` / `~^` cells are an oracle SPLIT and are excluded). ⚠️ re-measure at selection whether the fix touches the wide fold's ACCEPT SET; if it does it is behind the §11.8.1 region-sign wall and this slot moves on | §2 🆕 H | ① |
...
Do not start: rows 14/16/25/26/30 and 🆕 F (declared-width provenance / §11.8.1 region sign wall), row 34 (one oracle, zero demand), row 31 (pure half correct ⇒ performance), 🆕 Q (BLOCKED — a block-scoped CONSTANT binding is the prerequisite; the bare-name hoist was built and measured 5 new silent-wrongs, §4.5.451), any widening of the wide fold's accept set before §11.8.1 region sign stands.
