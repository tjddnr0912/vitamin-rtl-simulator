...
**오라클 주의 — iverilog 결함 5건(vita가 IEEE 정답)**: ① string **배열 원소**의 `.len()`이 문자열 길이가 아니라 **배열 크기**를 낸다(`string s[5]; s[0]="abcdefg"` → iverilog 5, vita 7; 같은 텍스트를 스칼라에 넣으면 iverilog도 7). ② 동시 fork 활성화가 automatic string 배열을 공유한다(`A!` 대신 `A!!`). ③ 같은 fd 에 `$fmonitor` 를 두 번 걸면 **누적**해 둘 다 찍는다 — 자기 자신의 싱글턴 `$monitor` 와 모순(vita 는 destination 별 replace). ④ 빈 string **배열 원소**를 `%s` 로 찍으면 공백 1칸(스칼라는 빈 문자열). ⑤ **`$clog2` 의 signed-wrap 인자를 32비트 정수로 승격 후 unsigned 로 읽는다**(`$clog2(4'sd7+4'sd1)` → 32) — §20.8.1 "treated as an unsigned value" 는 인자 **자기 폭의 비트 패턴** 읽기라 정답은 3(verilator 5.050·vita 런타임·상수 도메인 모두 3 · §4.5.343). 전부 회귀 테스트로 핀 고정.
...
> | 1e | 🆕 **계층 타깃 + 형제가 좁은 중첩 fill**(§4.5.355 잔여 · **PRE==POST**) | `logic [63:0] W; W = '1+1;` 은 **0**(iverilog 일치)인데 `u.wide = '1+1;` 은 **4294967296**. 규칙이 `max(ctx, sibling)` 이라 형제(`1` = 32비트)가 타깃보다 좁으면 ctx 가 유일한 폭 출처인데, 지연 타깃에선 그게 1 이다. ⚠️ **12비트 타깃에선 두 경로가 우연히 일치**해서 §4.5.355 의 중간 결론(*"bare 가 곧 깨진 집합 전체"*)이 한때 참처럼 보였다 — 64비트 스윕이 그것을 깼다 · **선행조건**: 닫으려면 resolve 시점에 **lowering 스코프를 가지고 `lower_expr_ctx` 를 재실행**해야 하는데(bare fill 과 달리 `(raw,kind,width)` 로 재구성 불가) 지금 아키텍처엔 그 스코프가 없다. 그것이 이 항목의 진짜 작업이다 |
> | **1c** | 🆕 **`case ('1)` 셀렉터가 1비트로 남는다**(pre-existing · PRE==POST) | §12.5 는 case 식과 **모든 item 식**을 공통 max 폭으로 sizing 한다. item(label) 쪽은 이미 `lower_case_label` 이 문맥을 준다 — **셀렉터만** 안 준다 · ⭐ **§4.5.354 가 재확인 + repro 를 날카롭게 했다**: `case ('1) 8'hFF: a=1; 1'b1: a=3;` 이 두 오라클 **a=1**(셀렉터가 라벨 max 폭 8 로 커져 `8'hFF` 와 match) / vita **a=3**(셀렉터가 1비트라 `1'b1` 과 match) — 라벨 둘이 서로 다른 답을 주게 배치해야 증상이 보인다 · ⚠️ §4.5.354 가 **label 쪽** `lower_case_label` 을 건드렸지만 이 항목과 **무관**하다(그건 real 셀렉터가 라벨에 폭을 안 빌려주게 한 것 · 여기선 셀렉터 자신이 fill) — PRE==POST 로 확인 |
> | 1d | 🆕 **§4.5.354 가 드러낸 오라클 분열 둘 + 관대함 하나**(전부 **PRE==POST · fill 무관** = 그 슬라이스가 만든 게 아니라 **드러낸** 것) | ⓐ `s <= 1'b1` 이 iverilog **1** / verilator **0**(vita 는 0 = verilator 편) — StrCmp 가 비-string 피연산자를 어떤 도메인으로 올리는지의 문제 · ⓑ `'1 ** r` 이 iverilog **480** / verilator **1**(vita 는 480) — `**` 의 밑수는 Table 11-21 상 문맥-결정인데 §11.8.1 이 식 전체를 real 로 만들 때 그 문맥이 살아남는지가 갈린다(PRE 는 **둘 다 아닌 0** 이었다) · ⓒ 분열은 아니고 **vita 가 더 엄격한** 자리: `%` 는 IEEE §11.4.3 이 real 피연산자를 금지하는데 iverilog·verilator 는 **둘 다 실행한다**(vita = E3009 · PRE 부터 그랬고 no-fill 쌍둥이도 그랬다) ⇒ 고칠 것 없음, 기록만 · **ⓐⓑ 는 2-오라클 합의가 없으므로 착수 불가** — 먼저 필요한 건 구현이 아니라 hand-IEEE 판정 |
