...
> | 15 | 🔄 **RE-GROUNDED 2026-09-01 — SPLIT: a 10-cell SILENT-WRONG sits under the recorded loud gap, and only that half is startable** | **⭐ THE STARTABLE HALF (2-oracle silent-wrong, exit 0)**: a parameter OVERRIDE carrying a sized x/z literal loses the unknown plane. `leaf #(.K(8'b1010_010x))` onto `parameter logic [7:0] K` is **`10100100`** in vita and `1010010x` in iverilog AND verilator; `8'hx`→`00000000`, `8'bzzzzz1z0`→`11111110` (z→**1**), and it propagates (`K === 8'b1010_010x` is 0 in vita, 1 in iverilog). **10 cells, every override channel** (named, positional, interface, generate-scope, untyped, all four bases, signed, z, `-G`). ⭐ Site = `params.rs:1397-1419`, which decides *"does it fit the i64 lane"* with `bp_get(..).0` — a **value** test that never consults the plane; `override_at_declared_width` has **exactly one caller**, and the correct guard already exists **twelve lines up in the sibling `fill` arm** (`fill_is_unknown` → decline) = a branch-parity gap. `bp_any_unknown` is already `pub(crate)`. ⭐⭐ **`format_version` 29 is NOT at risk — verified, not assumed**: `Elaborator` is not serialized and `ir::ConstVal` already carries `val`+`unk` at 29 (the ≥65-bit lane ships x through it today). ⚠️ **THE RECORDED HALF — do not start**: 22 loud cells (x/z literal init, const OOR select, `/0`, `%0`, x in a width/generate position…) all need a real unknown plane in the narrow `params` store = an `is_known` companion at **15 read / 25 write sites** plus 17 of `hier_params`, which is the *"a default is not a fact"* shape that produced four blockers in §4.5.399, and it stacks on row 14's unresolved wall. ⭐ **The >64-bit asymmetry is CONFIRMED exactly**: `localparam logic [W-1:0] P = W'bx` is E3009 at W=8/32/63/**64** and correct all-x at W=**65**/96/128 — and on one `[95:0]` declaration, an x at bit 95 is correct while an x at bit 7 is silent-wrong. Corpus demand **0** (no bench design puts x/z in a parameter).
> | 16 | 🔄 **RE-GROUNDED 2026-09-01 — MOSTLY FOLDS INTO ROW 17's ORACLE SPLIT; only one sub-case survives** | 12 override cells measured: 5 diverge from iverilog, but **verilator sides with VITA on 4 of them** (`-64'd1`, `<ones> + 64'd1`, `<ones> << 4` — verilator folds the override at its own width then zero-extends), and `~32'd0` is a **3-way split** (ivl all-ones / vlt `00…00ffffffff` / vita `00…00ffffffffffffffff`). That is row 17, and it swallows most of this row. ⚠️ **A fix here would "correct" one side of a live oracle split** ⇒ exclude row 17's shapes by construction. **The only 2-oracle actionable sub-case** = operands already ≥ the target width: `#(.K(~128'd0))` is vita `00…00ffffffffffffffff` vs both oracles all-ones. ⇒ demote this row and carry the sub-case inside row 21.
...
> | 21 | 🔄 **RE-GROUNDED 2026-09-01 — it is a 2-oracle SILENT-WRONG (48/108), not the loud gap the row describes** | The recorded `**` repros are real and loud, but they are the **rare corner** of the class: the same gate is silent whenever the i64 walk happens to answer. `localparam logic [127:0] C = ~32'd0;` is vita `0000000000000000ffffffffffffffff` and **both oracles** `ffffffffffffffffffffffffffffffff`, at exit 0 with zero diagnostics — same for `32'd0 - 32'd1`, `-32'd1`, `<ones> + 1`, `<ones> * 2`, `<ones> << 4`, a `?:`, and `~128'd0` at 256. **Census: 108 cells (9 forms × operand 32/64/128 × target 65/96/128/256) = 48 SILENT-WRONG · 13 loud · 47 ok**; the 13 loud are ONLY `**` plus the cells where the i64 walk itself declines. ⭐ **Strictly a >64-bit-TARGET domain** — a second census at targets 33/40/64 is **0 wrong** (at ≤64 the i64 container's semantics coincide). ⚠️ **Row 14 is NOT a prerequisite** — the silent cells reproduce with **pure literals and no name at all**, so the roadmap's *"shares row 14's provenance requirement"* is wrong for the dominant half. ⭐ Site = **`const_wide.rs:160 fold_self_bits`**, which evaluates at the expression's OWN width and takes no context-width parameter; four consumers gate on `wide_top_is_self_determined`. **The install plumbing already exists** — `wide_disagreeing_value` prefers the wide answer whenever the two domains disagree, so the moment `fold_self_bits` stops declining the 48 cells become correct with no new mechanism. Blast radius = **46 call sites, 40 of them internal recursion**; the 6 external callers pass `None` and stay byte-identical. ⚠️⚠️ **Prerequisite = row 27, written ONCE**: whatever propagates a context width down the tree must stop at a shift's RIGHT operand and `**`'s exponent, and writing that classification twice is the drift shape this repo keeps getting bitten by. Corpus demand **0** (10 wide parameter declarations in bench, every one self-determined).
...
> | 27 | 🔄 **RE-GROUNDED 2026-09-01 — it needs no function and no name, and the recorded CAUSE was wrong** (**2-oracle** · **silent-wrong**, not merely a constant-function corner · **선행조건 0** · ⭐ **다음 착수 1순위**) | §11.4.10 makes a shift count self-determined and *"always treated as an unsigned number"*. **A 158-cell census: 66 SILENT-WRONG · 22 loud · 68 ok** (⚠️ 32 of the 68 are LATENT, not correct — the bug is visible only when the count's unsigned value is below the target width). ⭐⭐ **Reachable at MODULE scope with no function and no name**: `logic [(16'h0100 >> 3'sb101)-1:0] bus;` is `$bits` **2** in vita and **8** in both oracles — a silently wrong bus width — and `generate if (16'hFF01 << 3'sb101)` takes the **wrong branch**, deleting the body at exit 0. Also an unpacked dim, a `repeat` count and a `-:` width. All four shift operators; `**` is correct (its own `const_pow_exponent_selfdet`); ⭐ **no other self-determined position is affected** (index, replication count, ternary condition, `$clog2`, case label, array dim, gen-for bound all fold correctly) — the defect is shifts alone. ⚠️ **The recorded diagnosis is refuted**: `eval_const_assign` does NOT push context onto the count — `const_fn_width.rs:394-404` already routes it through `eval_const_env_self`, which gives the operand its **own signedness** (right for a ternary condition and a select index, wrong here). ⚠️ Two *"already correct"* claims are also refuted: `C[1:0]` and `$unsigned(C)` are **LOUD**, and only UNSIGNED literal counts are correct — a signed literal (`3'sb101`) is wrong. Genuinely correct = anything that is not a bare/parenthesised/cast leaf (`{C}`, `-C`, `C*1`, `C+0`). ⭐⭐ **The fix belongs at the CALL SITE, not in the shared function** (`eval_const_env_self` has **17 callers** — 8 in `const_fn.rs`, 5 in `const_select.rs`, 3 here — and only one is a shift count): evaluate as today, then reinterpret the RESULT unsigned at the count's self width. ⭐⭐ **The reference implementation is already in the tree and correct** — `const_wide.rs:138 fold_shift_count` cites §11.4.10 verbatim, and the >64-bit lane answers `001fe020` on every spelling the i64 lane gets wrong. ⭐ **All 66 cells are vita SELF-CONTRADICTIONS** (runtime ✓ · wide const lane ✓ · i64 const lane ✗) ⇒ no oracle is strictly needed to prove it. ⚠️ **NOT blocked by the declared-width-provenance wall** — the fix reads only the count's SELF width. It is itself the first prerequisite of row 14, and the sizing rule it establishes must be written ONCE and consumed by the wide walk too (rows 16/21). Corpus demand **0** (1,505 files, no signed-literal shift count anywhere).
>
