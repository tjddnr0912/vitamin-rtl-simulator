...
> 🆕 L ⓓ, a `pkg::LABEL` member — a `pkg::`-scoped operand inside a constant concatenation has no
> fold arm at all, `{1'b1, p::X}` is loud where the bare imported `{1'b1, X}` folds — §4.5.413) —
> all "is not a constant"; the runtime reads are right. §4.5.413 measured two more consumers on
> the header spelling, identical on the body control: `A[i][3-:2]` as a child instance override
> (ibex_cs_registers:1485) and `$size(A)` in a `localparam`. Also loud, variable twin
...
> Order after §4.5.415: ⓔ (cs_registers' `.ResetValue(PMPRstCfg[i])` /
> `PMPRstAddr[i][PMP_ADDR_MSB-:PMPAddrWidth]` consumers) → the preprocessor rung (`define default
> arguments, prim_assert) → header-parameter member widths (lockstep).
>
