...
| **4** | **성능 — 이미 지어 둔 빠른 경로를 안 부르는 자리**(⚠️ **2026-08-21 재규정**) | ⭐ **§4.5.351 이 축을 다시 열었다** — `apply_nba` 가 `k_write_scalar` 를 한 번도 안 불러 picorv32 **−3.9%** 가 거기 있었다. 표적은 코드젠도 스케줄러 재작성도 아니라 **"증명해 놓고 버린 자리"** 다 | ⚠️ 옛 문구의 "스케줄러 29%" 는 **12%** 다(그 버킷의 `dispatch_body` 8.31% 는 문장 실행기, `settle_cont_assigns` 8.29% 는 cont-assign 평가+쓰기). settle 빈 패스(61%)는 공짜(실측 0%). **다음 후보 = `settle_cont_assigns` 의 같은 질문**(그쪽 `run_wprog` 는 아직 72바이트 `Value` 를 짓는다) |
