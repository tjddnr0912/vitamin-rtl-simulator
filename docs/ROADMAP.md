...
- **선언 범위 정규화 잔여 — §4.5.309 가 5분류 중 넷을 닫았고, 남은 하나는 loud 다**(오라클 ✓): `reg [7:0] ma [2:5]; reg signed [7:0] s8 = -6; ma[~s8]` 가 iverilog `50` / vita **`x` + E4002 + exit 1**. `~s8` 는 자기 8비트에서 5 인데 unpacked 기하(`seal_index_unsigned`→`dim_guard`/`dim_coord`)가 **무부호 도메인 전용**이라 부호 있는 인덱스를 봉인하지 않고 32비트로 0확장한다(`0xFFFFFF05`). packed 쌍둥이는 §4.5.309 가 고쳤다(`$signed(idx)`). 해소 = 그 기하를 부호 도메인으로 옮기는 별도 슬라이스. ⚠️ 같은 배열에 `mg[s8]`(s8=−1, `[-3:2]`)은 **오늘도 틀리다**(PRE==POST)라 이 항목을 미루는 것이 회귀를 만들지는 않는다.
- **🔴 무부호 인덱스의 봉인 결정은 부호 문제가 아니라 배열-워드 i32 재해석 문제다**(§4.5.309 differential 실측 · 34칸 측정된 기회): 음수 base 배열에서 봉인하지 않은 정규화는 `raw + |k|` 를 **32비트로 랩**하고, **그 랩이 iverilog 의 답**이다(`reg [7:0] ma[-3:2]` 를 `$stime + ii`(=`0xFFFF_FFFD`)로 읽으면 `ma[-3]` · 명시적 `reg [31:0]` 로 줘도 iverilog 는 같은 칸을 준다). §4.5.309 는 이 절반을 **PRE 결정에 동결**했다(`unsigned_seal_admitted` = 옛 `expr_provably_unsigned`, 이제 그 한 가지 일만 한다) — 실측상 **폭이나 base 로는 두 무리가 안 갈리기** 때문이다: `reg [31:0] ix` 와 `$stime` 은 같은 음수 base 아래 같은 32비트 무부호인데 PRE 는 앞은 봉인해야, 뒤는 안 봉인해야 맞다. 열려 있는 결정은 **배열 워드 인덱스를 i32 로 재해석할 것인가**(`eval::offset_of_index_value` 가 이미 그 모델을 문서화하고 있다). 재해석으로 통일하면 s7 매트릭스에서 **오늘 틀린 34칸이 맞아진다**(측정: 무부호 봉인을 음수 base 에서 물러나게 했을 때 `BOTH_WRONG` 38→4) — 단 packed 쪽 대가가 있어 함께 결정해야 한다(같은 값을 packed 로 주면 iverilog 는 재해석하지 **않는다**: `pk[u32]` 드롭 vs `ma[u32]` 착지). 뿌리 divergence 하나가 더 얽혀 있다 — **iverilog 는 `$stime`/`$urandom` 을 부호로 읽고**(`$stime + ii` 가 `-3`) vita 는 IEEE 대로 무부호(`4294967293`)로 읽는다.
...
- **런타임 배열-워드 인덱스에서 iverilog 가 자기모순**(§4.5.308 soundness 실측 · vita 는 값 의미로 일관): 같은 비트 패턴 `0xFFFF_FFFD` 를 iverilog 는 **런타임 배열 워드**에서는 부호로, **팩트 비트 오프셋**과 **상수 배열 워드**에서는 무부호로 읽는다(세 갈래). vita 는 세 경우 모두 값 의미(IEEE 문자 그대로)라 그 한 칸에서만 갈린다 → **vita-ahead**, 내부 앵커로 고정할 대상. ⚠️ `eval::offset_of_index_value` 의 *"iverilog 는 beyond-i32 배열 워드를 절단한다(`mem[64'h1_0000_0002]` 가 `mem[2]` 에 착지)"* 는 **재현되지 않았다**(측정: 드롭) — 그 문장의 근거는 다시 재야 한다.
