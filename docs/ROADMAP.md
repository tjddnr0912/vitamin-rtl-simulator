...
>
> **기준선(2026-07-27)**: format_version **25** · **4675 tests green** · 3-OS CI green · MsgCode **59** · **MSRV 1.85**. 최신 = **§4.5.245**(inner-NET shadow 메커니즘 확정). 직전 = **§4.5.244**(남은 대형 3건 착수 판단표). 직전 = **§4.5.243**(generate case real scrutinee 비목표 — real 가족 완결). 직전 = **§4.5.242**(generate 제어식 real 라우팅 — "real→정수 문맥" 완결). 직전 = **§4.5.241**(generate 스코프 `localparam real` loud→correct-support). 직전 = **§4.5.240**(크기 추정 정정 + if-cond 핀). 직전 = **§4.5.239**(스캔 4차 완료 — `$typename` 핀). 직전 = **§4.5.238**(스캔 3차 honest-loud 확인 + LOOPROMPT 압축). 직전 = **§4.5.237**(테스트 0건 스펙 스캔 2차 — 핀). 직전 = **§4.5.236**(`%p` real 반올림 silent-wrong). 직전 = **§4.5.235**(fresh-area 스윕 CLEAN + 무오라클 능력 핀). 직전 = **§4.5.234**(sized-literal enum label — enum 메서드 loud→correct-support). 직전 = §4.5.233(그 그라운딩/크기추정). 직전 = **§4.5.232**(`real` const-fold — 실수 산술 loud→correct-support). 직전 = **§4.5.231**(모듈 스코프 상수식 **비목표 판정** — iverilog 자기모순 실측 + 자기일관성 teeth). 직전 = **§4.5.230**(상수함수 인터프리터 폭 인식 — `localparam W=f()` 의 조용히 틀린 파라미터 값; 내부 차분=인터프리터 vs 런타임). 직전 = **§4.5.229**(상수식 BOUND/COUNT 단일 퍼널 — part-select 폭·replication count·indexed part 폭의 silent-wrong 8가족; format 불변). 직전 = **§4.5.228**(round-20 8항목 — fork-arm 재개·음수 하한 unpacked/packed·동시 활성화 dyn·generate/interface 스코프·VCD 선언범위·`$fmonitor`/`$fstrobe`). 직전 = **§4.5.227**(§0 **T1 잔여 11항목 전부 RESOLVED** — 임의 선언 bounds/방향·multi-dim `foreach`·중첩 decl-init·bounded string queue·계층 read+write·계층 assoc·frame-local 재귀·SoA whole-element). 직전 = §4.5.222~226(T1 1차 6종). **완료 슬라이스 상세·§번호별 원문 = [ROADMAP_ARCHIVE.md](ROADMAP_ARCHIVE.md)**(최신이 위) — 이 문서는 전방 전용이므로 완료 서사를 두지 않는다.
>
...
| **C. inner NET 이 outer PARAM 을 shadow 못 함** | 大 | **大 — 유일하게 남은 ①-급 silent-wrong**(오라클 有) | **§4.5.245 에서 메커니즘 확정**(아래 §2 참조). 선행 = **order-INDEPENDENT AST-gathered per-subroutine local-name set**(`gather_local_decl_names` 는 모듈 레벨만 모은다 — 서브루틴용이 새로 필요). 없이 `params` consumer 를 켜면 §4.5.218 S1 재발(중첩 generate body **조용히 삭제**·exit 0). **실패 전력 1회**. |
...
- **🔴 inner NET이 outer PARAM/enum-label을 shadow 못 함(§4.5.245 메커니즘 확정·오라클 有·DEEP)**: **트리거 = 모듈 스코프 param 과의 이름 충돌, 그것뿐**. 충돌이 없으면 로컬은 정상 동작한다(실측: `int X` 로컬 → 9 정확). 충돌하면 **param 이 이긴다** — `localparam W=4` + function-local `int W; W=9; return W` = vita **4** / iverilog 9. **범위는 기록보다 넓다**(§4.5.245 실측): function-local · **task-local**(`output` 경유도 동일) · **begin/end 블록 로컬** · `localparam int` 라벨 전부 동일 발화(vita `9 4 4 3` vs iverilog `9 9 9 8`). 근인 = `lower_expr` 의 이름 해석 순서가 **subst → out_subst → string-param → `lookup_scoped`(params) → net** 이라 **서브루틴 로컬 net 보다 param 이 먼저** 잡힌다. fix = 서브루틴 본문을 lower 하는 동안 **그 서브루틴의 AST-gathered 로컬 이름 집합**을 참조해 param 조회를 건너뛰기(집합은 **순서 독립·AST 순수함수**여야 §4.5.218 S1 재발 없음).
