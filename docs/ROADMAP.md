...
- **🔴 좁히는 사이즈 캐스트의 결과가 더 넓은 대입 문맥에서 절단되지 않는다**(pre-existing·PRE==POST·오라클 ✓·§4.5.319 적대 리뷰 발굴). `logic [7:0] a=255` 에서 `r4 = 4'(a+1)` 은 `0`(정답·대상 폭 == 캐스트 폭)인데 **`r16 = 4'(a+1)` 이 `0010`**(iverilog `0000`), `r16 = 8'(a*a)` 가 `fe01`(iverilog `0001`), `r16 = 8'(a+a)` 가 `01fe`(iverilog `00fe`). **`**` 전용이 아니다**(`+`·`*` 에서 난다) — 누출 조건은 정확히 *"캐스트 결과가 더 넓은 대입 문맥의 피연산자일 때"* 이고, concat 안(`{12'b0, 4'(a+1)}`)과 자기결정 문맥(`$display("%h", 4'(a+1))`)에서는 정상이다. verilator 는 이 형태를 거부해 오라클이 iverilog 뿐.
