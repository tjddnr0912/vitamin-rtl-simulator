...
> ⓕ **NEW rung the census exposed (pre-existing, keyword scalar `parameter logic [7:0] X = Dflt`
> identical): a header `import` is not visible to the header's own parameter defaults** —
> `module m import p::*; #(parameter perm_t X = Dflt)` is E3009 "undefined name `Dflt`" (both
> oracles: the value). Every ibex module header is this shape (`ibex_top.sv:15`, `ibex_core.sv:17`,
> `ibex_lockstep`, `ibex_if_stage`: `parameter lfsr_perm_t RndCnstLfsrPerm = RndCnstLfsrPermDefault`
> after `import ibex_pkg::*;`), so it blocks the module pages behind ⓓ.
...
> Order after §4.5.414: ⓕ header import → ⓔ (cs_registers' `.ResetValue(PMPRstCfg[i])` /
