...
| 단계 | 무엇 | 게이트 / 중단 판정 |
|---|---|---|
| **V0** | **커버리지 격차 측정.** 기본 백엔드를 `Native` 로 flip 하고 전 스위트를 돌려 fallback 한 테스트 수와 **거부 행 히스토그램**을 낸다. 되돌리기 0, 구현 아님 | — (계기). **이 숫자가 V1 의 슬라이스 목록이다** |
| **V1** | **커버리지 확장.** 거부 행을 큰 것부터 닫는다. 현재 S0 거부 = `real`·`string`·`queue`·`dyn_array`·`assoc`·`class`·`sva`·`force_release`·`stmt_effect`·`file_directed`·`disable_fork` · 실행기 층 = `final`·fork/`wait fork`·task 프레임·`$monitor`/`$strobe`/`$dumpall`·파일 시스템태스크 | 행마다 슬라이스 · 각 행은 **VM 과 바이트 동일** |
