...
- 잔여(§4.5.463 이 명시적으로 사퇴한 두 모양 · 2-오라클): ⓐ override 식의 leaf 가 **이름**이면(`#(.P(W8 + 1'b0))`) `ctx_width_names_are_evident` 가 거절 — `const_self_width` 가 이름을 `param_meta`(값-추론 폭이 사는 곳)에서 재기 때문. 두 오라클 8, vita 32. 부모가 자기 `Q` 를 override 받은 forwarding(`#(.P(Q))`)도 같은 줄. ⓑ **>64비트** 트리(`~128'd0`)는 `const_ctx_within_i64` 가 거절 — 값 재-fold 가 64에서 clamp 한다. 두 오라클 128, vita 32. 선행조건 = declared-width provenance 벽(rows 14/25/26/30).
...
| blocal-flatten | 잔여 3건, 전부 **모듈 경로와 동일**(interface 는 이제 쌍둥이와 완전 parity — 각 셀의 module twin 이 같은 답) ⇒ interface 행이 아니라 flatten 모델 행이다: ⓐ 같은 이름을 쓰는 두 형제 블록이 `int s = 8'h44` 처럼 **선언 초기화자**로만 대입돼도 read-before-assign 가드가 거절(두 오라클 실행, `o1=44 o2=55`) — 가드가 선언 초기화자를 definite assignment 로 안 센다 ⓑ 상호배타 `if/else` 두 가지가 같은 이름을 선언하면 같은 가드가 거절(두 오라클 모두 이 구문 자체를 거부 ⇒ 무오라클) ⓒ 중첩 shadow 가 있을 때 같은 이름의 interface/module **멤버**가 덮인다(`member_x=01`, 두 오라클 `a5`) | `block_local/gate.rs` 의 read-before-assign 가드 + flatten 모델 | ⓐ 선언 초기화자를 DA 에 세기 ⓑ 배타 분기 인지 ⓒ 멤버 shadow | ⓐ 2-oracle ⓑ 무오라클 ⓒ 2-oracle | — |
