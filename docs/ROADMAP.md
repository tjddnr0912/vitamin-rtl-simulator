...
> | 1f | 🆕 **`**` 의 밑수가 non-fill 경로에서 문맥을 못 받는다**(§4.5.361 이 ⓑ 를 기각하며 **진단을 뒤집어** 발굴 · silent-wrong) | `logic [15:0] a; real r=2.5; a = (4'd15+4'd1) ** r;` → iverilog **1024**(밑수를 대입 문맥 16비트로 재서 16, 16^2.5=1024) / vita **0**(밑수를 자기 폭 4비트로 재서 0) · ⭐ **fill 경로는 이미 맞다**(`'1 ** r`=480=iverilog) — 즉 머신러리는 있고 `lower_expr` 의 plain 경로가 안 부른다(§4.5.351/352/353 과 **같은 모양**) · ⚠️ 선행조건이 무겁다: plain `lower_expr` 에는 ctx 인자가 아예 없다 ⇒ `**` 밑수만 골라 `lower_expr_ctx` 로 라우팅하거나, 대입 rhs 전체를 ctx 경로로 올려야 한다 · ⚠️ **착수 전 288칸 PRE-3-way 재측정 필수** — ⓑ 를 기각한 그 corpus 가 그대로 게이트다 |
