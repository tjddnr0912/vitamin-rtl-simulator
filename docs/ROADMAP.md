...
- **선언 범위 정규화 잔여 5분류**(§4.5.308 이 뿌리를 고친 뒤 남은 것 · 전부 오라클 ✓ · soundness 렌즈가 한 줄씩 실측): ⓐ **좁은 부호 고정폭 인덱스**(`byte`/`shortint`/`reg signed [N:0]`)가 정규화로 **부호확장 대신 0확장** — `ma[~s8]`(s8=−6)이 원소 5 를 놓치고 `dn[bb]` 가 비트 27 을 놓친다(packed·unpacked 공통). ⓑ **32비트 부호 인덱스가 비-0 LSB 아래**일 때 — `integer`/`int` 는 증명 불가이고 bare `Const` 도 아니라 `d2[k+7 +: 2]`(k=−6)이 부분 쓰기 대신 통째 소실(packed 한정). ⓒ **음수/상수-산술 리터럴이 비-0 LSB 아래** — 접기가 bare `Const` 노드만 받아 `d2[-1 +: 4]`·`d2[1-2 +: 4]`·`dn[-34 +: 3]` 이 소실(같은 값을 `localparam integer` 로 주면 정상). ⓓ **부호 있는 사용자 함수 반환** — `Expr::Call` 은 구조적으로 증명 불가라 `dn[fs(8'd6)]`(`function signed [7:0]`)가 소실. **전부 한 전제를 공유한다** — 봉인은 concat 이라 부호를 지우므로 "증명 가능하게 무부호" 일 때만 걸린다. 근본 해소 = 자기-부호를 아는 봉인, 즉 `sim-engine::width` 정본 규칙을 **sim-ir 로 옮겨 공유**하는 것(오늘의 `expr_provably_unsigned` 는 그 보수 부분집합이고, class-field 패치와 `**`/`real`/좁은-부호 설계를 포함한 크로스-크레이트 성질 테스트가 건전성을 잰다).
