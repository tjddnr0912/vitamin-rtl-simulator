...
- **~~`foreach` over dynamic array/queue/assoc inside a FUNCTION or SUBSET-task body~~ RESOLVED §4.5.175**: 파서가 `foreach(a[i])`를 `__st=a.first/next(__i)`로 desugar하는데, 이 walk step이 iteration KEY를 side-effect로 WRITE한다(`&mut write_lvalue`·`Scheduler::assoc_iter_step`). **동기 frame executor**(`run_frame_call`=함수·`run_task`=subset task)는 `&self`라 key를 못 advance→plain eval이 key를 stuck시켜 walk 미진행→**조용히 0 반환**(적대 리뷰서 `function int g(); foreach(marr[i]) s+=marr[i]` = vita 0 vs iverilog 15로 발굴). **fix**: `run_frame_call`/`run_task` BB 루프서 rhs가 assoc-iter SysFunc(`AssocFirst/Next/Last/Prev`)면 `call_fatal` 래치(runtime fatal F4004·correct-or-loud). **미영향**: suspendable task(`$display`/`@`/`#`→`&mut` run_process)·module process·함수 内 `for(i<a.size())` 루프[제안 workaround]·fixed-array foreach·direct dyn index/`.size()` read. 신규 `frame_foreach_dynamic_loud.rs`×8.
