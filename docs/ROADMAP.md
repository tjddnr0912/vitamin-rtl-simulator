...
- **계층 참조를 담은 인덱스는 봉인이 물러난다**(§4.5.309 가 의도적으로 그렇게 뒀다·오라클 ✓·**조용하지만 PRE 와 동일**): `u.fs(0)` 같은 **계층 부호 함수**가 인덱스면 봉인 시점에 placeholder 라 질의가 `None` 이고, 그래서 §4.5.309 가 닫은 ⓓ(부호 있는 함수 반환)가 **로컬에서만** 닫혔다 — `reg [33:2] d2; d2[~u.fs(0)]` 가 iverilog `00000008` / vita `00000000`(로컬 `fs` 로 쓰면 정상). 같은 이유로 계층 `byte`/`shortint` 인덱스도 미해소. 물러남은 **회귀 방지책**이고(placeholder 위에서 봉인하면 correct→loud-wrong), 해소하려면 봉인 결정을 **해상 이후로** 미루거나 placeholder 에 선언 타입을 실어야 한다. ⚠️ **이 문장의 앞선 판본은 틀렸다**(§4.5.310 라운드-9 실측): *"계층 신호·localparam 은 해상 뒤에 다시 물어보는 경로가 있어 이미 정확하다"* 고 적었으나 `reg [7:0] mg[-3:2]; mg[~u.a]` 는 `x`+E4002 인데 로컬 쌍둥이 `mg[~la]` 와 두 오라클은 **42** 다. 그 주장을 뒷받침한다던 핀은 32비트 `integer` 에 **bare** `mg[u.k]` 라 봉인이 애초에 필요 없는 형태였다 — 즉 **거절을 검증했지 봉인을 검증하지 않았다**(그 테스트의 doc 은 "셋 다 봉인에 닿는다" 고 주장한다). 실제 범위 = **정규화 시점에 아직 placeholder 인 계층 참조를 담은 인덱스는 전부** 봉인을 거절한다(신호·localparam·함수 반환·select 공통). 36칸 스윕에서 18칸, 240칸 퍼즈에서 31칸 — 전부 PRE==POST.
