...
- **untyped param 값-결정 타입 잔여(§4.5.161 LITERAL·§4.5.162 IDENT/EXPRESSION 해소·이하 residual)**: (a) **`time` param value-inferred**: `localparam time C=<expr>`가 value-inferred(negative value→32-bit signed·`$bits(time-alias)`=32 vs net 64)—time은 §4.5.161-162 value-determined에서 의도적 제외라 declared 64-unsigned 미적용. fix=time param에 (64,unsigned) meta 배선. (b) **pkg-scoped ident**(`localparam C=p::X`·X signed)=unsigned(`const_expr_signed`이 single-seg ident만 sign 상속·2-seg 미처리). (c) **expr self-determined WIDTH**(`8'hFF+8'hFF` 등 sub-expr truncation·§2 size-cast DEEP 동근·§4.5.162는 sign만·width=`min_signed_bits(folded)` 근사). (d) **interface-member** positive param(`i.A<i.B`)=unsigned(module-hier/pkg 정상). oracle=iverilog ✓. 전부 pre-existing·stash-rebuild로 미악화 확인.
