...
- **untyped param 값-결정 타입 잔여(§4.5.161서 LITERAL-valued 해소·이하 residual)**: (a) **expression/ident-valued** untyped param(`localparam C=A+B`·`C=D`)=value-inferred 유지→positive면 unsigned(내부 inconsistent: 직접 리터럴 `D=7`은 signed인데 `C=D`는 unsigned·§11.8.1 오답). fix=`param_decl_width`에 folded 값 sign 추론 or expr-signedness(broad). (b) **interface-member** positive param(`i.A<i.B`)=unsigned(module-hier/pkg는 정상·interface-specific). (c) sized-literal `time` param `$bits`(=literal width vs 64·pre-existing·byte-identical). oracle=iverilog ✓. 전부 pre-existing·미악화(§4.5.161 differential/soundness 확인).
