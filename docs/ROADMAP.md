...
- `TimeLit` 잔여: REAL 타임 리터럴은 §4.5.457 이 해소(`#(2.5ns)`·`#(3.0ns)` 등 17칸, 두 오라클 — 반올림은 정밀도가 아니라 **모듈 시간단위**에서 한다). 남은 것은 식 안의 real 타임 리터럴 `#(2.5ns + 1ns)`(두 오라클 4틱 / vita 0 — 모듈단위 값이 정수가 아니라 정수 레인이 declines 하고 real 레인은 bare `TimeLit` 만 본다) · `#(2*2.5ns)` 는 분열(iverilog 6 / verilator 5) · 음수 real 타임 리터럴은 1-오라클(verilator 가 자기 런을 abort) · PROCEDURAL `#(2.5ns) stmt;` 는 정직한 loud(E3009 · §3).
...
|---|---|---|---|---|
| 1 | 1 | §3 ⑤ⓕ residue, in oracle-strength order: `$bits(T)` on the bare name of a DIM-CARRYING type parameter (**2-oracle, both 24**; vita is loud because the element width is the symbolic `T$w` and `casts.rs::bits_of_type_name` folds only literals — the fix is the symbolic product `sym_typedef_bits` already builds for a typedef), then the dim-carrying OVERRIDE `m #(.T(b_t))` (**2-oracle, both `bits=64 size=4`**; needs a dim slot on the `T$w`/`T$s` channel, which is why §4.5.457 refused it at the parse), then a `localparam` OF such a type (1-oracle, cheap — route `typedef_param_shape` into the existing `ConstArrayVar` channel). The DECLARATION subset and the tf-port FORMAL SHIPPED (§4.5.457 / §4.5.456). The function RETURN type is do-not-start (1-oracle + frozen `FunctionDef`) | §3 ⑤ⓕ | ② |
| 2 | 2 | §2 🆕 I ⓒ residue, found by §4.5.457's own differential lens: an UNSIGNED narrow net index into a NEGATIVE-base array — `logic [7:0] m[-2:1]; wire [3:0] k; assign k = -4'sd2; assign c = m[k];` is E4002 / `xx` where both oracles read `a5`, while the SIGNED declaration of the same net is correct at HEAD. One instance, PRE == POST. The seal reads the net's DECLARED sign; measure first what the oracles size the index to (the literal `m[-4'd14]` twin is the control) | §2 🆕 I | ① |
| 3 | 3 | §2 "Delays / events" residue: a REAL time literal inside an ARITHMETIC expression — `assign #(2.5ns + 1ns) y = a;` is 4 ticks on both oracles and 0 in vita. The bare-`TimeLit` spelling shipped (§4.5.457); this one needs the real lane to see a `TimeLit` LEAF, whose module-unit value is not an integer, so the integer lane declines the whole tree. ⚠️ `#(2 * 2.5ns)` beside it is a live split (iverilog 6 / verilator 5) — do not fold the two into one rule | §2 Delays | ① |
