...
| **4** | **성능 — 스케줄러 축**(⚠️ **미측정**) | Phase D 는 **표현식·저장** 축만 다뤘다. picorv32 비율이 안 움직인 것이 그 증거인데 **이유는 아직 안 쟀다** | ⭐ **첫 걸음은 코드가 아니라 프로파일**: picorv32 를 심볼 빌드로 `sample` 하고 함수별 self time 을 낸다(방법 = §5.1-az) |
