...
- **🔴 real 값이 `expr_is_real` 에 안 잡히는 생산자 = 全 index/size gate 우회**(2렌즈 리뷰 발견·일부만 해소): `atoreal()`·dyn `real d[]` 원소·`ArrSum/ArrProduct` 는 **해소**(arm 추가). **잔여 = real 반환 FUNCTION**: `v[7:f()]` 가 조용히 쓰기 소실(`f()` 가 `%0h` 로 `4008000000000000` = f64 비트패턴을 냄). 근인 = **inline 함수는 `func_metas` 가 비어 있어**(frame func lower 전까지) `Call` arm 이 안 물리고, inline 반환 net 이 `NetKind::Real` 이 아님. **fix 전제** = inline 함수 반환 net 에 real kind 를 실어주거나, inline 결과 표현식에서 real 을 전파.
