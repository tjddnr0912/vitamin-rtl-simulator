...
- **untyped param 값-결정 타입 미적용(§4.5.160 적대 발굴·multipart)**: 무장식 `localparam A = <expr>`가 IEEE §6.20.2 "값이 타입 결정" 미적용 → 항상 `signed=false`+value-self-width. ① sign: `localparam A=-1, B=2; A<B`=vita `ge`(둘 다 unsigned·§11.8.1 collective) vs iverilog `less`(A signed −1)·**param↔param 전용**(param↔literal은 정상). ② width: untyped sum `%b`(A=-5,B=-10; C=A+B)=vita 32-bit vs iverilog 33-bit. 동근=untyped param이 folded 값에서 sign/width 추론 필요(elaborate param-typing·sign+width **multipart**·전용 슬라이스). oracle=iverilog ✓. pre-existing(comma-list 없이·single param서도 재현·§4.5.159 무관).
