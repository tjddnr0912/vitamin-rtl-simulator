...
| **C. inner NET 이 outer PARAM 을 shadow 못 함** | 大 | **大 — 유일하게 남은 ①-급 silent-wrong**(오라클 有) | 선행 = **order-INDEPENDENT AST-gathered per-scope name set**. 없이 `params` consumer 를 켜면 §4.5.218 S1 재발(중첩 generate body 가 **조용히 삭제**·exit 0). **실패 전력 1회** — 반드시 그 선행부터. |
