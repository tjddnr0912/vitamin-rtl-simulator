...
- **enum `.name()` 미지원**(loud→supported·전용 슬라이스·iverilog ✓·§4.5.163+ 발굴 그라운딩): `x.name()`=vita E3009("hierarchical function call deferred")·iverilog는 label string 반환. `first/last/num/next/prev`는 i64 fold로 지원되나 name()은 `enum_method_expr`이 의도적 None(주석: dynamic string exact-length는 statement 필요). **그라운딩 확정**: (a) string-var **case-assign**(`case(x) L: s="L"; endcase`)은 exact-length=iverilog 일치(ternary는 result-width=max branch로 짧은 label PAD=silent-wrong→ternary desugar 금지). (b) direct-expr(`$display("%s",x.name())`)는 iverilog가 exact(RED→`[RED]` 3자)이므로 **hoisted temp-string + case-assign**(elaborate `fresh_string_temp`+`hoist_stmt_top` 패턴 mirror·§4.5.127 $sformatf-hoist 인프라) 필요. fix=statement-form desugar(assign-context 최단·expr-context는 hoist). 부수: chained `x.name().len()`·`arr.min()[0]`도 같은 "hierarchical call deferred" family(별개).
