...
> **§4.5.275 후속 2줄(2026-07-30 · ②는 2026-07-31 §4.5.277 에서 해소)**: ①output-formal 호출의 eval-order 수리가 **클래스/2-세그먼트 메서드 본문**을 해소하지 못해 `y = c.m(x) + f(x)` 는 정직한 loud — `call_effect` 가 그 본문에 `Inert` 를 증명할 수 있으면 correct-support(오라클 有: iverilog task 쌍둥이 `y=16 x=6`). ②~~frame body 안에서는 copy-out 을 낼 수 없다~~ **RESOLVED(§4.5.277·상세=ARCHIVE)** — "이름 없던" 패닉 조건은 `in_frame_body` 가 아니라 **copy-out 의 모든 목적지가 프레임 창 밖인가**였다(5-probe 측정: 목적지가 frame-local 이면 PASS, 모듈 넷이면 rc=101). **잔여** = 목적지가 모듈 넷인 형태(반환 lvalue 든 **output actual** 이든)는 여전히 정직한 loud — `r = nxt(5, gv)` 처럼 output actual 하나만 모듈 넷이어도 그렇다. 승격 전제 = 프레임 실행기가 프레임 밖 넷을 쓸 수 있게 되는 것(= 그 태스크를 `&mut` 경로로 올리는 것과 같은 축).
...
- **🔴 frame body 의 FIRST 문장에 있는 copy-out 이 엔진 assert 를 밟는다**(pre-existing·오라클 有·§4.5.276 리뷰 4라운드 발굴): `task automatic tk(output int r); nxt(5, gv); …` 처럼 output-formal 호출이 프레임 본문의 **첫 문장**일 때 `sim-engine/state/frame_eval.rs` 의 `frame write targets a frame-local net` 로 **rc=101 패닉**(PRE `8cf4165` 바이트 동일). **조건은 아직 이름 붙이지 못했고 추측하지 않는다** — 실측: `task automatic tk(output int r)` 에서는 앞에 문장이 있든 없든 패닉하고, output formal 이 없고 본문이 선언을 가진 중첩 `begin … end` 인 task 에서는 패닉하지 않고 값도 맞다. 즉 조건은 `in_frame_body()` 가 **아니다**. §4.5.276 에서 `in_frame_body()` 로 물러나게 하는 수정을 **두 번 시도해 두 번 되돌렸다**: 12 형태 중 **10 개가 PRE 에서 정답**이었고(iverilog 오라클) 그중엔 round-20 리포트의 `.rsp` 워커를 `task automatic` 으로 옮긴 것도 있었다. fix 는 **실제 패닉 조건**(측정 = copy-out 이 프레임의 entry 블록에 떨어질 때)을 술어로 만드는 것 — 엔진 쪽 이해가 필요한 별도 슬라이스. 패닉은 loud 이지 silent-wrong 이 아니므로 사다리 하강은 아니다.
...
- **dyn-formal 호출이 못 가는 자리 = `&&`/`||` 우변 · 다른 호출의 인자 · select/lvalue 인덱스 · `case` scrutinee · `repeat` 카운트 · cast/replicate 피연산자.** 17-위치 행렬 실측(§4.5.276): 지원 7(직접 rhs·concat·비교·systask 인자·`?:` arm·NBA rhs·`return`) / loud 10(그중 **9건은 iverilog PASS = false-loud** · 나머지 1건 `case` scrutinee 는 iverilog 가 `string` case 를 거부해 무오라클 → hand-IEEE · **wrong 0**). 근인은 §3.1 과 **같은 뿌리**다 — dyn 용 좁은 hoister(`has_unhoistable_dyn_formal_call`/`hoist_dyn_formal_calls`)와 §4.5.275 의 범용 hoister(`shape()` 기반)가 **서로 다른 위치 집합**을 갖는다. correct-support = 범용 hoister 가 dyn-formal 호출도 흡수(leaf 에서 copy-out 대신 `__t = f(arr)` 를 `lower_stmt` 로 방출; dyn callee 는 input formal 만 가지므로 순수해서 eval-order 수리가 불필요). **착수 시 함정**: `hoist_stmt_general` 은 `in_frame_body()` 에서 통째로 물러나는데 **dyn-formal hoist 는 frame body 안에서 지원되는 것이 정상**(Family C·r17)이다 — 그 stand-down 을 statement 단위가 아니라 **call-kind 단위**로 쪼개지 않으면 §3.1 클래스를 새 자리에 재생산한다. 그래서 별도 슬라이스로 뒀다.

**round-19 에서 loud 로 올린 silent-wrong 2건 — correct-support 후속(오라클 ✓):**

- **frame body 안의 파일 읽기**(R19-X2). `$fgets`/`$fscanf`/`$sscanf`/`$fread`/`$fgetc`/`$ungetc` 의 목적지 쓰기는 **프로세스 실행기만** 하는 문장 수준 효과라, `run_frame_call` 의 순수 `eval` 경로에서는 X 를 내고 아무것도 안 만졌다 — **vita `rc=0 loc=[]` / iverilog `rc=9 loc=[Len = 16]`, exit 0**(§4.5.274 에서 런타임 fatal 로 승격). correct-support = `SimState::files`/`read_state` 를 interior-mutable 로 만들어 `&self` frame 실행기가 몰 수 있게 — `dyn_heap` 이 frame-local `new[]` 에 해준 것과 같은 수(§4.5.194)이고, `frame_assoc_iter` 가 부작용 있는 문장 형태를 이미 그 실행기에서 처리하는 선례다. 접근 지점은 좁다(`read_state` 7 · `files` 8 · 5 파일). 대상 = `.rsp`/CAVP 벡터 워커 = 테스트벤치의 표준형.
