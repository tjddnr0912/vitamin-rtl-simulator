...
> | # | 항목 | 왜 이 순서인가 |
> |---|---|---|
> | — | ✅ **해소된 행 21개는 ARCHIVE 로 이관**(2026-09-02) | 1 · 1d · 1e · 1f · 🆕A · 2 · 2b · 3 · 4 · 6 · 8 · 8b · 9 · 11 · 12 · 13 · 18 · 21 · 22 · 27 — 원문·행번호 보존 = [ROADMAP_ARCHIVE](ROADMAP_ARCHIVE.md) 「§2 표 해소 행」. ⚠️ 번호는 재사용하지 마라(테스트 주석이 참조한다) |
...
> | **🆕 D** | **A cast whose SIZE is a bare identifier parses `CastTarget::Named` and has no constant-fold arm** (2026-09-03 · pre-existing · PRE == POST · 2-oracle) | `localparam int NN = 40; localparam logic [39:0] H = NN'('1 ^ 1'b0);` is `00ffffffff` while `(NN)'('1 ^ 1'b0)` — one pair of parentheses, the same expression — is `ffffffffff`; both oracles give `ffffffffff` for both. Site = `hdl-parser/src/casts.rs` (an `Ident` casting type goes to `Named`, everything else to `Size`); `fold_bits_at` has a `Size` arm only. The non-fill controls fold either way, so this is purely the fill lane. Fix = a `Named` arm that resolves a name to a width, or a parser disambiguation using the elaborate-time symbol kind. |
...
> Order after §4.5.418: the SVA parenthesised-property rung → header-parameter member widths
> (lockstep).
>
