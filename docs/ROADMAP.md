...
**inline(static-task) 경로 `foreach`-on-dyn-formal (pre-existing·§4.5.170 gap·§4.5.173 발굴):** static(non-`automatic`) task의 `input b[]` formal에 `foreach(b[i])`를 쓰면 `foreach`가 `first/next`로 lower되며 E3009("first/next/last/prev are only supported as the DIRECT rhs...")—clean HEAD서도 동일(내 변경 무관). 회피=`for(int i=0;i<b.size();i++)` 또는 task를 `automatic`으로(§4.5.173 frame 경로는 `foreach` 동작). fix=inline `foreach` desugar가 dyn-array actual일 때 size-loop로(별개 슬라이스).
