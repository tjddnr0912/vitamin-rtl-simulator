...
|---|---|---|---|---|
| 1 | 1 | §3 loud→correct, chosen by 3-tool re-measurement at start: `typedef T a_t[N]` (unpacked-array typedef, both oracles; parse error) · ⑤ ⓓ/ⓔ residues · CU-scope items in a class body · class `#(type T)` header (one oracle) · ibex DPI export 12 + core:2481 (DEEP) | §3 ⑤ | ② |
| 2 | 2 | §2 🆕 L ⓩ: `localparam logic [11:4] A1 = 8'h3C; localparam L = {A1[7:4], A1[7:4]};` → vita `33`, oracles `cc` (the placement resolver sizes a name without its declared LSB); beside it the runtime read `ROTA[1]` of an outer array parameter shadowed by a generate-scope scalar | §2 🆕 L | ① |
| 3 | 3 | §2 🆕 N residue: a `$error` inside a named block reports `[in top]` (iverilog `top.blk`); `%m` in an assertion action block omits the assertion label; a task declared in an unnamed block is loud (split) — re-measure verilator at start | §2 🆕 N | ① |
| 4 | next | mixed-caller callee (intersection rule, design decision) · `m #(8)` / `defparam u.T$w` (illegal input accepted) · VCD `$scope` `[0]` spelling · `genblk<N>` label collision (split) · 🆕 L ⓦ residue (package constants outside the i64 interpreter) | §4.5.432/436/437/440 residues | ② |
