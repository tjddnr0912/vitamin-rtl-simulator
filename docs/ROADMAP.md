...
- NEW(pre-existing · §4.5.461 differential · 2-오라클 · row 25 의 연산자 판): OVERRIDE 값 자체의 폭. `module sub #(parameter P = 1)` 에 `#(.P(-(|4'b1010)))` 가 `ffffffff` 32비트, 두 오라클 `1` 1비트(`~(|4'b1010)`·`~8'h5A`·`8'd200+8'd100` 도 같다). PRE·POST **바이트 동일** = 회귀 아님. §4.5.461 이 기본값 쪽 false-loud 를 걷어내면서 이 클래스가 **도달 가능한 설계가 늘었다**(전엔 기본값이 loud 라 설계 전체가 거절됐다). 자리는 row 25(`ovr_bits` 가 override 식을 `const_self_width` 로 재지 않는다) = WALL(provenance).
...
| iface-blocal | interface 본문의 **사용자 작성** 블록로컬(`begin int x; … end`)이 loud. §4.5.461 은 파서 합성 `__foreach_*` 쌍만 hoist 한다(그것만으로 interface 안 `foreach` 가 9에러→정답, nested·multi-dim 포함 3-way 동일). ⚠️ generate 안의 `foreach` 는 이 행이 아니다 — interface 안 `generate` 자체가 상류에서 E3009(`generate blocks inside an interface are outside the MVP`), 별개의 pre-existing MVP loud | `iface_inst.rs` 가 hoist 를 아예 안 했다. hoist 만으로는 module 쌍둥이가 아니다 — `instance.rs` 는 먼저 `local_decl_names`·`decl_block_locals`·`scoped_block_locals`·`per_entry_block_locals`·`coalesced_block_locals` 를 **`&ast::ModuleDecl` 에서** 만들고, 그것이 스코프 이름과 **충돌하는** 블록로컬이 flatten 에 섞이는 것을 막는다. interface 경로에선 그 맵들이 부모 모듈 것이라 실측 loud→silent-wrong(`integer b` 멤버 + `begin integer b; b=7; end` → `OUTER b=7`·`u.b=7`, 두 오라클 `99`; module 쌍둥이는 PRE·POST 다 정답) | 선행조건 = 그 다섯 패스를 interface 본문에 대해 돌리고 Nets·Logic 두 패스에 걸쳐 유지. 현 게이트는 body 단위 all-or-nothing(사용자 블록로컬이 하나라도 있으면 `foreach` 도 같이 거절) | 2-oracle | S |
...
|---|---|---|---|---|
| 1 | 1 | §2 "Index sealing" row 25 의 **연산자 판**(OVERRIDE 값 자체의 폭). `#(.P(-(|4'b1010)))` 가 `ffffffff` 32비트, 두 오라클 `1` 1비트. §4.5.461 이 기본값 쪽을 열면서 **도달 가능한 설계가 늘었다**(PRE·POST 는 바이트 동일 = 회귀 아님). 자리 = `params.rs` 의 `ovr_bits` — override 식을 `const_self_width` 로 재지 않는다. ⚠️ WALL(provenance) 안이므로 착수 전 §11.8.1 region sign 전제를 먼저 재라 | §2 Index sealing | ① |
| 2 | 2 | §3.b `iface-blocal`: interface 본문의 사용자 작성 블록로컬. 선행조건은 실측됐다 — `compute_scoped_block_locals` 외 네 패스를 interface 본문에 대해 돌리고 Nets·Logic 두 패스에 걸쳐 유지. 지금은 `&ast::ModuleDecl` 타입이라 그 시그니처가 슬라이스의 전부. §4.5.461 이 합성 `__foreach_*` 만 열어 둔 상태 | §3.b | ② |
| 3 | 3 | §2 "Index sealing" ORACLE-SPLIT 아래 남은 것 또는 §3 ⑤ⓕ 의 비-arity 축(아래 6번의 첫 항목) — 착수 전 §1 슬롯 규칙으로 재선택 | §2 / §3 | ① |
