...
| §2 | recorded defects by mechanism | 164 | 81 | 83 | oracle split / pinned / oracle disqualified 46 · named prerequisite 18 · WALL (AST self-width) size-cast cluster 6 · one oracle 5 · held on purpose 0 · pair columns not measured 1 | inline / frame binds 15 · size cast / signedness 16 · constant domain (i64) 12 · scoping / imports / block-locals 27 · delays / events 20 · real 4 · performance 6 · index sealing 11 · ranges / selects 6 · diagnostics 8 · class fields 3 · oracle splits 35 | ① | |
...
- A continuous-assign hop of a write made earlier in the batch is delivered after the WHOLE
  ...
  Both oracles propagate the continuous assign before the next process runs. BLOCKED BY the
  same-time resume order (start-order table row 7), measured in §4.5.539: settling the dirtied
  ...
  fires the level wait in iverilog only. The `@(posedge w)` twin is the EDGE half below. Once
  row 7 lands (the resume order), the yield settle + the UDP nonblocking output is the slice
  (S–M; the diff is in `s26/review/DIFF.patch`). (§4.5.537 review cell soundness/cells/c07.)
