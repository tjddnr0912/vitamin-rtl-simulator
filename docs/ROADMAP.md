...
- **round-14 리포트 잔여 (전부 loud·오라클=iverilog ✓·§4.5.167서 스코프)**:
  ...
  - **V5 (frame dyn-array local `new[]`)**: `byte loc[]; loc=new[n];` in a frame→E3009(local이 DynArray handle 아님). frame slot에 heap dyn-array handle 저장 인프라 필요(→ V2A-frame[automatic task dyn-array formal]도 해소). "순차적으로 전부" 다음 순위. iverilog ✓.
