...
### 5.0 ★★ ③층 — T0·S0·S1(전부)·**S2 닫힘**·**S3a**·**S3 슬라이스 1~2 ✅** → **S3 재설계 판정 필요** → S4 → S5 → S6 → 재측정

> **⭐⭐ S3 코드젠 본체 착수 = census 가 S3 의 전제를 반증했다 (2026-08-10 · §4.5.334).** §5 S3 의 논거는
> *"넷은 슬롯 주소로 직접 store/load — 커널 호출 없음(여기가 ②층 JIT 이 진 지점이다)"* 이다. 실행 census
> (picorv32: `WProg` 6.29M 회 · op 24.86M · 활성화 663k)가 셋을 반증했다 — ⓐ **인라인할 산술이 1.5% 뿐**,
> ⓑ **`WOp::Load` 는 이미 `buf[vi]` 직접 접근**이라 ②층 JIT 이 진 이유를 **S1 이 이미 없앴고**,
> ⓒ 인라인 불가한 **30.8%(`LogBin` 17.4 + `Unary1` 7.9 + `Cmp` 5.5)가 하필 §4.5.315·330·331 이 한 철자로
> 통합한 함수들**이다 — cranelift IR 재작성은 **의미 재진술**(§4.5.279 클래스), 콜백은 **op 당 경계**(②층이
> 진 모양). **⚠️ 즉 "cranelift 로 커널 호출을 없앤다"는 이 워크로드에서 성립하지 않는다.**
>
> ⭐⭐ **win 은 프로그램이 아니라 그 주위 기계장치에 있다** — 실행의 **47.2% 가 op 하나짜리 프로그램**이고
> 그 두 번의 메모리 읽기 주위에 `Rc` clone·IR 워크·72바이트 `Value`·`resize`·퍼널 재유도가 붙어 있었다.
> 대입 전체를 한 seam 으로 묶어(기본 구현 = 기존 두 호출 ⇒ ②층 불변) ③층만 평면화: **0.540 → 0.517**,
> native/vm **1.71×**.
>
> **★ 다음은 구현이 아니라 판정이다.** doc-21 §5 S3 의 cranelift 절은 위 census 와 모순되므로, S3 를
> ⓐ 남은 기계장치(`settle` 8.3%·`write_lvalue`·NBA 큐) 제거로 재정의할지 ⓑ 4-state 규칙을 cranelift 로
> 내리는 비용/위험을 감수할지 ⓒ S3 를 닫고 S4 로 갈지를 **오너 판정**으로 정한다. 중단 판정의 원문은
> *"호출을 삼키지 못하면 중단"* 이고, 지금 삼켜야 할 "호출"은 프레임 호출이 아니라 **4-state 규칙 함수**다.
