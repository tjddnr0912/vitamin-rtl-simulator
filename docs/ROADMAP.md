...
> | **1b** | 🆕 **`{<string>, '1}` 이 무한 루프**(진단도 종료도 없음 · pre-existing · PRE==POST) | ⚠️ **correct-or-loud 의 최악 결과** — 틀린 답도 loud 도 아닌 **비종료**. `lower_expr`(expr_main.rs:12)의 front gate 가 `Concat`+fill 을 `lower_expr_ctx` 로 보내고, `lower_expr_ctx` 의 Concat arm 이 string part 를 보면 **같은 노드를** `lower_expr` 로 돌려준다. 둘 다 tail call 이라 릴리즈 빌드에서 스택도 안 자라고 **RSS 고정 3.6 MB 에 100% CPU**. `wire [7:0] a = {s, '1};` · `assign a = {s, '1};` 둘 다. **최소 조치는 loud 화**(비종료 ≪ loud 는 사다리 상승) · ⭐⭐ **2026-08-21 추가 실측 — 이 항목은 "지원"이 아니라 "loud"가 정답이다**: 이 모양엔 **오라클이 없다**(iverilog 는 `internal error: vvp_fun_concat::recv_string not implemented` + assertion 으로 **죽고**, verilator 는 **빌드 거부**) ⇒ 저장소 원칙 *"검증 불가면 구현하지 마라"* 가 그대로 적용된다. 그리고 **string-concat 경로 자체는 멀쩡하다** — `{s, 8'h0F}`(fill 없음)는 vita 가 `00001111` 로 정상 종료. 즉 **fill 이 사이클을 트리거하는 것 하나**이고, 고칠 것은 `lower_expr`↔`lower_expr_ctx` 의 상호 tail-call 을 끊어 **정직한 E3009 로 떨어뜨리는 것**뿐이다 |
