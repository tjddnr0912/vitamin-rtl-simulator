...
- 숫자 literal `%s` NUL-byte · `%d`-of-real width. §4.5.119/120 잔여.
...
- **VCD `$var` 잔여 fidelity**(§4.5.138 `[msb:lsb]` range 후 남은 iverilog 차이·전부 cosmetic·range는 일치): ① real/realtime size field=vita `64` vs iverilog `1` ② packed/logic vector VCD var-type=vita `wire` vs iverilog `reg`·`int`=vita `reg` vs `integer` ③ `parameter`를 `$var parameter`로 미덤프. + 근본: elaborate가 packed-multi-dim NetVar.lsb를 stale로 둠(lib.rs 8435/7862·현재 VCD helper서 flat `[width-1:0]`로 우회·sub-select는 packed-dims 메타 사용이라 무영향).
