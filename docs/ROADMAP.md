...
| ⑤ⓕ | unpacked-array typedef residue (§4.5.445 the declaration, the ANSI port and every declaration binder; §4.5.447 the NON-ANSI port and `$bits` of the bare TYPE name; §4.5.453/454 `$bits(a_t)` under a PARAMETER dim and its `pkg::` twin; **§4.5.456 the tf-port FORMAL — both binders, every direction, the comma continuation, 2-oracle on the DYNAMIC `[]` formal**): still loud — a function RETURN type (**1-oracle**, and iverilog does not merely refuse it, it SIGABRTs: `Assertion failed: (lwid == ivl_signal_width(lsig))`; needs an unpacked slot on the FROZEN `hdl_ast::FunctionDef` ⇒ format bump — worst evidence-per-cost in the row) · a `parameter` of that type (**1-oracle**, verilator `$bits` 24: iverilog refuses BOTH spellings, `Unable to evaluate parameter` for the typedef form and `sorry: unpacked array parameters` for the explicit one; the destination exists — `typedef_param_shape` need only route into the `ParamItem::ConstArrayVar(NetVarDecl)` channel the explicit `localparam` already uses, and the module-body overridable-`parameter` gate stays) · `parameter type T = a_t` (**2-oracle and CORRECT on both**, `$bits` 24 — the row's old "both oracles 32" is measured false and this is the strongest cell left; but it is a THREE-wall axis, `type_params.rs:94` first and then `:334` (`T$w`/`T$s`) and `:542`/`:583`, so ship only the `T v;` declaration subset or re-scope) · dims on BOTH the typedef and the declarator (`a_t y [0:1]`, a live oracle SPLIT on dimension ORDER — iverilog `$size(y,1)=4 $size(y,2)=2`, verilator `2` / `4`, and iverilog contradicts its own answer for the identical explicit type) (§4.5.453 shipped `$bits(a_t)` when a dim names a PARAMETER — the desugar now multiplies the element by every dim, so the answer is an EXPRESSION and follows the override; §4.5.454 shipped the package-SCOPED twin of the same shape) | each consumer reads `TypeInfo` and has no slot for unpacked dims; every one of them now DECLINES on `!info.unpacked.is_empty()` rather than binding the element type | per-consumer, each its own slice; the split row is do-not-start | 2-oracle except the function return type (1) and the declarator-dims split (0) | S each |
...
|---|---|---|---|---|
| 1 | 1 | §3 ⑤ⓕ residue, in oracle-strength order: `parameter type T = a_t` (**2-oracle, both correct, `$bits` 24** — but three walls, ship the `T v;` declaration subset only), then a `localparam` OF that type (1-oracle, cheap — route `typedef_param_shape` into the existing `ConstArrayVar` channel). The tf-port FORMAL SHIPPED (§4.5.456). The function RETURN type is do-not-start (1-oracle + frozen `FunctionDef`) | §3 ⑤ⓕ | ② |
| 2 | 2 | §2 🆕 I ⓒ residue: a wire index driven by exactly ONE constant continuous assign (`assign k = 2'd1; assign c = m[k];` — both oracles `a5`, vita `xx`). Needs a net-CONSTNESS test inside `word_const`; its `buf`-driven twin is a split, so the separating property is "one constant continuous driver", not "is a wire". The full-range word SELECT shipped (§4.5.456) and the plain RUNTIME index is refuted as a split | §2 🆕 I | ① |
| 3 | 3 | §2 row 7 residue that is NOT process order: an output PORT bind `child u1(.o(w))` and `assign w = u1.s;` read `xx` where both oracles read `ee`. Reached through the t0 structural SETTLE, so it is untouched by any process reordering — and row 7's own ordering half is BLOCKED (§4.5.456 built and reverted the permutation; the prerequisite is a per-resumption-kind ordering model) | §2 row 7 | ① |
