...
>
> **기준선(2026-09-06)**: format_version **30** · **6,452 tests green** · **워크로드 코퍼스 10/10** · 3-OS CI green · MsgCode **68** · **MSRV 1.85** · 기본 백엔드 **native**. 완료 슬라이스의 한-줄 요약과 상세는 **전부** [ROADMAP_ARCHIVE.md](ROADMAP_ARCHIVE.md)(상단 인덱스 · `#### 4.5.<N>` 검색)에 있다 — 이 문서는 전방 전용이므로 완료 서사를 두지 않는다(옛 헤더의 34-슬라이스 요약 체인은 2026-08-19 에 트림 · 전부 ARCHIVE 인덱스와 중복이었다).
>
...
- 🔴 **크로스바의 등록 valid 출력이 리셋 직후 x 여야 하는데 0 이다.** verilog-axi 가 처음 끝까지 돌면서
  ...
  **2026-08-31 국소화 (착수 시 여기서 시작하라 — 아래는 전부 실측)**
  ...
  - ✅ **CLOSED 2026-09-01 by measurement — `buf` is NOT a bit move.** The row read
    *"`buf b(o, i);` lowers to `assign o = ~(~i);`, whose rhs is an `Expr::Unary`, so the predicate
    refuses it"*, on the theory that the double negation merely hides a rename. It does not hide
    one: **that spelling IS the IEEE 1364 §7.3 z→x coercion** a gate input performs, and `gates.rs`
    says so at the site. Measured on an undriven wire: `buf b1(o1, zin)` gives vita **x** and
    iverilog **z**, while the neighbouring `assign o2 = zin;` gives **z** in both — so the gate
    computes, its output has an initial state like any other computed driver, and there is nothing
    to admit into the copy set. ⚠️ The vita/iverilog difference on that cell is a separate ruling
    (the LRM's table says x); pinned in `oracle_split_rulings.rs`.
  - 🔴 **FST loses the `$dumpvars` snapshot, and the cause is NOT the missing time step**
    (re-measured 2026-09-01). Two designs differing only in a declaration initializer produce
    **byte-identical 473-byte `.fst` files** in which every signal reads `x`, at exit 0. ⭐ The VCD
    the transcoder reads emits the whole `$dumpvars` block BEFORE the first `#t` (`$dumpvars … $end`
    then `#0`), so its `signal_change` calls had no open time step — but **opening time 0 lazily
    before the first value does not fix it**: built, measured (the arm fires exactly once), and the
    file is unchanged, still `xxxxxxxx`. ⇒ the value is being absorbed into fst-writer's per-var
    INITIAL value rather than recorded as a change, so the next step is that library's
    initial-value API, not the transcoder's time handling. The speculative change was **reverted**
    rather than shipped. ⚠️ It also makes FST useless as a waveform differential oracle — 24
    designs whose VCDs differ produced identical FSTs.
  - ⚠️ **오라클은 iverilog 뿐**(verilator 는 x/z 오라클이 아니다). vita 가 **낙관적**(x 자리에 definite)이라
    진짜 x 전파 버그를 가릴 수 있다 ⇒ silent-wrong 으로 분류.

## 2-P. 외부 리포트 성능 요구 — R8 ✅ **해결 (2026-08-31)**

> ✅ `wprog` 에 봉인 arm 을 넣어 해결. ⭐ **부호 게이트 완화는 필요 없었다** — 봉인의 피연산자는 *제 폭·제 부호*로 평가되므로(그게 봉인의 존재 이유) 자식이 자기 부호로 컴파일된다. ⚠️ arm 을 self-determined 넓히기 경로로 **재사용하면 틀린다**: `$signed` 는 채움을 **문맥 부호**로 강제하고 `$unsigned` 는 항상 0채움이라, 자식 부호를 읽는 일반 규칙(`sw.signed && signed`)과 다르다. 마이크로 **0.678→0.332s**(봉인 없는 식과 동률) · 768칸 iverilog 전수 일치 · 코퍼스 10/10 바이트 동일 · ⚠️ **우리 코퍼스는 ±1% 안**(이 모양이 우리 핫루프엔 없다 — §4.5.396 판정 재확인) · teeth = 봉인/비봉인 **op 수 동일** 어서션(봉인이 op 를 안 낳으므로 값 differential 은 발화를 볼 수 없다).

<details><summary>옛 등록 내용</summary>

- **부호 봉인(`$signed`/`$unsigned`)이 감싼 식이 컴파일 레인에서 떨어진다.** 리포터의 `top` 실행에서
  최다 builtin 호출이 **`$signed` 14,616,553회**인데 **소스엔 `$signed` 가 한 번도 없다**. 발생원은
  size cast: `expr_cast.rs:~330` 이 `n'(e)` 결과를 **항상** `SysFunc{Signed|Unsigned}` 로 봉인한다
  (계산이 아니라 **부호 도장**).
- ⭐ **실측(HEAD)**: `w[8*j +: 8] = 8'((a<<4)|b)` 160만 회 루프에서 봉인 있음 **0.678s** / 봉인만 뺀
  같은 식 **0.480s** ⇒ **봉인이 그 런의 29.2%**. 리포터 마이크로는 −9.7%(그쪽은 반환형을 바꿔 회피).
- ⚠️ **본문은 쫓겨나지 않는다** — 두 형태 다 `codegen able 2/2`. 비용은 **식 단위**다:
  `wprog` 에 **`SysFunc` arm 이 없어**(거절 목록에 명시) 봉인된 RHS 가 제네릭 평가기로 내려간다.
- ⚠️⚠️ **봉인만 승인해도 부족하다** — `$signed(x)` 의 self-sign 은 signed 이고 자식은 unsigned 라
  `compile_node` 의 **부호 게이트에서 자식이 거절**된다. 그 게이트의 set-wide 제거는 §4.5.396 이
  지어서·재서·**되돌렸다**(코퍼스 1.00×). ⭐⭐ **그런데 그 슬라이스의 큐 문구가 정확히 이 경우를 적어
  뒀다**: *"혼합부호 트리에서 2~4×인데 이 설계들 핫루프엔 그 모양이 없다"* — **리포터 워크로드엔 그
  모양이 1,460만 번 있다.** 즉 **되돌린 완화에 처음으로 측정된 수요가 생겼다.**
- **착수 형태 둘**(선택은 census 후): ⓐ `wprog` 에 봉인 arm + 부호 게이트를 **이 가족에 한해** 완화
  ⓑ elaborate 가 **봉인을 안 만든다**(폭이 안 변하고 부호가 문맥과 같으면 seal 이 inert) — ⚠️ 후자는
  `expr_cast.rs` 의 `trusted_w`/`signed_op` 논증 전체를 재검해야 하고 진단·폭 provenance 에 닿는다.
...
- **round-35 / aes_top R6 (2026-08-31) ✅ 해결** — 패키지 함수 안의 `case` 하나가 **설계 전체의
  elaboration 을 막았다**(E3009 · 인스턴스마다 1건 · 실측 201건). 아무것도 그 함수를 호출하지 않고
  `import p::*;` 만 해도 발화했다. ⭐ **원인은 `case` 가 아니다** — 패키지 루틴의 본문은 **두 키**(맨
  이름 · `pkg::name`)로 각각 예약되는데 §12.5 scrutinee hoist 의 capture 넷을 **소스 span 하나로만**
  키잉했다. 두 번째 예약이 첫 번째를 덮어 첫 프레임이 **다른 프레임 윈도의 넷에 쓰는 코드**를 낳았고,
  본문 검증기는 그걸 정확히 *"assignment to a net outside the function"* 이라고 보고했다 ⇒ 두 span 맵
  (`frame_case_tmp` · 형제 `frame_repeat_cnt` — `repeat` 도 같은 증상)을 **`(span, 소유 프레임)`** 으로
  키잉. ⚠️ 틀린 프레임의 조회는 이제 **miss** 이고, miss 는 hoist 가 이미 안전하다고 적어 둔 경로
  (arm 마다 재평가)다 · 클래스 메서드는 `in_frame_body` 를 세우지만 예약을 안 하므로 owner 를 **명시적으로
  비운다** · ⚠️ **모듈 스코프 쌍둥이는 늘 정상이었다** = *같은 해저드의 두 철자가 갈리면 신호* 그대로 ·
  ⚠️ 진단 문구는 **고치지 않았다** — 문구는 정확했고(vita 가 실제로 바깥 넷 쓰기를 만들었다) 틀린 건
  코드였다. 대신 그 문구가 말 못 하던 것(**누구의 넷인가**)을 준다: `$…` **컴파일러 생성** 넷이면
  그렇게 말하고 버그 리포트를 요청한다(소스로는 피할 수 없으므로) · 값은 iverilog 로 두 맵 + 중첩
  패키지 호출까지 검증 · 코퍼스 10/10 바이트 동일 · hoist 발화 유지 확인(꺼졌다면 keccak_f 가 +25%).
  ⚠️ **미착수 = 리포트 ③**: 미사용 패키지 함수도 프레임화되고 같은 원인이 **인스턴스마다** 보고된다.
  둘 다 사용성 항목이고 이 P0 와 다른 축이다.
...
> | # | 항목 | 왜 이 순서인가 |
> |---|---|---|
> | — | ✅ **해소된 행 25개는 ARCHIVE 로 이관**(2026-09-02 · 2026-09-05: 28 · 🆕 D · 🆕 E · 🆕 G = §4.5.419~421 묶음) | 1 · 1d · 1e · 1f · 🆕A · 2 · 2b · 3 · 4 · 6 · 8 · 8b · 9 · 11 · 12 · 13 · 18 · 21 · 22 · 27 · 28 · 🆕D · 🆕E · 🆕G — 원문·행번호 보존 = [ROADMAP_ARCHIVE](ROADMAP_ARCHIVE.md) 「§2 표 해소 행」. ⚠️ 번호는 재사용하지 마라(테스트 주석이 참조한다) |
...
> | 3b | 🆕 **클래스 속성만 남았다 — 구조가 다르다**(§4.5.359 · PRE==POST · ⏸ **2026-09-01 재측정: 착수 전 다른 갭에 막힌다**) | 클래스 필드는 **net 이 아니다**(`ClassField` → heap slot)인데 정규화 맵은 **NetId 로 키잉**된다 ⇒ 기록할 자리가 없다 · **선행조건 = 필드 키 정규화 맵** · ⚠️ 오라클도 하나뿐(iverilog 는 이 모양에서 assertion 으로 죽는다) · ⚠️ 게다가 최소 재현조차 **다른 이유로 loud** 하다: `C c = new();` 는 *"a class-handle declaration initializer is outside the N7 MVP"* 라 프로브를 절차 블록으로 다시 써야 한다.
...
> | 7 | 🔄 **REWRITTEN (§4.5.376) · 재측정 2026-09-01: 여전히 산다** — **a parent `initial` READING a child net at t0 sees X** (**2-oracle** · ⚠️ **zero corpus demand** · ⚠️ **format_version 30 을 요구한다**) | `module sub; logic [7:0] s; initial s = 8'hEE; endmodule` 를 부모가 `r = u1.s;` 로 읽으면 두 오라클 `ee` · vita `xx`. ⭐ 좁다: 선언 초기화·`always @*` 읽기·fork-arm 읽기·지연된 자식 `initial` 은 **전부 이미 정확**하고 직접 `initial`→`initial` 읽기만 틀리다. ⚠️ **사다리만으로 순위를 매기지 마라** — 코퍼스 열 워크로드 중 아무도 이 모양을 쓰지 않고, 고침은 `push_process` 에 rank 를 다는 것인데 `sim_ir::Process` 는 **동결**이라 사이드카가 필요하고 그 사이드카가 **format 29→30** 이다(v26 이 `init_procs` 로 똑같이 했다). 두 백엔드 모두에 실어야 한다.
> | 10 | 🆕 **A >64-bit package parameter's DECLARED range reaches one of four consumer × spelling quadrants** (§4.5.383 · **2-oracle** · ⚠️ pre-existing · **재측정 2026-09-01: 넷 중 둘이 이미 닫혔다**) | 오늘 실측: `pk::K[31:24]` = **dd** ✓ 정확하고 `$bits` 로 폭을 잡은 net 도 **128** ✓ 정확하다(기록된 *"204 vs 238"* 은 더 이상 재현되지 않는다). **남은 것은 bare-import 철자 하나** — `import pk::*;` 뒤의 `K[31:24]` 가 **11** 이고 두 오라클은 **dd** 다. ⚠️ 그 절반의 뿌리는 그대로다: wide import 는 `wide_param_bits` 에 바인딩하는데 `param_sel_range` 의 `walk_scopes_key` 는 거기를 보지 않고, 넓히면 값이 사는 두 번째 맵의 ~10 바인더에 staleness 가 돌아온다 ⇒ **`bind_param_value` 가 표현 불가능하게 만든 그 문제** · 두 절반은 여전히 **다른 머신러리**다.
> | 14 | 🔄 **BUILT, MEASURED, REVERTED 2026-09-01; re-built and re-measured 2026-09-05 (§4.5.423 review A B-1) with the same verdict** — the routing also fixes a TYPED declaration's `/ % >> >>>` under a wrapped intermediate (`localparam logic [7:0] M = (P + 8'd100) % 8'd7`, `P = 200`: 6, both oracles 2 — 17 lens designs), the package-consumer cells (`pk::PA ^ 64'h0` → `00…fe`) and `int S = (D - C) / 2` (2147483641), and reproduces the generate-scope `time NM` shadow (2c for 12c) and the 65-bit-leaf `/ %` at once. **The fix is right and the walk it needs is not** (**2-oracle** · silent-wrong · the diagnosis below is now complete, so the next attempt starts from measurements) | `localparam logic signed [7:0] NM = -8'sd2; localparam logic [63:0] X = NM ^ 64'h0;` is `fffffffffffffffe` and both oracles give `00000000000000fe`. ⭐⭐ **vita contradicts itself**: the same expression over a FUNCTION LOCAL folds `00…fe`, because a local's declared width lives in `envw` and the module-scope initializer folds through the width-UNLIMITED `const_eval_in_scope`. ⭐ **What was built** = route an initializer whose target has DECLARED width/sign through `eval_const_assign` (the width-aware entry the constant-function interpreter already uses). It works: 44-cell census **27 divergent → 3**, §4.5.366's four-operator module-scope residue closed, and the pinned `>>>` self-contradiction (`18446744073709551615` constant vs `4294967295` runtime) gone. ⚠️⚠️ **Reverted after THREE review rounds and SEVEN BLOCKING.** Rounds 1–2 were mine and were fixed — the gate must demand provenance of every LEAF as well as the target (`param_meta` is a DEFAULT for an untyped parameter and ABSENT for a `time` one), decline above the i64 lane (else it returns the correct value TRUNCATED = a silent-for-silent trade), refuse an unsized FILL operand (the old fold's overflow DECLINE was the only thing keeping `'1`'s mis-sizing honest), answer THREE-valued in the scope walk (a `remove` is indistinguishable from *"this scope never bound the name"*, so an inner `localparam time NM` let the walk vouch for a module-scope `logic [7:0] NM` and `NM | 64'h0` came out **44** where all three tools say 300), and NOT route the PACKAGE binder (row 26). ⚠️⚠️ **Round 3 found two more, both in the SHARED walk and both there since round 1**: ⓐ §11.4.10 makes a shift's RIGHT operand self-determined and UNSIGNED and the walk pushes the assignment context onto it, so a narrow signed count sign-extends and `16'hFF01 << 3'b101` becomes **0** (30 correct→wrong + 36 loud→wrong) — ⭐ **PRE-EXISTING and reachable today through a constant FUNCTION with no routing involved** ⇒ its own row, and it must close first; ⓑ the i64-lane bound was on the TARGET only, so a `logic signed [64:0]` LEAF whose value fits an i64 passed the provenance test and was evaluated as a 64-bit operand, changing what `/` and `%` answer (83 cells). ⇒ **prerequisite = a width-aware walk that is correct on its own terms**, which is shared with the constant-function interpreter and is a different blast radius. `param_declared_width_provenance.rs` pins the reverted state, the two prerequisites and the cells a fix must not move.
> | 15 | 🔄 **RE-GROUNDED, then BUILT/MEASURED/REVERTED 2026-09-01 — the startable half needs a fact the AST does not carry** | **The silent-wrong half is real**: a parameter OVERRIDE carrying a sized x/z literal loses the unknown plane — `#(.K(8'b1010_010x))` onto `parameter logic [7:0] K` binds `10100100` at exit 0 where both oracles keep the x, and `8'bzzzzz1z0` binds `11111110` because z's value bit is 1. Five cells, every override channel. Site = `params.rs`'s *"does it fit the i64 lane"* test, which reads only VALUE bits (`bp_get(..).0`); the sibling `fill` arm twelve lines up already declines with `fill_is_unknown`. ⚠️⚠️ **Refusing with `bp_any_unknown` was shipped and reverted: it turns 76 CORRECT cells loud.** A 2-STATE declaration (`bit`/`byte`/`shortint`/`int`/`longint`) converts x and z to 0 on assignment, so dropping the plane IS that conversion and the i64 lane was exactly right — `parameter bit [7:0] K` with `#(.K(8'b1010_010x))` is `10100100` in vita AND iverilog. ⭐⭐ **The discriminator is not in the AST**: `hdl-parser/src/params.rs` computes `var_kind` for exactly those keywords and DROPS it, because `ParamDecl` has no such field — the same gap its own comment documents for the 1-bit-`logic` range. ⇒ **prerequisite = record the parameter's 2-state-ness** (an `hdl-ast` field + a SchemaHash re-pin; parser-only, so no `sim-ir` format bump — §4.5.384 precedent). ⭐ That one field closes a sibling silent-wrong this arm cannot see either: on a 2-state declaration **z must also become 0**, and today it becomes 1. ⚠️ The row's ORIGINAL headline (a real unknown plane in the narrow store, for an x/z LITERAL initializer and a constant OOR select — 22 loud cells) stays a separate, larger item: ~40 read/write sites, corpus demand 0, and it stacks on row 14. ⭐ The >64-bit asymmetry is exact and was measured in both directions: what survives above bit 64 is **z** (value bit 1), not x.
> | 16 | 🔄 **RE-GROUNDED 2026-09-01 — MOSTLY FOLDS INTO ROW 17's ORACLE SPLIT; only one sub-case survives** (⚠️ row 21 landed and this row is **still open by construction**) | 12 override cells measured: 5 diverge from iverilog, but **verilator sides with VITA on 4 of them** (`-64'd1`, `<ones> + 64'd1`, `<ones> << 4` — verilator folds the override at its own width then zero-extends), and `~32'd0` is a **3-way split**. That is row 17, and it swallows most of this row. ⚠️ **A fix would "correct" one side of a live oracle split** ⇒ row 21's slice deliberately did NOT thread the context through `override_bits`, and review measured the channel unmoved across five spellings. **The only 2-oracle actionable sub-case** = operands already ≥ the target width: `#(.K(~128'd0))` is vita `00…00ffffffffffffffff` vs both oracles all-ones.
...
> | 23 | 🔄 **RE-GROUNDED 2026-09-01 — the collision half is a FALSE REJECTION, not a message-quality issue** | ⚠️ The recorded claim *"the collision always co-occurs with a pre-existing `redeclared` error"* **inverts cause and effect: the `redeclared` IS the defect.** `clocking cb; input a_b;` beside `clocking cb_a; input b;` is a **legal** design (verilator runs it, `R1=17 R2=34`) that vita refuses with ``net/variable `top.__clk_cb_a_b` redeclared`` at exit 1 — nothing in the source is redeclared; vita's own synthesized name collides with itself. Same on the output side (`__clkout_`). ⇒ **correct→loud, verilator is the accept/reject oracle** ⇒ this half belongs in §3. Site = the `__clk_`/`__clkout_` mangling in `sva_clocking.rs:727`/`:657`; the separator is VCD-visible, and §4.5.374 showed a new sigil must be taught to the VCD **and** FST filters. ⭐ **The naming half is a one-token fix**: `sva_clocking.rs:745` already computes the instance-qualified `alias` and line 750 re-formats the identical expression **without** `fq` — clone `alias`. The missing `[in …]` suffix is the `lvalue.rs:179` path, not `const_eval.rs`. Fold into any slice that touches this file.
> | 24 | 🔄 **RE-GROUNDED 2026-09-01 — FAR WORSE than recorded, and it SPLITS: declaring a clocking OUTPUT destroys the signal with no write anywhere** | ⚠️⚠️ The row says *"a write lands as x"*. Measured: the write path is **correct at steady state**, and the damage happens **with no write at all**. **24a (CLOBBER · silent-wrong · exit 0 · verilator is the oracle · STARTABLE)** — 4 cells including one **across a module boundary**: a signal merely DECLARED as a clocking output is destroyed to `x` or frozen forever (`vita x,171,171,171,171,171` vs `verilator 170,171,172,173,174,175`), so an ordinary RTL driver stops working. ⭐ Site = `sim-engine/src/state/init_diag.rs::clocking_commit_plan` (~1202), whose OUTPUT phase is **unconditional** — no *"was the holding net assigned since the last edge"* gate — so the x-initialised holding net is driven onto the source at the first edge and re-driven forever. The INPUT phase in the same function is correct and must not move; the written flag has to be produced at the write site, so `sva_clocking.rs`'s `out_pairs` grows a third field — check it rides `SimOpts` out-of-band like `clocking_inputs` does (no format bump). **24b (one-cycle LAG · silent-wrong · NOT STARTABLE)** — 4 cells land one clock late (§14.16's default output skew `#0` in Re-NBA of the same event); it is a scheduler-REGION question and ROADMAP N4 already flags that axis as having **no anchor** (iverilog cannot parse clocking, verilator samples in Observed). ⚠️ Corpus demand **0** — `endclocking` appears only in `bench/ibex/src/dv/` UVM interfaces; ibex is not a corpus row, no row compiles a `dv/` path, and `examples/` has none. ⇒ **do not start on demand grounds**, but 24a outranks most of §2 on the LADDER and is decidable today.
> | 25 | 🔄 **BUILT, MEASURED (3 review rounds), REVERTED 2026-09-02 — an override does not retype an UNTYPED parameter; the fix is at ONE producer and its patch is kept** (§4.5.405 · **2-oracle** · silent-wrong · pre-existing, PRE == POST) | `parameter P = 5` + `#(.P(32'hF0F0F0F0))` binds a SIGNED 32-bit −252645136 (`P < 0` = 1, `64'(P)` sign-extended, `%0d` negative); `parameter Q = 8'sd1` + `#(.Q(32'hDEADBEEF))` is `ef` with `$bits(Q)` 8 — both oracles bind the override's own type (§6.20.2). ⭐ **Site**: `params.rs::param_decl_width_opt`'s literal arm answers the DEFAULT's literal type even when `default_binds == false`, and `bind_one_param`'s chain has no channel carrying the override's type (`ResolvedOverride` carries `signed` but no `width`). ⭐ **The fix was built** (`scratchpad/r29/row25/producer.patch`, 317 lines, + its 5 oracle-pinned tests): `width` channel on `ResolvedOverride`/`ParamOverrides` (`const_self_width` at the parent; `#()`, interface, `-G`, `defparam` collector), the decline gated on `declared_only && !default_binds && Implicit`, `param_range` from the override's width, `override_at_declared_width` resizing to the declared type or the override's own. It fixed **7,982 cells** in a 122,774-cell override matrix and serv's `|WITH_CSR` — and each of three review rounds found a NEW correct→silent edge in it: the `defparam` channel with a NAME rhs (the collector types only sign-evident literals, so the default's type came back), a `time` parameter with a DECIMAL default forwarded (`const_self_width` guesses 32 for a meta-less name), `$signed(64'h…)` (`SysCall ⇒ 32`) resized down before being read wide, fills. ⚠️ **Three consecutive rounds of blockers on one axis = revert** (ENGINEERING_RULES); the size-cast slice ships with the classifier DECLINING every guessed type instead (`param_type_guessed`: an overridden untyped parameter and every body constant whose initializer reads one). ⭐ **Next attempt starts from the patch and these three edges**: type the override at the parent with a resolver that DECLINES on meta-less names and answers `$signed/$unsigned` by operand (the `const_self_width` guesses are the root of two of the three), carry the pair through every channel including `defparam` names, and a fill onto an untyped parameter binds `(1, false)` (both oracles). |
> | 26 | 🆕 **A package parameter's stored VALUE cannot be made canonical until its CONSUMERS fold width-aware** (§2 row 14 round-2 review · **2-oracle** · built, measured, REVERTED) | Row 14 routed the module and generate binders through the width-aware fold; routing the PACKAGE binder as well looks like the same one-line change and is a **net loss**: it makes `pk::X`'s stored value canonical (i64 −2 for `logic signed [7:0] PA = 8'hFE`) while every consumer of `pk::X` still folds through the width-unlimited walk, which has no context to convert the leaf into and sign-extends it. Measured over **8,748 package-consumer designs: 1,233 correct→silent-wrong against 714 fixed**, plus one correct→loud (`(pk::PA >> 8'h0)` stops folding over a negative i64). Affects every signed package declaration form whose value has the top declared bit set, through all three read spellings equally. ⭐ **The prerequisite is on the CONSUMER side**: `every_name_has_a_declared_width` does not enumerate `ExprKind::PkgScoped`, and an imported constant is in no provenance set — closing both turns those 1,233 cells correct instead of wrong. ⚠️ Until then the package fold stays byte-identical to what it always was, which means the identical text answers `00…fe` in a module and `ff…fe` in a package. That asymmetry is a MEASUREMENT, not an oversight; `param_declared_width_provenance.rs` pins it. |
> | 30 | 🔄 **BUILT, MEASURED (3 adversarial rounds), REVERTED 2026-09-03 — the fix is right, the domain it lives in is not sound enough to carry it** (**2-oracle** · **165/264 silent-wrong** · every number below is HEAD 3-tool) | `localparam logic [127:0] C = '1 ^ 1'b0;` → vita `…00000000ffffffff`, both oracles 128 ones. ⭐⭐ **vita contradicts itself**: the same text as `r = '1 ^ 1'b0;`, as `assign c = …` and through a port all print 128 ones — only the CONSTANT domain is wrong, so the direction is not in doubt. ⚠️ Not >64 bits and not two spellings: **165 of a 264-cell census** (22 operator forms × {32,33,64,65,96,128} × {`logic`,`logic signed`}), 0 oracle splits, in all four binder copies. The row's old band (*"wrong from 33 up"*) is a property of ITS operands — every form pairs the fill with a ONE-BIT sibling; give it a wider one and narrow declarations are wrong too (`logic [7:0] A = '1 >> 2` is `ff`, oracles `3f`; 82 more cells at widths 1..31). ⭐ **The fix is 4 pieces and they work**: `const_wide.rs`'s `fold_bits_at` fill arm folds at `ctx` when `ctx>0` and the fill is not x/z · a `param_i64_fill_at_declared` ahead of the i64 walk in all four binders · a LOCAL predicate with a `Cast` arm (the shared `expr_contains_fill` has none, and its other callers are the runtime lowering routes) · a `fill_width_survived_the_fold` guard. Measured: **778 FIXED / 0 new-silent / 0 new-loud** over the lenses' 1,622-cell sweep, 48 x/z cells still loud, corpus 10/10, staged byte-identical. Patch = `scratchpad` slice diffs; the three round-fixes are in git history's review notes. ⚠️⚠️ **Reverted because each round's fix produced the next BLOCKING defect, and round 3's is on a DIFFERENT AXIS in code this slice only routes into.** R1 — freezing the fill at `ctx` at the LEAF is wrong: §11.6.1 evaluates at `max(ctx, EVERY self-determined operand's width)`, so a wider sibling makes the operator ZERO-extend the frozen fill (150 cells, 36 of them loud→silent). R2 — the guard reused the ROUTING predicate, which reports a fill in a SELF-determined position too, so it declined folds PRE performed correctly (104 NEW-LOUD; and `param_bits_at_declared` sits at the LAST link of every binder chain, so a decline there is `E3009`, not a fall-through — the *"declining is always safe"* argument was false). R3 — with the left operand folding, `'1 << '1` exposed the shift COUNT's own hard 32 (**shipped separately**, see below), and **the soundness lens then found the wall**: of 504 PRE-loud→value cells the slice opens, **215 are silently wrong on the SIGN axis** — `localparam logic [7:0] B = ($signed(4'hF)+1) | 8'h00;` is `00` against both oracles' `10` **with no fill anywhere, in PRE and POST alike**, because `fold_bits_at` decides an expression's sign NODE-LOCALLY (`sg = ls && rs`, `const_wide.rs`) where §11.8.1 makes the whole region unsigned if ANY operand is. ⭐⭐ **PREREQUISITE = §11.8.1 region sign in the wide fold** (row 14's declared-sign provenance wall, seen from the constant side). Until it holds, opening the fill spellings trades a loud for that silent-wrong. ⭐ **Next attempt starts from the 4-piece patch and these three edges**, and its ACCEPT set must be *"correct a value, never create one"* — which the chain cannot express today, because the fill arm is visible to `param_i64_at_declared` at the end of every chain as well as to the new link. |
> | **🆕 C** | ✅ **RESOLVED (§4.5.409)** — the i64 constant lane sizes an unsized fill by its CONTEXT: `const_self_width` gives a fill no width of its own (`Some(0)`), the width-aware walk folds a fill leaf at the region width (one bit with nothing around it), an implicit parameter's fill-bearing initializer takes that walk's value and width (`untyped_fill_init`), a fill OVERRIDE of an implicit parameter is one unsigned bit (§6.20.2, verilator), `$bits('1)` is 1, and a right shift by `'1` shifts by one (the §4.5.406 residue). Census 93 cells: **21 silent→correct · 0 regressed** · `-G U='1` = verilator · examples byte-identical. ⚠️ The root (`parse_int_literal`'s `unwrap_or(32)`) is NOT changed — it is the container the module-scope unlimited walk and the declared-context binders still read, and those are right through `expr_as_fill` / the wide fold; what moved is every consumer that had no context to size against. Residue → 🆕 J. |
...
> | **🆕 I** | **row 33 residue (2026-09-03 · §4.5.408 census · all 2-oracle)** | ⓐ **another process's read in the same delta**: `initial #1 v = 8'hA5;` declared before `initial #1 $display(c);` — both oracles `a5` (declaration order runs the writer first and the copy is collapsed), vita `00`. A §5.4.1 race; kept on the settle's value ON PURPOSE (the store-side forward that would fix it broke picorv32 / UDP / keccak parity). ⓑ ~~multi-driver with a z driver~~ ✅ **RESOLVED (§4.5.433)** — an all-`z` whole-net constant driver at the net's width is a null driver (`alias::null_driver`), dropped from the move and multi-driver counts; a narrower `4'hz` (zero-extends) or a partial `8'hzx` still computes (iverilog `X5` / `ax` at t0, PRE == POST); two real drivers beside the `z` stay a resolution. ⓒ ~~constant-index array-word source~~ ✅ **RESOLVED (§4.5.433)** — `copied_source` admits a constant in-range word of a flat array and the alias carries the word (`alias_word`) to all three readers; 34 cells, 11 fixed-silent. Residue: a RUNTIME index `m[k]` (both oracles `a5`, iverilog propagates eagerly; PRE == POST `xx`), a 2-D word `m[1][2]` (an ORACLE SPLIT at the same-delta read — iverilog `a5`, verilator `00`; the word tree carries the §7.4.6 bounds ternary, which the fold declines), an `integer` array into an unsigned copy (ⓕ's sign rule; also a split — iverilog `xx`, verilator `a5`), a 32-bit unsigned literal `m[32'hFFFFFFFE]` and `m[64'd0 - 64'd2]` (splits: iverilog `xx`, verilator `a5`; vita E4002 / W4029), ~~a NEGATIVE-base word `m[-2]` on `m[-2:1]`~~ ✅ **RESOLVED (§4.5.436)** — `alias::word_const` folds the word-index tree the way the engine evaluates it (§11.6 / §11.8.1 context width and sign; `Concat`/`Replicate`/bit-`Select` seals as self-determined leaves) instead of the saturating width-tree folder, so `m[-2]`, `m[-1]` on `m[-2:1]` / `m[-4:-1]` / ascending `m[1:-2]`, `m[P]`, `m[-2+1]`, `m[2-4]`, `m[-2*1]`, `m[4'sd14]` / `m[8'shFE]` / `m[-16'sd2]` (sign-extended) and `m[4'd14 + 4'd4 - 18]` (context-width sum) all read through; 32 cells, 15 fixed-silent; the fold gates ADMISSION only (the alias carries the index eid the engine evaluates). Still not folded (review A, PRE == POST `xx`): a GENVAR-derived index `assign cw[g] = m[g-2]` inside a generate loop (iverilog `a5 5a`; verilator `00 00`, not an oracle on a negative-base array), an all-`z` driver beside a partial or delayed driver (E3001, loud), a `force`d copy after `release` (never reads through, pre-existing). ⓓ ~~a blocking write the writer predicate cannot see~~ ✅ **RESOLVED (§4.5.438)** for a frame body — `proc_read_alias` (moved to `levelize/read_through.rs`) walks every frame body the process runs (a `Terminator::Call` target resolved through the task-call sidecar's `callee`, an `Expr::Call` function, transitively) for its write set and its reads. A `fork` arm's write was already seen (measured `a5`). ⓔ **2-state destination** `bit [7:0] c; assign c = v;` with x/z in `v`: excluded (the settle's write coerces to 0; a read-through would not) — ⚠️ unexercised: vita refuses `bit`/`wire bit` copy destinations at elaborate (`E-ELAB-LVALUE-KIND`), so the guard is dead until that lands. ⓕ **sign-mismatched copy** `wire signed [7:0] c; assign c = v;` (unsigned `v`): both oracles read through with the COPY's sign (`r = c` → 4294967295); excluded, because the read substitutes the net and the interpreter/VM take the extension sign from the slot (255) while the native compiled path takes it from the node (4294967295) — the review's backend split. 2 census cells + the widening shapes; ⚠️ a mismatch ANYWHERE in a chain disables the tail below it (`v(u) → c(s) → d(u)`: `d` stays stale too, review-measured). Fix = re-stamp the copy's declared sign on the aliased read in `eval_core` AND `read_scalar_words` (the VM's leaf path reads the slot's sign). ⓖ ~~an `automatic` task's actual~~ ✅ **RESOLVED (§4.5.438)** — the in-bind actuals (the `TaskCallProc` / `TaskCallFunc` sidecars) and the callee bodies' own reads are marked; 26 cells, 10 fixed-silent, 0 regressions (a recursive task, a nested pair, a recursive function, a `disable` inside the callee, a call from a generate process). **Residue (pre-existing, recorded):** a callee body is ONE set of expressions, so it is marked only for the roots EVERY calling process writes — a task shared by a writer and a non-writer keeps the writer's read on the settle's value (`xx`, both oracles `a5`; review A sized it at 3 of 4 cells for two callers with disjoint write sets); an `always_comb` whose only read is inside a called task never runs (`xx`, iverilog `a5` — the callee's reads are not in the sensitivity derivation, a different root); an array-WORD copy (`assign c[0] = v[0]`) and a sign-EXTENDING copy (`logic signed [7:0]` → `[15:0]`) are not aliases at all (review A F-4/F-5, `xx`, iverilog `a5`/`ffff`). |
> | **🆕 J** | **🆕 C residue (2026-09-03 · §4.5.409 census · 2-oracle unless noted)** | ⓐ **a fill as the LEFT operand under a TYPED declaration** — `localparam logic [7:0] A = '1 >> 2;` is `ff` (oracles `3f`), every `logic [39:0] U = '1 <op> 1'b0` cell — that is **row 30** (the wide fold's fill arm + §11.8.1 region sign), 11 census cells, PRE == POST. ⓑ **a fill in a declaration BOUND** `wire [('1)+2:0] x;` — both oracles 2 (`'1` sized to the unsized 2's 32 bits, wraps), vita loud (the bound goes through the unlimited walk); loud, not silent. ⓒ **`$bits` of a fill under an operator** `$bits('1 & 4'hF)` — both oracles 4, vita loud (`bits_of_selfdet` has no operator arms). ⓓ **`{'1, 1'b0}`** — illegal (§11.4.12 forbids an unsized operand in a concatenation), both oracles lenient (2), vita loud; keep. ⓔ **`v['1]`** — oracle split (iverilog 0, verilator 1), vita = iverilog. ⓕ **`localparam U = '1 * 2'd2` / `'1 + 1'b1` / `4'd8 - '1` widths** — verilator sizes by the sibling (2 / 1 / 4), iverilog one wider (3 / 2 / 5); values agree; vita = verilator. ⓖ **`$bits` of a DERIVED untyped localparam** `localparam U = '1; localparam Y = U + 4'd1;` → `$bits(Y)` 32 where both oracles say 4/5 — fill-INDEPENDENT (`localparam U = 1;` shows the same 32), the row-14 value-inferred tail (`min_signed_bits(v).max(32)`), merely unmasked here by the loud removal. |
> | **🆕 K** | ✅ **RESOLVED (§4.5.410)** — two pre-existing silent-wrongs the §3 ⑤ census exposed on the KEYWORD spelling (a control twin per cell): ⓐ **a package's derived constant read a sibling's UNTRUNCATED initializer** — `package p; localparam logic [3:0] P = 5'h1F; localparam W = P + 1;` folded W = 32 (both oracles 16; `int I = 40'h1_0000_0001; WI = I + 1` gave 4294967298 for 2) because `elaborate_package` bound the unlimited i64 walk's value without the `coerce_param_value_with` the module-body and generate twins apply; a module reading `P` was right (15) — only the in-package fold was wrong. ⓑ **a wildcard import bound a package constant OVER a module-local declaration of the same name** (IEEE §26.3) — `import p::*; logic [5:0] P = 42;` read the package's 35 (both oracles 42) for a VARIABLE, a header `parameter P`, a package variable and a >64-bit constant alike; a local `localparam` was right only because its own later bind overwrote the import's. `apply_import_consts` now skips the module's `gather_local_decl_names` set on the wildcard arms (consts, wide consts, package vars); an explicit `import p::P` over a local `P` stays the loud `add_net` conflict. Residue → 🆕 L. |
> | **🆕 M** | **§4.5.419~421 batch residue (2026-09-05 · every cell PRE == POST · loud or oracle-split, no silent)** | ⓐ **a fill inside an INSTANCE-OVERRIDE expression** `m #(.P('1 ^ 1'b0)) u();` onto `parameter logic [39:0] P` — the parent folds the override before the target's width is known (`resolve_param_overrides` → `ovr_by_name`), so the fill is 32 bits (`00ffffffff`); iverilog sizes it to the target (`ffffffffff`), verilator to ONE bit (`0000000001`) — an oracle split, 40 pre-existing + 9 split cells of §4.5.420's census; the prerequisite is a target-typed override evaluation (row 17's axis). ⓑ **`(|'1)` and `{('1 ^ 1'b0)}` in a constant** stay loud (a reduction of a fill has no width source; a concat part is self-determined). ⓒ **`$bits(u.K)` / `u.K[7:0]` / `u.K[0]` on a hierarchical PARAMETER** are loud at EVERY width including 64 (`lower_bits_fold` lowers a still-deferred read; `resolve_deferred_hier_sel` resolves nets only — it now says so: `a bit / part-select of the hierarchical parameter … is unsupported`; `$bits` still reports "argument shape unsupported") — a class, not the wide side map. ⓓ **an EXPRESSION override of a >64-bit parameter** `m #(.K(128'h… ^ 128'd3))` is `W3056 override … is not a constant; default kept` + exit 1 (pre-existing; the literal override folds). ⓔ **`cover property (… (a |-> b))`** is loud — `cover.rs` has its own sequence-only grammar (§4.5.419 stripped the parens for `assert`/`assume`/named properties only). ⓕ verilator prints NO failure for `a |-> b and b |-> a` where IEEE §16.12.8 fails at the first failing operand (vita fails at t=35 on both spellings) — verilator is not an oracle for property-level `and`. **ⓒ ⓓ ✅ RESOLVED (§4.5.424, 2026-09-05)** — a bit / part / indexed-part select of a hierarchical parameter (any width, non-zero LSB normalised through the persistent `hier_param_range`), `$bits` of a hierarchical net or parameter (deferred like a whole read), and a bitwise `& \| ^` expression override of a wide parameter (`wide_ext_invariant_bitwise`, folded in the parent at the operands' width and resized in the child); an UNTYPED, unranged parameter overridden through the wide channel now takes the override's own width (§6.20.2 — the literal spelling `#(.UK(64'h…))` onto `parameter UK = 128'h2` printed 128 bits, row 25's wide half). **Residue:** a select of an ascending (`[0:7]`) or value-sized hierarchical parameter stays loud; `$bits` of a hierarchical string is loud (oracles split 1/16); an override that CARRIES past the operands' top bit (`~`, `+`, `<<`, unary minus, `?:`) stays loud; a fill inside an override expression is still 32 bits in the i64 lane (ⓐ, both oracles 128 ones when a wide sibling sizes it); a decimal / `-(64'sd1)` override of an untyped 128-bit-default parameter keeps 128 bits (row 25's i64 half — the i64 channel carries no width). |
> | **🆕 N** | **`%m` inside a named block prints the instance scope only (2026-09-05 · §4.5.422 census · pre-existing · 2-oracle)** | `initial begin : blk $display("%m"); end` prints `top` where both oracles print `top.blk` (nested `top.blk.inner`); a statement label `L: $display("%m")` inherits it (`top`, verilator `top.L`). `%m` is rendered by the engine from the per-process `proc_scopes` path, which knows no block labels; a fix needs either a per-statement scope sidecar or a lowering-time rewrite of the format literal under a block-label stack with an inline-task guard. Also pre-existing, illegal SV accepted: two `L:` labels in one block and a label spelled like a net (verilator: duplicate / unsupported). **✅ RESOLVED (§4.5.426, 2026-09-06 · format 29 → 30)** — elaborate keeps the named-block label chain while lowering and records it per system task (`stmt_scopes`) and per `$sformatf` (`expr_scopes`); the engine appends it after the frame names (`render`), for the dispatch seam, both `k_sformatf`s and the frame executor's own arms; strobe/monitor captures keep the registering chain; the staged trailer carries both maps. 32-cell census: 17 fixed-silent, 0 regressions. **Residue:** a `generate if` singleton scope renders `top.g[0].blk` where both oracles print `top.g.blk` (the instance-path naming of `g` ⇒ `g[0]`, pre-existing — PRE printed `top.g[0]`); an unnamed block contributes no segment (iverilog invents `$ivl_for_loop0`, verilator `unnamedblk1`); a fork's own label (`F: fork … join`) is not a scope segment yet (`top` for verilator's `top.F`, pre-existing); a class method's `%m` omits the class name (`top.show` for `top.C.show`) and a recursive frame subroutine stacks its own name per level (`top.f.f.fb` for `top.f.fb`) — review B F4/F5, pre-existing. Review A F2 widens that to a CLASS: `%m` inside a frame names vita's DYNAMIC call chain (recursion `top.f.f.f`, a module task called from a generate block `top.gl[0].t`, a callee under its caller `top.b.a`, a class method without its class) where both oracles name the STATIC declaration scope (`top.f`, `top.t`, `top.a`, `top.C.show`) — `frame_scope` is a stack of the frames on the call path; the LRM wants the subroutine's declaring scope. Duplicate labels / a label spelled like a net are still accepted (illegal SV). **Residue ⓐ ✅ RESOLVED (§4.5.429)** — `%m` and the diagnostic context render a singleton generate scope (`if` / `case` / nested) as `label` (storage stays `label[0]`; `display_prefix`, keyed on the new `gen_singleton_labels` — review B B1: a first draft keyed on "not a loop label" and stripped an instance-array element `w[0]`), and a fork's own label is a scope segment; 21 cells, 13 fixed-silent, 0 regressions. **Residue ⓑ ✅ RESOLVED (§4.5.432)** — an unnamed generate block is `genblk<N>` (§27.6: N counts every construct in its scope, named or not; nested scopes and loop iterations restart at 1; an `else if` chain is one construct) with its own storage segment `genblkN[0]` (nets scoped, `genblkN.v` resolves, instances `top.genblkN.u`), an unnamed loop is `genblkN[i]`, and the taken ELSE scope of an `if/else` with different labels is named by the else block's own label (the parser keeps it as a `GenItem::Block`); 22 cells, 17 fixed-silent, 2 silent→loud where both oracles are loud. **Still open:** VCD `$scope` names the block `gi[0]` / `genblk1[0]` (iverilog `begin gi` / `begin genblk1`); a task declared in an unnamed block is an oracle split (iverilog `top.genblk1.t`, verilator `top.genblk1.genblk1.t`; vita loud); a user block explicitly named `genblk1` beside an implicit one is not disambiguated (iverilog `genblk01`, verilator refuses; vita gives both the same name — review B); `%m` inside a concurrent assertion's action block omits the assertion label (`top.nb` for verilator's `top.nb.ap`); ~~the F2 dynamic-chain class~~ (✅ §4.5.435, above); the recorded leniency `gi[0].x` on a conditional scope (both oracles reject); ~~`%m` inside a function/task body called from a generate scope names the CALLER's scope~~ **✅ RESOLVED (§4.5.435)** — the F2 dynamic-chain CLASS is closed: `%m` in a module subroutine's body names its DECLARING scope (IEEE §21.2.1) — `top.t` from a generate-block process (was `top.gi.t`), from a generate loop, from a fork arm, through another task (`top.a`, was `top.b.a`), at every level of a recursion (`top.f`, was `top.f.f.f`), for an inlined body and a frame alike, with the body's own labels appended (`top.t.ib`, verilator's spelling; iverilog drops a label inside a task), for `$sformatf`, `$strobe` (iverilog's `top.t`; verilator prints `top`), `$error`; elaborate roots the recorded chain at `inst_prefix` (an absolute `.top.t…` string, `block_scope_root`) and `func_names` carries `.<inst>.<name>`. 43 cells, 19 fixed-silent, 0 regressions. Residue (pre-existing, recorded): ~~a CLASS method still prints without its class~~ ✅ **RESOLVED (§4.5.439)** — `%m` in a class method is `<process scope>.<class>.<method>` (+ the body's own labels; the `$class$C$m` storage segment no longer leaks), a virtual call names the dispatched method's class (hand-IEEE §8.20; iverilog names the base), and the `[in …]` context of every located runtime diagnostic (`$error`/`$warning`/`$info`/`$fatal`, W4029, E4002, `$readmem`) inside a frame / inlined task / class method is the declaring scope (`top.t`, `top.t.ib`, `top.C.show` — was `top.$func$t` / `[in top]` / `$class$C$show`); 28 cells, 16 fixed-silent, 0 regressions. **Still open** (same root: the class table is global, its declaring INSTANCE unknown, so the CALLING scope is prefixed): a class declared in a module but called from a generate-block process (`top.gi.C.show`) or from a frame TASK (`top.t.C.show`) where both oracles print `top.C.show` — and `%m` (process scope) vs the diagnostic context (`cur_scope`) can differ for one such call; a `$unit` class prints `top.C.show` (`$unit::C.show` / `$unit.C.show`, a split); a package class and a class method calling a module task / `$strobe` inside a class are loud; a parameterized class prints its mangled `C__8` (verilator `C__N8`); an ELABORATE-time diagnostic inside a class still spells `[in $class$C$m]`; a process's own context of a `$error` inside a named block is the instance (`[in top]`, iverilog `top.blk`); a package function is `top.pf` (iverilog `p::pf` / verilator `p.pf`, a split); `--hier-tree` / `--inst-paths` list no generate scopes at all. Seen beside it (pre-existing, loud): an instance ARRAY of a PORTLESS module (`ch w[1:0]()`) is refused with the message "child has non-ANSI ports" — a false reason on a valid design. |
> | **🆕 L** | **§3 ⑤ residue (2026-09-03 · §4.5.410 census · pre-existing on the keyword spelling · loud or oracle-split, no silent)** | ⓐ **`$bits` of a `real` parameter** `localparam real P = 3; $bits(P)` — vita **32** / verilator **64** / iverilog **1**; §6.12 makes `real` 64 bits, so verilator is the LRM reading and vita's 32 is a silent-wrong on an oracle-SPLIT axis (typed-prefix `typedef real r_t` behaves the same; scoped `p::P` answers 64 — the two `$bits` paths disagree with each other). ⓑ **`$bits` of a string parameter** — vita + iverilog 16 (8 × length, §6.16), verilator 64: vita = LRM, keep. ⓒ **a `real` parameter with a non-integral value in a derived integer constant** `localparam real Q = 1.5; localparam W = Q * 2;` — loud E3009 in vita (`Q` is a real…), both oracles 3.0: pre-existing loud, §3 ⑨'s axis. ⓓ **a 2-state struct's `'{…}` in a constant** — the desugar's `w'(longint'(e))` has no const-fold arm (`localparam logic [5:0] P = {1'(longint'(1'b1)), 5'(longint'(5'd3))}` is E3009 "no constant-fold arm"), so `localparam bit_st_t P = '{…}` is loud; a 4-state struct's `'{…}` folds. ⓔ **a fill inside a `'{…}` in a constant** `'{a: '1, b: '0}` → `{1'('1), 5'('0)}` — the size cast of a fill has no const-fold arm (§2 🆕 J's family), loud. ⓕ **a string or `real` parameter of a package read through `import p::*` / a header import** — E3010 `tb.S` / E3009 `R` (the string and real side maps are not on the import channel; scoped `p::S` works; §4.5.415 census). ⓖ **an assignment pattern as an INSTANCE OVERRIDE value** `m #(.X('{1'b0, 5'd7}))` of a struct-typed header parameter — loud E3009 (the override has no target type at parse time); verilator 21. ⓗ **a scoped member access on a package struct variable/parameter** `p::v.a` — E2002 (the struct desugar keys on the bare first segment). ⓘ ~~an explicit `import r::X` colliding with a module-local declaration `X` silently answers the package `X`~~ ✅ **RESOLVED (§4.5.415)** — loud for every namespace (constant, wide, variable, type) through `local_names` in the explicit arm (§26.3; iverilog rejects it, verilator answers the local); a package importing a package still passes an empty set (ⓙ). ⓙ **the wildcard-shadow skip set** (`gather_local_decl_names`) omits functions, tasks, genvars, instance names, typedef names, array parameters and generate-block contents, and a package importing a package passes an EMPTY set — a wildcard constant named like one of those still binds (review residue, unmeasured cells). ⓚ **an `import` inside a generate BLOCK** — was silently ignored (both oracles apply it: `import q::*; if (1) begin : g import r::P; … P` reads r's), now loud E3009 unless redundant with a module-scope import of the same package/symbol (§4.5.410); an import directly in a bare `generate … endgenerate` REGION is applied at module scope (§27.3). Per-scope application inside a block is the remaining work. ⓜ ~~an unnamed generate block's variable is visible at module scope~~ ✅ **RESOLVED (§4.5.432)** — the block is `genblk<N>` with its own scope segment; a module-scope read of its net is loud (both oracles refuse); `genblkN.V` resolves. ⓛ **`union packed` containing an anonymous `struct packed`** fails to parse; **`import`/`localparam` inside a function body** loud — both pre-existing. ⓝ ~~`$bits` of a BLOCK-local variable that shadows a wildcard-imported package constant answers the constant's width~~ ✅ **RESOLVED (§4.5.430)** — `bits_of_view` applies the value read's shadow test (`bare_ident_route`, over `params ∪ symbols`) before the constant table, and an unpacked local array is element width × count; 17 cells, 10 fixed-silent. Residue (review B F2, pre-existing): the mirror covers two of the five constant maps — a `string` / `real` constant shadowed by a net still answers the net's width for `$bits` while the value read answers the constant. Still loud, pre-existing: two sibling blocks each declaring the same shadowing local (the flatten model coalesces them), a block-local `localparam` shadowing an import, an `automatic` block local. ⓞ ~~a part-select on the MIDDLE dimension of a 3-dim packed variable is the wrong width~~ ✅ **RESOLVED (§4.5.427)** — `packed_inner.rs`: a `[m:l]` / `[b+:w]` / `[b-:w]` on a non-innermost dim under one or more indices (a packed net or an unpacked array of packed) selects whole sub-elements, read and write; 43 cells, 19 fixed-silent, 0 regressions; an out-of-range, reversed or variable-offset inner select is loud (iverilog rejects all three). ⓟ ~~a scoped typedef whose dims name the package's own constants is unusable without importing them~~ ✅ **RESOLVED (§4.5.415)** — the `p::t` twin's `range`/packed dims are re-spelled `p::W` at `endpackage` (same `respell_pkg_dims` as the packed-md parameter dims); a dim naming a constant the package IMPORTED stays bare (pre-existing). ⓠ **(§4.5.412 review, PRE==POST, no oracle — illegal SV) a part-select with a RUNTIME bound on a parameter, and a `+:` of width 0, both answer 0 in silence** — `localparam logic [11:0] F; logic [1:0] i; F[i*4+3:i*4]` → `0000`, `F[i*4 +: 0]` → `0` (verilator refuses a non-constant part-select bound); an elaborate leniency the packed-md rewrite had to route around (it emits `+:` and refuses a reversed range at parse time). ⓡ **(§4.5.412 review B2, PRE==POST, both oracles disagree) a block-local plain variable shadowing a wildcard-imported package VARIABLE is read as the local AFTER its block** — `package p; logic [7:0] SX = 8'hA5; … import p::*; initial begin : b logic [7:0] SX; SX = 8'h11; end … $display(SX)` → `11` (oracles `a5`): the v1 block-local flatten model makes the local a module net of the same name, which then wins over the import at module scope. ⓙ/ⓝ's family (the same collision made a restored struct binding read the local through the package layout, so §4.5.412 restores block-shadowed bindings for packed-md PARAMETERS only). ⓢ **(§4.5.413 census, PRE==POST, scalar control identical, illegal SV) a header parameter redeclared in the body** — `module c #(parameter X = 1) (); localparam int X[2] = '{1,2};` (and `localparam int X = 2;`) is accepted and answers the body declaration (verilator: "Duplicate declaration"; iverilog rejects the scalar control). ⓣ **(§4.5.413 census, PRE==POST, scalar control identical, illegal SV) a duplicate named override** `c #(.A(x), .A(y)) u();` is accepted, last wins (verilator: "Duplicate parameter connection"). ⓤ **(§4.5.414 census, PRE==POST, flat control identical, iverilog oracle — verilator is 2-state) an x/z WRITE into a 2-state member of a 4-state packed struct keeps the x/z** — `typedef struct packed { logic a; bit [3:0] q; bit s; } t; o.q = 4'bx1z0;` → vita `xx1z0x`, iverilog `x0100x` (§7.2.1: a 2-state member of a 4-state struct converts on write); a `'{…}` value for the member IS coerced (the desugar's `longint'` squash), a whole-struct write is not coerced by iverilog either. The member write is a plain part-select on the 4-state net — the parser knows the member is 2-state (`StructFieldLayout.5`) but the write path does not squash. ⓥ ~~`parameter type T = logic [7:0]` is not parsed~~ ✅ **RESOLVED (§4.5.437)** for the integral vector subset (see §3 ⑤). Residue: a value passed positionally to a type parameter (`m #(8)`) and `defparam u.T$w = 12` are accepted (both oracles refuse — the synthesized `T$w`/`T$s` are in the user namespace; the shape flag `T$s` cannot be forced, F4004); `typedef T a_t [N]` is the existing unpacked-alias loud; a class `#(type T)` header is untouched. ⓦ **(§4.5.415 probes, PRE==POST, both oracles) a package FUNCTION in a constant** — `import p::*; #(parameter int X = dbl(W))` is E3009 "`dbl(…)` has no constant-fold arm" (the §4.5.186 const-function interpreter collects the MODULE's functions only); in a RANGE BOUND (`parameter logic [dbl(2)-1:0] X`, review A a55) it is **silently 32 bits** — `check_const_range_bound` keeps a const-but-unfoldable bound silent by design, and the module-local spelling folds (`9 4`, both oracles). ⓧ **(§4.5.415 review, verilator side, iverilog rejects) two ordering leniencies** — a compilation-unit `import` written AFTER the module still applies to it (B b09; the body-localparam and net-range twins were already lenient), and a package constant used before its declaration folds through the re-spelled `p::t` twin (B b25). ⓨ **(§4.5.415 review B b29) the declared-range gate's span dedup reports a generate loop's bad bound once** (iverilog three times) — loud preserved. ⓩ **(§4.5.416 census, scalar control twin, PRE==POST, both oracles) a select of a NON-ZERO-LSB parameter inside an UNTYPED constant concatenation reads raw internal bits** — `localparam logic [11:4] A1 = 8'h3C; localparam L = {A1[7:4], A1[7:4]};` → vita `33`, both oracles `cc`: `const_placement_wide`'s resolver sizes a NAME from `param_meta` without its declared LSB, so the wide `PartSelect` arm indexes from 0 (the typed twin and the element twin decline through `narrow_param_bits` / `const_array_elem_bits`, so they are loud, not wrong). Fix = route the placement resolver's bare-name arm through `narrow_param_bits` (provenance-filtered) and let the LSB-shifted read decline. Also pre-existing, loud: the runtime `$size(P)` of a scalar parameter; a multi-packed ELEMENT array parameter (`logic [1:0][3:0] A[2]`) is not captured at all (every read of `A[1]` loud, variable twin fine). And pre-existing SILENT (review B B1's runtime twin, verilator 1): the RUNTIME read `ROTA[1]` inside `generate if (1) begin : g localparam int ROTA = 99;` with an outer `localparam int ROTA[0:3]` prints the outer array's element (20) — the runtime lowering resolves the bare name to the outer NET before the inner generate-scope parameter, while every constant consumer now takes the inner scalar (§4.5.416 closed GAP-G's root). **(aa) (§4.5.418 census control twin + review B F1, decimal spelling PRE == POST, both oracles) the parse-time constant table folds at i64, not at the expression's width** — `localparam logic [3:0] C = 15, D = 1; logic [C+D:0]` was 17 bits (oracles 1) and `int unsigned W = 4294967295; [$clog2(W+2):0]` 35 (oracles 2). CLOSED for the packed-struct MEMBER-WIDTH consumer (`member_width` / `member_ascending` / `member_dbase` fold through the width-aware twin `try_const_index_w`: literals at their own width, named constants at their declared width and sign, `+ - * / %` at the larger operand width, unary minus, `$clog2`, parens); STILL i64 for the table's other readers — the constant generate-array index, the enum-label fold, the size-cast bound (`try_const_index`) — and the based-literal spelling of a narrower-than-32-bit parameter is declined so it does not enter those readers. Also recorded by the §4.5.418 review, pre-existing: the `endpackage` export of `packed_md_params` (`module_items.rs`) has no `local_decl_names` filter (the constant export has one — an imported multi-dim parameter is re-exported); `foreach` over a multi-dim packed formal is loud with a misleading enum-method diagnostic. **(aa) ✅ RESOLVED (§4.5.423, 2026-09-05)** for every reader of the parse-time table (generate index, cast bound, `'{default:}` shape, the range bounds a recorded value is shaped by, a TYPED recorded value at its declared width) and for every declared-range bound fold in elaborate (`const_range_bound_fold` = width-aware self-determined walk, i64 fallback; 56 sites). **Residue (2-oracle census 324 cells):** an UNTYPED `localparam G = C + D` stays i64 (iverilog 16 / verilator 0 — an oracle split); enum-label values are the same split (iverilog folds a label self-determined, verilator in the base type's context; vita = verilator); the elaborate VALUE lane (`localparam logic [7:0] M = (P + 8'd100) % 8'd7` = 6 for the oracles' 2; `int S = (D - C) / 2` = −7 for 2147483641) is row 14's wall — §4.5.423's review built the routing again, measured row 14's regression (`time NM` generate shadow → 2c) and reverted it; `$clog2(C+D)` in an untyped declaration folds the 32-bit sum (both oracles the 4-bit 0); a `byte` operand in a bound (`[Y+Y:0]`, `Y = 100`) now reads the LRM/iverilog 57 bits where verilator and PRE read 201. |
...
> | 33 | ✅ **RESOLVED (§4.5.408)** — a procedural read of a whole-net copy after the reader's OWN blocking write of the source now reads the source (`levelize::proc_read_alias` → `WidthTable::read_alias`, consumed by all three backends). Census 91 cells: **59 silent→correct · 0 regressed** · examples' VCD byte-identical · corpus 10/10. ⚠️ The STORE-side forward the row proposed was built and measured first and is REFUSED: it moved picorv32's oracle digest, made a UDP chain sample its fresh input in the same delta (iverilog: the old one) and split native from the VM on keccak — the settle's consumers are order-sensitive, a process's read after its own write is not. Residue → 🆕 I. |
> | 34 | ⛔ **DO NOT START — §2 rows 23/24 (clocking), ruled out by measurement 2026-09-02** | 36 silent-wrong cells and startable, and still excluded: **1-oracle only** (iverilog 13 cannot parse `clocking` at all), and demand is zero **twice over** — no corpus row uses clocking, and all 16 `endclocking` files in the tree are inside interfaces, which vita refuses through an unrelated gate, so closing 24a unlocks no real design even in principle. ⚠️ Worse than recorded, for whoever does start it later: the signal is destroyed PERMANENTLY (not the recorded `x,171,171,…`), and with a 2-state declaration it clobbers to `0,0,0,…` — a plausible-looking value with no `x` anywhere. ⭐ Row 23's naming half IS a one-token fix (`sva_clocking.rs:743` computes `alias = self.fq(...)`, `:750` re-formats it without `fq`) and can ride any nearby slice. |
>
...
>
> - **module-scope `localparam` 의 `/`·`%`·`>>>` 는 여전히 부호를 잃는다** ⚠️ **선언 폭이 있어도 그렇다** ⚠️ **2026-09-01 재측정: `>>` 는 이미 정확하다**(옛 문구가 넷을 셌는데 셋이다 — `64'hFFFF…FFFF >> 4` 는 PRE 도 iverilog 와 일치). 나머지 셋 실측: `% 64'd10` **18446744073709551615**(ivl 5) · `/ 64'd10` **0**(ivl 1844674407370955161) · `>>> 4` **18446744073709551615**(ivl 1152921504606846975). ⭐⭐ **§2 표 14 의 수정이 이 셋을 전부 닫는다** — 지어서 재고 **되돌렸다**(표 14 참조: 선행조건 둘이 공유 walk 안에 있다). ⇒ 이 줄은 표 14 와 **한 항목**이고 따로 착수하지 마라. 진단은 정확했다(*"순서비교만 폭 인식 walk 로 redirect 되고 `/`·`%`·시프트는 `ctx_w`/`ctx_signed` 를 읽는데 거기가 (64, unsigned) 가 아니다"*) — 답이 그 arm 이 아니라 **초기화식 전체를 문맥이 있는 walk 로 보내는 것**이었을 뿐이다.
...
- **🔴 A size cast over a FUNCTION-CALL leaf still evaluates at self width** (pre-existing · PRE == POST · 2-oracle · §4.5.405 measured and left it): `ast_ctx_signed` answers `None` for a call, so `64'(f(1) - 40)` zero-fills the top half — `00000000ffffffd0` for the oracles' `ffffffffffffffd0` — 16 of 720 cells, every one of them the call leaf. Resolving it means giving `expr_self_signed`'s `_ => false` (21 callers) the declared return type; that is its own slice, not one more arm in the classifier. Everything else this bullet used to name — parameter / package / enum-label / >64-bit leaves, and the *"AST self-width pass first"* prerequisite — is resolved: the width is `size_ctx_self_width` + `max(N, W)` in `lower_size_ctx_entry`, and a walk that declines recurses at N (every decline is an unsigned opaque leaf, whose expression's low N bits do not depend on the evaluation width) instead of probing. Residue of the same slice: an element of a DYNAMIC / queue / associative array has a sign the classifier cannot see (declines); an operand that reads a HIERARCHICAL or class-member name (`u.v`, `u.arr[0]`, `c.fld`) keeps the pre-slice classifier entirely — the read is a placeholder with no width here, and each cast over one is right on one path and wrong on the other (both measured); a `time` constant derived from a guessed parameter declines (no `time` meta arm).
- **🟡 A FILL override (`'1`/`'0`, `#()` or `-G`) binds at the default's width; both oracles bind ONE bit** (§4.5.405 round-2 measurement · 2-oracle · pre-existing · PRE == POST): `#(.P('1))` onto `parameter P = 5` is `-1 bits=32` in vita and `1 bits=1` in iverilog and verilator (§6.20.2: the value `'1` is a 1-bit unsigned fill). The fill channel re-folds at the target's `pw`, which for an untyped parameter is the default's; the override carries no width by construction. Fix = a fill onto an UNTYPED parameter binds `(1, false)`.
- **🟡 A `time` parameter with a DECIMAL default forwards as 32-bit unsigned** (§4.5.405 round-2 · pre-existing): `parameter time T = 1 << 40` has no `param_meta` (the literal arm answers only for `Implicit`/sized), so a child `#(.P(T))` types the override `(32, unsigned)` via `const_self_width`'s `map_or(32)` and truncates 2^40 to 0; both oracles bind 64 bits. Fix = a typed (`time`/`integer`/`int`) declaration records its type as meta even for a non-literal default, and `const_self_width` declines instead of guessing 32.
- **🟡 The verbatim inline actual's mirror is wrong on its own path too** (§4.5.405 round-2 · pre-existing · PRE == POST): an 8-bit signed frame-call actual into a 16-bit signed formal (`fs16_add(g(-16))` → `00f0`, oracles `fff0`) — `bind_formal_actual` widens by the actual's MIRROR sign (`Call ⇒ false`) when it cannot trust the width. Same root as the call leaf above (`expr_self_signed`'s `_ => false`).
...
> ⚠️⚠️ **이 블록의 역할은 2026-09-02 로 끝났다.** 여기 적힌 다섯은 전부 해소됐고
> (`①` §4.5.370 · `③` §4.5.374 · `④` §4.5.376 · `⑥` §4.5.378 · `⑨` §4.5.377 · `⑧` §4.5.404),
> **코퍼스 10/10 에 거절이 0** 이다 ⇒ **워크로드 코퍼스는 더 이상 §3 착수 순서를 정하지 않는다**.
> 그것이 이 절의 유일한 외부 동인이었으므로, §3 의 남은 줄은 이제 **§2 정확성 큐 뒤**에 선다.
> 코퍼스에서 값을 더 뽑으려면 **행을 늘려야 한다**(REMAINING_WORK 2a · ibex 가 첫 SV 워크로드).
>
...
> **⑤ ibex — the typed-parameter prefix** — ✅ **RESOLVED (§4.5.410)** for the rule the row named,
> ⚠️ **and the row's claim that it was "the one parse rule that opens the workload" was false**: measured at
> HEAD after the rule, ibex still has **four** blockers behind it (below). `parse_param_prefix` knew
> only keyword prefixes, so every `localparam exc_cause_t E = …` / `parameter lfsr_seed_t S = …` was an
> E2002 at the NAME — all 50 of ibex's first error page, not the assignment pattern the old text blamed
> (a `'{…}` on a struct VARIABLE already desugared). The prefix now resolves a typedef to the same
> `(kind, sign, range)` the equivalent variable gets, binds a struct/enum-typed parameter name like that
> variable (member part-select, `'{…}` positional/named, enum methods), and the bindings cross
> `import pkg::*` / `import pkg::X` (`pkg_bindings`, captured at `endpackage`) — which also opened the
> pre-existing gap where `v.b` on an imported package struct VARIABLE was E2002. Census 227 cells
> (16 typedef kinds × {body localparam, body parameter, package + wildcard import, package scoped,
> header default + override, generate, comma list} × {value, `$bits`, member, slice, derived constant,
> enum method}): **182 loud→correct · 0 regressed · 0 silent**; iverilog cannot parse a struct/enum-typed
> parameter (and aborts on its member access), so verilator 5.050 is the oracle there and iverilog
...
>
> **The ibex ladder after this rule (HEAD 3-tool, `bench/ibex/vitarun.sh`)** — after §4.5.412
> `ibex_pkg.sv` parses CLEAN (0 diagnostics); the first error page is now `ibex_cheriot_pkg.sv`
> (50 = the cap: 11 "struct member width must be a named integer type or a constant-literal range"
> = a member `logic [ADDR_W:0]` with a PARAMETER range, and the nested struct members of ⓓ).
> ⓐ ~~`ibex_pkg.sv:742` a multi-dimensional packed typedef on a parameter~~ ✅ **RESOLVED
> (§4.5.412)** — and the row's "typedef" framing was half the class: the KEYWORD spelling
> `parameter logic [Dw-1:0][Iw-1:0] StatePerm` (prim_lfsr.sv:53, the consumer of every ibex use)
> was loud too. The parser declares the parameter FLAT (`[total-1:0]`, dims as expressions) and
> rewrites every select chain on the name (`P[i]`, `P[i][j]`, `P[i][a:b]`, `P[i][o+:w]`, `P[a:b]`)
> to the flat bit/part-select (`packed_md.rs`, the packed-struct member precedent) — body, header
> (override channel intact, dims may name the instance's own overridden parameters), package
> (dims re-spelled `p::W` at `endpackage` for scoped/explicit-import reads). Census 195 cells
> (8 positions × 10 reads + runtime/const/dims/header/package/scope cells, a keyword control and a
> variable twin per position): 141 loud→correct · 24 unchanged · 0 silent · 0 regression; verilator
> sole oracle (iverilog: "packed array parameters are not supported yet"). Residue: `$size`/`$left`/
> `$dimensions` on it (loud, as on any parameter), `'{…}` as its value (loud), an ARRAY parameter of
> such a type (loud), `import p::*; import q::*` both exporting `P` (loud; verilator answers p's).
> ⓕ ~~a header `import` is not visible to the header's own parameter defaults~~ ✅ **RESOLVED
> (§4.5.415)** — elaborate applies a compilation-unit import and an ANSI HEADER import (`module m
> import p::*; #(parameter T X = Dflt)`, recognised by a span before the first header parameter)
> BEFORE `bind_params` and a body import after (a header default naming a body import's constant is
> an oracle split — iverilog rejects, verilator folds — and stays loud); the interface twin applied
> no imports at all before. Census 196 cells (26 header shapes × 4 import positions × a `p::`-spelled
> control twin): 80 loud→correct · 89 unchanged · 0 silent · 4 pre-existing silent-wrongs fixed
> (a DECLARED parameter range that does not fold went value-inferred — `$bits` 32 at exit 0, both
> oracles refuse — now loud through `check_const_range_bound` from all three binders) · an explicit
> import colliding with a local declaration is loud (§26.3, was 🆕 L ⓘ) · the scoped typedef twin's
> dims are re-spelled `p::W` at `endpackage` (was 🆕 L ⓟ). Residue (loud, pre-existing, 2-oracle):
> `parameter type T = …` is not parsed (🆕 L ⓥ); a package FUNCTION in a constant (`parameter int X
> = dbl(W)`) has no fold arm, and in a range bound is silently 32 (🆕 L ⓦ); a `real` package
> parameter is not on the import channel (🆕 L ⓕ's family); ordering leniencies 🆕 L ⓧ; dedup ⓨ.
> Review-round fixes: the body-parameter binder skipped the range gate; a package's own declaration
> did not shadow a wildcard import INSIDE the package (both oracles); a typedef dim naming an
> imported constant bound in the importer (both oracles); a CU explicit import is shadowed by a
> local declaration, not refused. ibex: every module header now binds; the whole design is still the preprocessor
> rung (24 E1013), and behind it ibex_alu's generate-block unpacked-array write / keyed pattern.
...
> ⓒ ✅ **RESOLVED (§4.5.411 package half · §4.5.413 header half).** The PACKAGE half —
> `ibex_pkg.sv:791` — closed by IEEE §6.20.1 (`in_package`). The HEADER half — `module ibex_top
> #(parameter ibex_pkg::pmp_cfg_t PMPRstCfg[PMP_MAX_REGIONS] = ibex_pkg::PmpCfgRst, parameter
> logic [PMP_ADDR_MSB:0] PMPRstAddr[PMP_MAX_REGIONS] = ibex_pkg::PmpAddrRst, …)` — is the A2a
> desugar placed at the front of the body plus a scalar `ParamDecl` TWIN (same name, same span)
> in `module.params` that holds the override slot; elaborate recognises the pair
> (`array_param_twin`), folds an override as a whole array in the parent
> (`const_array_override_vals`: a `'{…}` of constants, `pkg::Arr`, the parent's own array
> forwarded, a pattern of the parent's elements) and feeds both init consumers (const capture +
> decl-init flush) through `array_param_vals_src`, which also resolves a whole-array DEFAULT
> (`= pkg::Rst`, an imported bare name, a sibling header array — the body `localparam L[2] =
> p::R` too, loud before). Census 183 cells: 109 loud→correct · 25 unchanged · 0 silent · 0
> regression; the ibex_top→ibex_core chain with the real pmp types matches verilator. **Residue
> (all loud, verilator-value):** a NESTED (2-D) override pattern; an override / whole-array
> default of elements wider than 64 bits (the literal-pattern default works); `defparam` onto an
> array parameter; an interface-header array parameter; a `'{default: v}` keyed pattern as an
> override; `$size(A)` in a constant and `A[i][MSB-:W]` as a child override (= ⓔ, the
> ibex_cs_registers:1450/1485 consumers, pre-existing on the body-localparam control); the
> element width naming a BODY `localparam` declared after the header (review B — header
> parameters bind before the body, the scalar spelling too).
> ⓔ ✅ **RESOLVED (§4.5.416).** A select / struct member / bit of an element (`A[1][4:0]`, `S[1].b`,
> `A[0][7]`, `A[1][7-:4]`) folds in every constant position (localparam typed or untyped, range
> bound, generate-if, child override — ibex_cs_registers:1486 `PMPRstAddr[i][PMP_ADDR_MSB-:PMPAddrWidth]`
> — replication count, ternary, concatenation, `$bits`), for a body / generate / header (overridden)
> / package (`p::A[1]`, `import p::*`) array, with the element's DECLARED range (direction, non-zero
> LSB, sign — the scalar parameter's select rules through `ArrayConstMeta`, captured beside
> `array_const_vals`); the `$size` family folds over an array parameter and over a parameter with a
> declared range, and the parser answers it for a multi-packed parameter from the dims it flattened;
> `pkg::X` and an element fold inside an UNTYPED constant concatenation; an untyped parameter takes
> its width from a select initializer (`localparam L = W[3:0]` 4 bits, `localparam L = A[1]` the
> element's type — was silently 32, the scalar spelling too); a vector typedef whose range names a
> package constant casts (`cbound_t'(…)`, ibex_cheriot_pkg — the next rung of the same page). Census
> 504: 258 loud→correct · 31 fixed-silent · 0 new-silent. ibex_cs_registers with `PMPEnable=1`:
> exit 0, the four landed `ResetValue`s equal verilator's. **Residue (all loud, verilator-value):** a
> MULTI-PACKED element (`logic [1:0][3:0] A[2]` — `A[1][0]`, a whole-element read, `$size(A, 2)`; the
> element capture itself declines it, PRE too); an ascending / non-zero-LSB element inside a
> concatenation or a replication count (the wide domain declines them as it declines the scalar
> twin); a header struct-typed array under an override pattern (§4.5.413); `p::S[1].b` (🆕 L ⓗ); the
> runtime `$size` of a scalar parameter (`ctl_*_rt`); a select reaching outside the element (`x`,
> declined); an UNTYPED child parameter overridden by a select of an element (review A F1 — loud; the
> scalar `.P(W[3:0])` / whole-element `.P(A[0])` spellings are row 25's pre-existing 32). **Pre-existing
> found by the scalar control twin** = §2 🆕 L ⓩ.
> ⓓ ✅ **RESOLVED (§4.5.414).** `ibex_cheriot_pkg.sv:84-131` was TWO rungs, not one: a member whose
> type is another packed struct/union typedef (`perms_t perms; cap_t cap; decoded_cap_t cap;`) and a
> member whose width names a CONSTANT (`logic [ADDR_W:0] top33`, `cperms_t` = `logic [CPERMS_W-1:0]`,
> `logic [EXP_W:0] explen`). The parser lays a nested member out flat at the nested type's total
> width and keeps its type key on the field (re-spelled `pkg::t` at `endpackage`, `stable_type_key`
> for an import copy), so any chain `s.a.b.c` / `arr[i].a.b` resolves to the leaf's geometry at the
> summed offset — read, write, sub-select, `$bits`, ports, function locals, a `'{…}` that recurses
> per member, `s.perms = '{…}`, a `default:` FILL; the member width folds through the same
> `const_locals` table the constant generate index uses, widened by IEEE §6.20.1 (a package
> `parameter`, a body `parameter` behind an ANSI header are localparams), exported/imported with the
> package (explicit wins, a local declaration shadows, two wildcards with different values drop it),
> `pkg::W`, `/`, `$clog2`, and a declared-type fit. Also on cheriot page 1: a fill inside a struct
> `'{…}` is emitted as the sized literal (`parameter cap_t NULL_CAP = '{default: '0}` was loud, 🆕 L
> ⓔ), and a struct-typed cast `cap_t'(e)` (size + signing; the signing cast over a size cast folds in
> both constant domains — `ROOT_CAP_TX = cap_t'(ROOT_DECODED_CAP_TX)`, 112 → 35 bits). Census 102
> cells (leaf types vector/byte/enum/ascending/non-zero-LSB × read/sub/write, 3-level chains,
> union-in-struct and struct-in-union, 2-state nesting, patterns positional/keyed/default/decl-init/
> localparam/package/scoped, array element, port, function, generate, always_comb, compare/concat,
> casts, scoped/explicit/cross-package types, same-name collision, block-local typedef; and for the
> width table every source kind, shadow, import and value-fit cell, with a flat / literal control
> twin per shape): 65 loud→correct · 12 unchanged · 19 still-loud (below) · 0 regression · 0
> silent; two oracle splits resolved on the LRM side (iverilog reads a nested member part-select as
> the whole member; verilator lets a wildcard win over an explicit import, §26.8). The widened table
> also reaches its two PRE-EXISTING readers: a `$clog2`/`/`-valued localparam as a constant
> generate index and as an enum label go loud→correct, and a generate-local `localparam` shadowing
> a module one — a PRE silent-wrong (the outer value folded) — is now scoped (review A F1). ibex:
> `vitarun.sh` (both packages + tb) 50 errors → **0**; the whole-design run is unchanged (24
> preprocessor E1013: `define default argument values in dv_fcov_macros, prim_assert's `ifdef
> structure — the next ladder rung for the module pages, together with ⓕ and a member width naming a
> HEADER parameter: `ibex_lockstep.sv:243 logic [MemDataWidth-1:0]` inside a body typedef, which
> both oracles lay out per instance, so the parser cannot fold it — loud).
> **Residue (all loud, verilator-value unless noted):** `default: v` for a nested slot with a
> non-fill non-zero `v` (whole-vs-per-leaf application unpinned — verilator whole, iverilog rejects);
> an enum method through a chain `o.i.e.name()` (iverilog: "method name nesting"); a packed ARRAY of a
> struct typedef as a member `in_t [1:0] i` (2-oracle); a packed struct inside an UNPACKED record; a
> hierarchical read through a member `u.c.perms.q` (2-oracle, flat `u.c.q` loud too — pre-existing);
> `$bits(pkg::T)` (2-oracle 7, flat too — pre-existing); an indexed write `o.i[1+:2] = …` (the
> existing rule); a member width from `1 << 3` / `8'd5` / a `byte`-typed or vector-typed localparam
> that does not hold the value / a forward-referenced localparam (iverilog rejects) / a header or
> no-header body `parameter` (overridable — correct-loud). Pre-existing SILENT found by the flat
> control = §2 🆕 L ⓤ.
> Behind those: `ibex_if_stage.sv:218 ExcCauseIrqNm.lower_cause` now parses (import carry). verilator
> lints all of ibex (`vlint.sh`, 8/8 probes); iverilog is an oracle for the nested shapes without a
> `'{…}` pattern, verilator for the rest.
> ~~the preprocessor rung~~ ✅ **RESOLVED (§4.5.417)** — `define default argument values (§22.5.1:
> omitted and empty actuals take the default, bound at use time), a macro body that keeps one
> directive per continued line (`ifdef … `else … `endif inside a body, evaluated at expansion time)
> with `// comment \` lines continuing the body, and `__FILE__ / `__LINE__ (§22.13, the use's file
> and the line where its argument list closes). ibex whole design: 24 E1013 → 0 preprocessor errors;
> the page is now the PARSER — `prim_cipher_pkg.sv` tf-port formals of a multi-packed type
> (`logic [15:0][3:0] shifts`) and `prim_ram_1p_pkg.sv`. **Residue (loud, both oracles or split):** a
> formal without a name (`M(a, = 3)`, verilator lenient) and a duplicate formal (oracle split) stay
> loud; a default naming another formal (`b = a`) and a default calling a macro with a formal as its
> argument are rejected by both oracles. Pre-existing (PRE == POST, review): a `//` comment inside a
> formal list without a continuation ends the list (both oracles run it past the newline); the
> macro-redefinition warning points at the FIRST definition and a redefinition differing only in a
> default's whitespace is silent (verilator warns); a continued `timescale / `include /
> `default_nettype line is accepted where both oracles reject.
> ~~the parser rung~~ ✅ **RESOLVED (§4.5.418)** — a multi-dimensional packed tf-port formal
> (`logic [15:0][3:0] shifts`, inline or typedef, ANSI / non-ANSI, task / function, input / output /
> inout / ref, default, continuation) is declared flat and its body selects, element WRITES and
> `$size`-family queries are rewritten from the dims (the §3 ⑤ ⓐ parameter machinery, tf-scoped);
> a lone based literal (`parameter int unsigned W = 32'd12`) is a parse-time constant (declared
> width ≥ 32), and a packed-struct member width folds at the expression's IEEE §11.6 width
> (`try_const_index_w`: `int unsigned W = 32'hffff_ffff; [$clog2(W+2):0]` and `logic [3:0] C, D;
> [C+D:0]` wrap as in both oracles — the pre-existing i64 silent of §4.5.414's table, closed at this
> consumer). ibex whole design: `prim_cipher_pkg.sv` / `prim_ram_1p_pkg.sv` 50 → 0; the page is now
> the **SVA parser** — a parenthesised property with an implication inside `assert property
> (@(posedge clk) disable iff (r) (a |-> b))` (the `ASSERT` macro body: ibex_branch_predict,
> ibex_compressed_decoder, ibex_controller, ibex_core — 17 uses, 50 E2002). **Residue (loud):** a
> multi-dim packed formal that is also an unpacked array (`logic [1:0][3:0] a [2]`, both oracles run
> it; the array-parameter twin is not captured either); a narrower-than-32-bit parameter with a
> based-literal value stays out of the parse-time table (its decimal twin is in, and the table's
> OTHER readers — generate index, enum label, cast bound — still fold at i64: §2 🆕 L (aa)).
> ~~the SVA parenthesised-property rung~~ ✅ **RESOLVED (§4.5.419)** — a property wrapped whole in
> parentheses (`assert property (@(posedge c) disable iff (r) (a |-> b))`, the `ASSERT` macro body;
> nested `((…))`, `|=>`, `##1`, `inside`, an `else begin … end` action block, a named property's
> `;` form, `assume`) parses and lowers byte-identically to the unwrapped body
> (`paren_group_is_whole_property`: a group containing a property operator at any depth that runs to
> the property's own terminator). ibex whole design: branch_predict / compressed_decoder /
> controller 50 → 0; the page is now **`ASSERT_INIT`** — a LABELLED immediate assertion inside
> `initial` (`__name: assert (__prop) else begin … end`; ibex_core 7 · ibex_cs_registers 17 ·
> ibex_icache 6, E2002 "expected '=' or '<=' after lvalue, found ':'"), then `export "DPI-C"
> function …` (ibex_if_stage 12, DPI — expected to stay loud) and the lockstep header-width struct
> members (ibex_lockstep 8, §3 ⑤ ⓒ). **Residue:** `cover property (… (a |-> b))` (own grammar,
> §2 🆕 M ⓔ); ranged / property-`and`-with-`disable iff` consequents are loud in elaborate for both
> spellings (pre-existing subset).
> Order after §4.5.419: the `ASSERT_INIT` labelled-immediate-assert rung → header-parameter member
> widths (lockstep).
>
> **§4.5.422 — the `ASSERT_INIT` rung (2026-09-05)**: a statement label `L: stmt` (IEEE §9.3.5) parses
> as the named block `begin : L stmt end` (`L: begin … end` takes the label; a block carrying both a
> statement and a block label is E2002). ibex_core / ibex_cs_registers / ibex_icache 30 → 0. The page
> is now **ibex_top_tracing (20)** — a scoped type with a packed dimension naming a scoped constant in
> a PORT (`prim_ram_1p_pkg::t [ibex_pkg::IC_NUM_WAYS-1:0] p`, E2002 at the `::` inside the dim),
> then **ibex_pmp (9)** — `csr_pmp_cfg_i[r].mode` inside a generate loop is parsed as a
> generate-array hierarchical reference with a non-constant index (the genvar `r`), the same
> misparse as ibex_core 2481 (`g_pmp.pmp_i.x[PMP_D][i_region]`); then `export "DPI-C" function`
> (ibex_if_stage 12) and lockstep (8, §3 ⑤ ⓒ). Residue → §2 🆕 N (`%m` inside a named block).
>
> **§4.5.425 — the ibex_top_tracing / ibex_pmp rungs (2026-09-06)**: a typedef name followed by
> packed dims (`pkg::t [pkg::C-1:0] p` in an ANSI port or a declaration — the outer dims of a packed
> array whose element is the typedef; `v[i].field` desugars on the packed element) and a struct-typed
> port ARRAY whose element member is read with a genvar / runtime index (`c[r].mode`, was a
> generate-array hierarchical reference). ibex 50 → 20: top_tracing 20 → 1, pmp 9 → 0. The page is
> now **`$fatal` as a module item** (ibex_top_tracing:140, an elaboration system task, IEEE §20.11, 1),
> `export "DPI-C" function` (ibex_if_stage 12), lockstep (6, §3 ⑤ ⓒ) and ibex_core:2481
> (`g_pmp.pmp_i.x[PMP_D][i_region]` — a hierarchical reference whose LAST segment is a net indexed
> by a genvar, 1). Non-ANSI `<type> [dims]` ports, an atom typedef with dims, and a SIGNED typedef element with dims (review A F1 — the element's sign has no flat representation) stay loud.
>
> **§4.5.428 — elaboration system tasks (2026-09-06)**: `$info` / `$warning` / `$error` / `$fatal`
> as a module item (also inside a generate branch, IEEE §20.11) run at elaboration with constant
> arguments; `$error`/`$fatal` fail elaboration (no simulation). ibex_top_tracing's `$fatal` under
> `ifndef RVFI` parses now (it fires once the parse page is clear — the design requires
> `+define+RVFI`). Page: 50 → 19 = `export "DPI-C" function` (ibex_if_stage 12, loud by design),
> lockstep (6, §3 ⑤ ⓒ), ibex_core:2481 (a hierarchical reference with a genvar-indexed generate
> array in a MIDDLE segment — `cs_registers_i.g_pmp_csrs[i_region].x` — the parser folds only a
> constant index into the segment name; a genvar is constant per iteration but only elaborate knows
> it: needs an index expression on a path segment, an AST shape, so DEEP).
>
> **§4.5.431 — a member width naming an overridable parameter (2026-09-06)**: `typedef struct
> packed { logic [W-1:0] d; … }` with `W` a header `parameter` (or a header-less body `parameter`,
> or a localparam derived from one) is laid out PER INSTANCE — the parser keeps the layout as
> expressions (`sym_struct_layouts`, disjoint from `struct_layouts`) and elaborate folds them; a
> member sub-select, `$bits(T)`, a `'{…}` pattern, a union, a nested chain and a `localparam` the
> table declined stay loud; so do (review A/B, both oracles value them) a member of such a struct on
> a subroutine FORMAL, on a tf-port, and through a cross-instance path `u.s.d`, and the `$bits(T)` /
> sub-select refusals sit behind a misleading first diagnostic. ibex: 19 → **13** — lockstep 6 → 0;
> the page is `export "DPI-C"` (12, loud by design) and ibex_core:2481 (DEEP).
>
> **§4.5.437 — type parameters (2026-09-06)**: `parameter type T = <type>` / `type T` / `localparam
> type` on a module or interface header, in a body and at the unit scope (IEEE §6.20.3), desugared in
> the parser with no AST change — two value parameters `T$w` (width) + `T$s` (shape: signed / 2-state)
> and a symbolic-width typedef `T` = `logic|bit [T$w-1:0]`; `$bits(T)` = `T$w`, `T'(e)` = the size cast
> at `T$w` with `T`'s sign; an override by a type (`.T(logic [15:0])`, positional, a typedef, `pkg::t`,
> `.T(T)`) is the two value overrides; a shape-changing override is refused by a synthesized `initial
> if (T$s != …) $fatal` (a process, not a generate construct, so `genblk<N>` numbering is untouched).
> 34 cells, 26 loud→correct, 0 silent (2-oracle; a `.T(T)` pass-through and `type T` are verilator's —
> iverilog refuses the spelling). Loud by design: a struct / enum / union / real / string / class or
> multi-dimensional default or override. Residue: see §2 🆕 L ⓥ.
>
> **§4.5.434 — compilation-unit-scope declarations (2026-09-06)**: a `typedef` (vector / atom /
> signed / chained / packed struct, nested / packed union / enum), a `localparam` or `parameter`
> (a list too; a unit `parameter` is a localparam, §6.20.1), a `function` or a `task` written BEFORE
> the first `module` (IEEE §3.12.1) was refused at the file's first token (`expected 'module', found
> keyword 'typedef'`) where both oracles run it — the parser's top level knew only design units,
> `import`, `class`, `primitive` and `bind`. Now the parser registers the type / constant under the
> unit scope as it parses (a later module resolves the bare name; the snapshot taken before each unit
> already holds it) and clones the item to the front of every LATER module's body (interfaces:
> constants only — their elaboration refuses typedef/function/task items) unless the module declares
> the same name itself (header parameter, ANSI / non-ANSI port, body typedef / parameter / function /
> task / net / genvar / instance name, an explicit import's name, a wildcard import's package exports —
> review A/B: four BLOCKINGs of this one root, fixed; a unit enum is withheld from a module that
> declares one of its LABELS; a wildcard import replaces a unit type name in the parser scope,
> §26.3), so elaborate binds a unit declaration exactly as a module-
> local one. 42 cells: **31 loud→correct · 0 silent · 0 regressions**; a typedef AFTER its first use
> is refused (both oracles refuse). Pre-existing, control-twinned (not this rung): an enum-typed
> output port driven by `assign` (E3018, the module-level twin too), an unpacked-array typedef
> (`typedef logic [7:0] a_t [0:3]`, refused inside a module too), ~~a module-local typedef whose range
> names a header parameter (`typedef logic [W-1:0] t;` → E3010 `top.t`)~~ (✅ §4.5.437: `$bits(t)` and `t'(e)` fold
> per instance through `sym_range_width`),
> `$unit::t`, a net named like a unit typedef (accepted; iverilog refuses, verilator accepts), a
> struct-typed header parameter under a `'{…}` override (🆕 L ⓖ). A unit `typedef enum`'s labels
> reach a package only through the parser scope (a package constant typed by the enum folds; its
> LABEL inside a package constant is loud). Not done: a unit-scope VARIABLE / net (one storage shared
> by every module — loud), ~~`parameter type T`~~ (✅ §4.5.437), a class body reading a unit
> enum label (loud), a forward reference between unit constants (oracle split; vita loud with
> iverilog). Recorded: an enum withheld for a label collision keeps its TYPE, so `c.name()` on such
> a `c` prints empty (iverilog refuses the uncast assignment that feeds it).
>
...
| 게이트 | **6,452 tests green** · 워크로드 코퍼스 **10/10 · 거절 0** · no-oracle 축 green · clippy 0 · fmt 0 · format_version **29** · MsgCode **68** (2026-09-02 · §4.5.405) |

### 다음 후보 — 우선순위 순

| 순위 | 트랙 | 왜 여기 | 착수 조건 / 첫 걸음 |
|---|---|---|---|
| **1** | **정확성 큐 — §2 silent-wrong 잔여** | 이 저장소의 **최상위 원칙**이 정확성이고 성능 축은 수확 체감에 도달했다 | ✅ **§4.5.437~439 (2026-09-06, 일곱째 3-슬라이스 묶음)**: `parameter type T`(§3 ⑤ / 🆕 L ⓥ · 파서 desugar `T$w`/`T$s` · 34칸 26 loud→correct) · frame 본문·actual 의 copy read-through(🆕 I ⓓ/ⓖ · 26칸 · fixed-silent 10) · 클래스 메서드 `%m` + 서브루틴 안 진단 문맥 = 선언 스코프(🆕 N 잔여 · 28칸 · fixed-silent 16). 잔여 = 혼합 호출자 callee · generate/frame 에서 부른 클래스 메서드의 스코프 · `m #(8)`. ✅ **§4.5.434~436 (2026-09-06, 여섯째 3-슬라이스 묶음)**: 컴파일 단위 스코프 선언(§3 · typedef/localparam/function/task before the first module · 42칸 31 loud→correct) · `%m` 은 서브루틴의 선언 스코프(🆕 N F2 동적 체인 CLASS · 43칸 · fixed-silent 19) · copy alias 의 상수 워드 인덱스 fold(🆕 I 음수 base · 32칸 · fixed-silent 15). 잔여 = class 메서드 `%m` · 2-D 워드(오라클 분열) · `$unit::` · `parameter type`. ✅ **§4.5.431~433 (2026-09-06, 다섯째 3-슬라이스 묶음)**: 헤더 파라미터 폭의 struct 멤버(§3 ⑤ ⓒ · lockstep 6→0 · ibex 19→13 · 28칸 16 loud→correct) · 🆕 N 잔여 ⓑ 무명 `genblk<N>` + else 라벨(22칸 · fixed-silent 17) · 🆕 I ⓑ/ⓒ 상수 워드 copy + all-z 드라이버(34칸 · fixed-silent 11). 잔여 = VCD 스코프 철자 · 프레임 동적 체인 · 런타임 인덱스 copy · DPI export. ✅ **§4.5.428~430 (2026-09-06, 넷째 3-슬라이스 묶음)**: 모듈 항목 `$info/$warning/$error/$fatal`(§20.11) · `%m` 의 싱글턴 generate 스코프 `g`(🆕 N 잔여 ⓐ · 21칸) · `$bits` 블록 로컬 그림자(🆕 L ⓝ · 17칸). 잔여 = VCD 스코프 이름 · 무명 generate 블록 · 프레임 동적 체인 · string/real 상수 그림자. ✅ **§4.5.425~427 (2026-09-06, 셋째 3-슬라이스 묶음 · format 30)**: typedef 뒤 packed dims + struct 포트 배열 원소 멤버(ibex 50→20) · 🆕 N `%m` 블록 체인(32칸 · fixed-silent 17) · 🆕 L ⓞ 안쪽 차원 part-select(43칸 · fixed-silent 19). 잔여 = `generate if` 스코프의 `g[0]` 이름 · fork 라벨. ✅ **§4.5.422~424 (2026-09-05, 둘째 3-슬라이스 묶음)**: 문장 라벨 `L: stmt`(`ASSERT_INIT` 단, ibex 세 파일 30→0) · 🆕 L (aa) 파스 시점 상수표·선언 범위 폭 인식(324칸 · fixed-silent 48) · 🆕 M ⓒⓓ 계층 파라미터 select/`$bits` · wide 비트 연산 override(198칸 · loud→correct 136). 잔여 = 🆕 N(`%m` 블록 스코프). ✅ **§4.5.419~421 (2026-09-05, 3-슬라이스 묶음)**: 🆕 E(fill inside a sized initializer, 153 cells) · 🆕 G + row 28(wide parameter read hierarchically, 120 cells) RESOLVED · 🆕 D 는 HEAD 재측정에서 이미 정답(§4.5.414 부수효과) · 잔여 = 🆕 M. ⚠️ **§2 를 위에서부터 읽지 마라** — 그 절은 주제별 묶음이지 착수 순서가 아니다. **2026-09-02 재그라운딩(8 에이전트 · 전 항목 HEAD 3-툴)이 정한 순서**: ~~row 29~~ **RESOLVED §4.5.405** → ~~row 30~~ **BUILT · 3라운드 리뷰 · REVERTED 2026-09-03**(§4.5.406 이 분리 가능한 절반만 실었다 · 4-piece 패치와 세 edge 는 row 30 에 · **선행조건 = §11.8.1 region sign**, 🆕 F 와 같은 뿌리) → ~~§3 ⑦~~ **RESOLVED §4.5.407** → ~~row 33~~ **RESOLVED §4.5.408** → ~~🆕 C~~ **RESOLVED §4.5.409**(fill 문맥 사이징 21/93 · 잔여 = §2 🆕 J). → ~~§3 ⑤ 파스 규칙~~ **RESOLVED §4.5.410**(typed-parameter prefix 182/227 · 키워드 철자 대조군이 드러낸 pre-existing 둘 = 🆕 K 도 함께 · 잔여 🆕 L). ⚠️ **큐가 적은 «한 파스 규칙이 워크로드를 연다» 는 거짓이었다** — 규칙 뒤에 블로커 넷(§3 ⑤ ⓐ~ⓓ · ⓓ = 중첩 packed struct 가 가장 크다). ~~§3 ⑤ ⓑ~~ **RESOLVED §4.5.411**(struct/enum typedef 배열 파라미터 114/230 · ⓒ 의 패키지 절반은 §6.20.1 로 같이 닫힘 · pre-existing silent 하나 = typedef 배열 파라미터의 명시 부호 무시 · 잔여 = ⓔ 상수 문맥 클래스). ~~§3 ⑤ ⓐ~~ **RESOLVED §4.5.412**(멀티딤 packed 파라미터 141/195 · typedef 와 키워드 철자 둘 다 · ibex_pkg.sv 에러 0 · census 가 드러낸 pre-existing 셋 = 🆕 L ⓝⓞⓟ · 새 사다리 단 ⓕ = 헤더 import 가 헤더 기본값에 안 보임). ~~§3 ⑤ ⓒ 헤더 절반~~ **RESOLVED §4.5.413**(헤더 배열 파라미터 · 기본값 `= pkg::Rst`/import/형제 · override 채널 `'{…}`/`pkg::Arr`/부모 배열 전달/부모 원소 패턴/positional · 109/183 · 0 silent · ibex_top→ibex_core 체인 verilator 일치 · pre-existing 둘 = 🆕 L ⓢⓣ). ~~§3 ⑤ ⓓ~~ **RESOLVED §4.5.414**(중첩 packed struct + 상수 폭 멤버 + struct 패턴의 fill + struct 타입 캐스트 · 65/102 · 0 silent · cheriot_pkg 50→0 · 리뷰 B BLOCKING 셋 + 델타 재채점 1 = 전부 고침 · pre-existing = 🆕 L ⓤ). ~~§3 ⑤ ⓕ~~ **RESOLVED §4.5.415**(헤더 import 를 헤더 기본값·범위에 · 80/196 · 0 silent · pre-existing silent 4 = 파라미터 선언 범위 미-fold → 32 · 🆕 L ⓘⓟ 함께 닫힘 · 새 pre-existing = 🆕 L ⓥⓦ). ~~§3 ⑤ ⓔ~~ **RESOLVED §4.5.416**(배열 파라미터 원소의 상수 문맥 소비자 · select/멤버/bit·`$size` 계열·concat·무타입 폭 · 504칸 258/31 fixed-silent/0 silent · cs_registers PMP 페이지 exit 0 · 사다리 다음 단 = cheriot `cbound_t'()` 캐스트도 같이 · pre-existing = 🆕 L ⓩ). ~~전처리기 단~~ **RESOLVED §4.5.417**(`define 기본 인자 · 본문 안 지시어/주석 이음 · `__FILE__/`__LINE__ · census 60/44 · 0 silent · ibex 전처리 에러 24→0, 다음 페이지 = 파서 prim_cipher_pkg tf-port multi-packed formal). ~~파서 단~~ **RESOLVED §4.5.418**(tf-port multi-packed formal · based 리터럴 파라미터 상수표 · 멤버 폭 §11.6 폭 fold · census 646/418 · 0 silent · fixed-silent 1 클래스 · ibex prim 두 파일 50→0, 다음 페이지 = SVA 괄호 property `(a |-> b)`). **다음 = SVA 괄호 property 단 → 헤더 파라미터 폭 멤버(lockstep)**. ⚠️ row 30 이 남긴 새 행 다섯(🆕 C~G)은 **전부 pre-existing** 이고 🆕 C 는 CLASS 다 — 트랙을 선점하지 않는다. ⚠️⚠️ **여덟 줄을 재측정했더니 여덟 줄 다 모양이 바뀌었고 셋은 severity 등급이 바뀌었다** — 착수 첫 행동은 구현이 아니라 census 다 |
...
**다음 착수**는 §2 「다음 착수 순서」 표에서 고른다. ⚠️ 2026-09-02 기준으로 이 자리에 적혀 있던 후보(12·13·14·6)는 **전부 해소되었거나 제외**다 — 12·13·6 은 RESOLVED, 14 는 선언폭 provenance 벽(16·25·26 과 **한 인프라 항목**). 새 후보 목록은 **§2 표 자체**를 보라. ⚠️⚠️ **줄을 그대로 믿지 마라** — 이 프로젝트에서 큐 줄을 HEAD 에서 재측정하면 **거의 매번 모양이 바뀐다**(§4.5.402 는 여섯 줄을 재서 **넷이 다른 방향으로** 틀렸음을 보였다). 착수 첫 행동은 구현이 아니라 census 다.
...
**serv 프로파일(2026-08-30 · `/usr/bin/sample` · 작업 샘플 6,734 · leaf self)** — 이 슬라이스를 촉발한
측정이고, 위 표와 **다른 설계**라 대체가 아니라 두 번째 좌표다. ⚠️ macOS 에서 `sample`/`vmmap`/`leaks` 가
시스템 전체로 멈춰 있었는데 결함이 아니라 `DevToolsSecurity` 가 disabled 라 보이지 않는 승인 프롬프트에
막힌 것이었다(`sudo DevToolsSecurity -enable` 로 복구).

| 층 | self | leaf |
|---|---:|---|
| 표현식 평가 | 37.8% | `WProg::run` 18.4 · `exec_vm::run` 4.7 · `run_wprog` 4.2 · `k_eval_for_lvalue` 4.1 · `vm_exec` 4.0 · `wprog_for` 2.4 |
| **스케줄러** | **31.2%** | `settle_cont_assigns` 11.6 · `propagate` 4.7 · `note_change` 4.5 · `wake` 3.3 · `simulate` 3.3 · **`quicksort` 2.1** · `dispatch_body` 1.8 |
| 쓰기 퍼널 | 7.2% | `write_chunk_word` |

⭐ **`WProg::run` 이 단일 최대(18.4%)** — serv 는 레인 커버리지가 98.6% 이고 레인이 실제로 일하고 있다.
serv 에서 지는 이유는 커버리지 갭이 아니다. ⭐⭐ 그리고 **핫 경로에 `quicksort` 가 2.1%** 로 잡혔다 —
호출자 귀속상 `propagate` 128 샘플 · `settle_cont_assigns` 24 ⇒ 표적이 되어 위 4b 로 해소.
⚠️ 남은 자리: `wprog_for` 2.4% 는 **캐시 조회만으로** 쓰는 비용이고(serv 는 72.5M 회), `run_wprog`(4.2)·
`WProg::run`(18.4) 과 별개 leaf 다.

### settle 안 wprog 거절 census (2026-08-31 · 계측 후 되돌림 · 제품 코드 변경 0)

`k_eval_for_lvalue`(serv 전체의 25.6%)의 직접 자식을 쪼개면 **`run_wprog` 64.3% · `eval_ctx`
12.0% · `wprog_for` 7.0% · self 16.0%** 다. 그 `eval_ctx` 가 **wprog 거절의 제네릭 폴백**이고
= 전체의 **3.07%**. 거절은 요청의 1.4%뿐이라 **요청당 ~8배**다 — 그래서 표적으로 삼고 쟀다.

⭐⭐ **그런데 job 이 아주 작다.** wcache 가 `(eid,w,signed)` 당 한 번만 컴파일하므로 100만 거절
**요청**의 실체는 **컴파일 시도 11번 · 모양 7개**다(가장 안쪽에서 실패한 노드로 귀속):

| 모양 | 시도 | 실패 이유 |
|---|---:|---|
| `Ternary` (요청 479k = 48%) | 2 | **가지에 `LoadIdx` 가 있으면 거절** — 컴파일형은 양쪽 가지를 항상 평가하는데 `LoadIdx` 는 범위초과를 **보고**하므로, 안 고른 가지의 보고는 제네릭이 하지 않는 진단이다 |
| `Select(Bit)` / `Select(PartConst)` | 2 / 2 | arm 안 `?` (미확정) |
| `Const` | 2 | `const_planes` — 비-`Numeric`(문자열/real) 또는 상수 폭 >64 |
| `Concat` | 1 | zero-width part — `eval_concat` 이 그것도 **평가**하므로 E4002 를 잃지 않으려 **의도적** 거절 |
| `Unary(RedOr)` | 1 | 피연산자 폭 >64 = **wide lane**(aes `w=128` 과 같은 항목) |
| `Binary(Mul)` | 1 | `WOp` 에 `Mul` 이 없다 |

picorv32 도 같은 모양이 지배한다(요청 기준 `Ternary w=32` **70%** · `AShr w=33 signed` 8.7% ·
`Lt w=1` 8.7%; 시도 기준으로는 `Const` 계열이 최다).

⚠️⚠️ **싼 우회는 없다 — 그리고 그건 실행부를 읽고 알았다.** *"인덱스 폭이 `elems` 를 넘을 수
없으면 그 `LoadIdx` 는 보고할 수 없다"* 는 아이디어는 **거짓**이다: `LoadIdx` 는 `idx >= elems`
뿐 아니라 **인덱스에 unknown 비트가 하나라도 있으면**(`word_index_of` → `WORD_UNKNOWN`) 보고한다.
definiteness 는 런타임 성질이라 컴파일 시점 폭 경계로 증명되지 않는다.

⇒ **상한이 작다**: 이 가족 **전부**를 승인해도 되찾는 것은 serv 전체의 **~2~2.5%** 이고,
그중 최대 항목은 이 모듈의 문서화된 *"제어흐름이 없다"* 불변식을 바꿔야 고쳐진다
(설계 = `Tern` 을 조건부 점프 셋으로 — cond 가 definite 면 안 고른 가지를 **건너뛰고**, unknown 이면
둘 다 돌려 기존 `Tern` 이 x-병합; `run_2s`/`run_4s` 두 실행기를 pc 루프로 바꿔야 한다).
**측정된 상금에 비해 기구가 크므로 착수하지 않는다** — 남기는 것은 이 표다.
...
- ⛔ **cranelift 재착수** — 기각. 경계가 런의 **~38%** 인데 천장이 **8.9~11.3%** 다(§5.1-be). ⭐ 재개 조건은 **하나**: leaf 로드와 2-state 산술을 **생성 코드 안에 인라인**(호출 0)하면서 **의미를 두 번 적지 않을** 방법. 전제조건은 Phase D 가 이미 만들어 뒀다(leaf = 컴파일 시점 인덱스의 워드 둘 · 산술 = 평범한 정수 연산).
  ...
  ⭐⭐ **재가격(2026-08-31) — 기각은 유지, 이유는 더 강해졌다.** 옛 판정의 전제(*"승인 커버리지가 좁아
  dispatch 가 전체의 일부"*)가 이 세션에 바뀌었는지 물었고, 실행되는 프로그램의 **op 개수 분포**로 답했다:

  | | leaf율(1-op) | ops/run | wprog runs | 이 슬라이스 효과 |
  |---|---:|---:|---:|---:|
  | serv | 56.4% | 3.62 | **88M** | **−2.3%** |
  | picorv32 | 60.0% | 4.88 | 40M | **−1.85%** |
  | biriscv | 72.5% | 2.58 | 14M | **−1.25%** |
  | keccak | 26.6% | 2.76 | 10M | −0.35% |
  | aes | **85.9%** | 1.65 | ~2M | +0.35% |

  ⭐ **실행되는 프로그램의 56~86% 가 op 하나짜리 `Load`/`Const` 다.** 재개 조건이 *"leaf 로드를 생성 코드
  안에 인라인"* 이었는데 — **그 절반 이상엔 생성할 코드가 없다.** 필요한 것은 기계어가 아니라
  **인터프리터를 안 부르는 것**이고, 그건 `WFast` 판별자 하나(§ 아래)로 끝났다. 기계어가 남겨진 자리에서
  더 지울 수 있는 것은 그만큼 줄었다 ⇒ **천장은 내려갔고 경계 ~38% 는 그대로다.**

  ⚠️⚠️ **그리고 leaf 율은 이득의 예측자가 아니다** — 내 첫 설명이 그것이었고 **측정이 반박했다**:
  aes 는 leaf 율 **최고(85.9%)인데 회귀**했고 keccak 은 **최저(26.6%)인데 이득**이다. 예측자는 **run 횟수**다
  (절감이 run 당이므로 당연하다 · 88M→−2.3% ⇒ 40M→−1.85% ⇒ 14M→−1.25% ⇒ 10M→−0.35% 로 단조).
  aes 는 wprog 자체가 미미해(비용이 frame 안 — §4.5.390) 이 변경이 닿는 표면이 없다 ⇒ +0.35% 는
  기구가 아니라 **코드 레이아웃**으로 읽는다.
