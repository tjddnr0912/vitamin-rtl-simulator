...
- ~~**🔴 `resize_inline_assign` 은 봉인 안 된 쌍둥이다**~~ **RESOLVED**(§4.5.321·상세=ARCHIVE) — 스탬프가 무조건이 되어 선언 반환 폭이 봉인한다. **잔여 넷**(전부 pre-existing·PRE==POST·적대 2렌즈 실측):
  ...
  - **🔴 인라인 인자 바인드가 formal 의 선언 타입을 적용하지 않는다 — 셋 다**(오라클 ✓·inline↔frame split·2026-08-10 에 **지어서 재고 되돌렸다**, 상세=ARCHIVE §4.5.323). 프레임 경로는 formal 이 넷이라 폭·부호·2-state 를 엔진에게서 공짜로 받고, 인라인은 셋을 손으로 지어야 하는데 하나도 안 짓는다: **부호** — `function signed [7:0] fi(input signed [7:0] x); fi = x/3;` 에서 `fi(8'hf7)` 가 inline `82` / frame·iverilog `-3`(`>>>` `123`/`-5` · `x>0` `1`/`0` · `byte` 처럼 **kind 가 부호를 나르는** 형태도 같다) · **2-state** — `byte` formal 에 x actual 이 `X`(프레임·iverilog `0`) · **폭** — 넓은 actual 이 절단되지 않아 더 좁은 내부 formal 로 재전달되면 남은 비트를 읽는다(`fs(int unsigned x) = inner(x)` 가 `1431655762` / iverilog `-3`). ⚠️ **셋은 상호작용하므로 부분 적용이 미적용보다 나쁘다**(측정: 부호만 45칸 회귀 · 폭만 넣으면 아래 선행조건에 걸린다). ★ **선행조건 = 아래 상수-인덱스 carve-out 의 부호** — 절단은 상수를 접고, 접힌 상수 인덱스는 무부호로 읽히므로 `fs(16'hFFFF)` 가 loud `xx` 에서 조용한 `a5` 가 된다. 그 carve-out 이 부호를 읽으면 이 항목 전체가 열린다.
  - **🔴 상수 인덱스 carve-out 이 인덱스를 무부호로 읽는다**(pre-existing·오라클 ✓·§4.5.310 의 의도적 carve-out). 함수 없이도 보인다 — `logic [7:0] arr[0:255]` 에서 `arr[-8'sd1]` 이 vita `a5`(= `arr[255]`) / iverilog `xx`. 위 항목의 **유일한 선행조건**이고, 그것만 고치면 인라인 바인드가 절단을 안전하게 할 수 있다. ⚠️ 상수성은 **철자가 아니라 값**으로 판정해야 한다 — `16'hFFFF`·`~16'h0`·`{8'hFF,8'hFF}`·`{2{8'hFF}}`·`1?16'hFFFF:0`·`P & 16'hFFFF`·`16'(…)` 가 전부 같은 상수이고 §4.5.310 이 이미 같은 함정을 기록했다.
