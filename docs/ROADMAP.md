...
**외부 round-20 리포트(2026-07-27) = 12 가족 중 10 RESOLVED**(§4.5.229 part-select 바운드 · §4.5.234 enum formal `.name()` · §4.5.248 8 가족 · §4.5.249 §6 진단 위치). **잔여** = ① §4.11 의 미격리 79 건 ② frame FUNCTION 본문의 `$sformatf`(temp 가 모듈 넷인데 frame-function executor 가 못 쓴다 — 면제 시 엔진 패닉 실측·직접 표현식 낮추기는 `+<` 로 조용히 틀림) ③ `$sformatf` 를 ternary arm/단락 우변/replication 값/`$monitor`·`$strobe` 인자에 두는 형태(§4.5.250 이 **의도적으로** loud 유지 — 평가 횟수·순서 보존) — 리포트 자신의 16-블록 충실 복제본도 재현 못 했고 이 체크아웃에서는 볼 수 없다. §4.5.249 의 file:line 이 리포터가 좁힐 수단이다.
