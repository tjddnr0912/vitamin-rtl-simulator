...
Declared-width provenance — rows 14, 25, 26 and 30 share one lane: a module-scope initializer of 64
bits or less folds through the width-UNLIMITED i64 walk `const_eval_in_scope` (a function local's
declared width lives in `envw`), so a signed narrow leaf is sign-extended whatever the region's sign.
The §11.8.1 wall that stood in front of them is down: since §4.5.542 the wide walk `fold_bits_at`
evaluates a region the way §11.8.2 does (width and sign decided over the whole tree, pushed into every
context-determined operand; a self-determined position a region of its own; a comparison's operands a
region sized to the larger side), and it is correct on its own terms at 65 bits and wider
(`region_sign_wide_fold.rs`, 206 cells). What remains is ROUTING a ≤64-bit declared target through
that walk (`param_i64_at_declared` / `param_bits_at_declared` ahead of the i64 walk) instead of
through `eval_const_assign`, whose i64 bound is on the TARGET only. `param_declared_width_provenance.rs`
pins the current state and the cells a fix must not move; `region_sign_wide_fold.rs`'s last test pins
...
| 14 | OPEN (routing) | `localparam logic signed [7:0] NM = -8'sd2; localparam logic [63:0] X = NM ^ 64'h0;` is `fffffffffffffffe` against the oracles' `00000000000000fe`; the same expression over a FUNCTION LOCAL folds `00…fe`. The same routing also fixes `localparam logic [7:0] M = (P + 8'd100) % 8'd7` (P=200: 6 against 2), `pk::PA ^ 64'h0`, `int S = (D - C) / 2` (2147483641), the generate-scope `time NM` shadow (2c for 12c), and the 65-bit-leaf `/` and `%` | a module-scope initializer folds through the width-UNLIMITED `const_eval_in_scope` | route a DECLARED-width/sign target through `eval_const_assign`; the gate must demand provenance of every LEAF (`param_meta` is a DEFAULT for an untyped parameter and ABSENT for a `time` one), decline above the i64 lane, refuse an unsized FILL operand, answer THREE-valued in the scope walk, and route the PACKAGE binder WITH the module lane, never alone: at HEAD a `localparam` in the module, in a package read as `pk::X`, over an imported `PA` and over `pk::PA` answer identically cell for cell (48 cells, the same 6 wrong in all four — `package_lane_matches_module_lane.rs`), the consumer side certifies an imported and a `pk::` name, and row 26 (absorbed by §4.5.546) measured a package-only routing as a net loss (1,233 correct→silent against 714 fixed). The width-aware walk is `fold_bits_at` since §4.5.542 (region sign, positions, comparison regions all §11.8.2; the §11.4.10 shift count closed as row 27), so the route is `param_i64_at_declared` ahead of the i64 walk with the same gate — not `eval_const_assign`, whose i64 bound is on the TARGET only (83 cells). Re-measure the 44-cell census on HEAD first; `region_sign_wide_fold.rs` pins `S8 + 16'd0` / `(S8 >>> 1) + 16'd0` / `+ 64'd0` known-wrong (`fffd` / `fffe` / `ff…fe`, both oracles `00fd` / `007e` / `0…07e`) and the generate-condition twin (`if ((~S8 + 128'd0) > 128'd255)` picks the else branch, both oracles the then branch) |
...
| 25 | OPEN | an OPERATOR-topped override over an ELEMENT of an unpacked array parameter binds the DEFAULT literal's type: `#(.P(~A[0][3:0]))` onto an untyped `parameter P = 0` is 32 signed bits `fffffffa` and `#(.P(A[1][3:0] + 4'd1))` 32 bits `0000000d`, where verilator binds 4 bits `a` / `d` (iverilog has no unpacked-array parameters; §11.5.1 makes the select 4 bits). Everything else this row named binds the override's own type since §4.5.545: the headline literal cells (already at HEAD), the `defparam` channel, an operator over a select of a declared name, a bare element select | `declared_override_widths` certifies a declared-width vector name and a select of one (`narrow_param_decl_width` needs a scalar `params` entry), not an array-parameter element, so `override_self_meta` declines and the binder falls back to `param_decl_width`. (A SIGNED override onto a declaration without a `signed` keyword is also still a guessed type for the size-cast classifier — that one is the `parameter unsigned` bullet of "Index sealing") | certify an element read (`const_array_elem_read` has its declared width) as a leaf in `declared_override_widths`, and give `ctx_width_names_are_evident` / `const_self_width` the element arm; measure against verilator and the `localparam` twin (`override_channel_types_the_parameter.rs` pins the two cells) |
| 30 | OPEN (routing) | `localparam logic [127:0] C = '1 ^ 1'b0;` is vita `…00000000ffffffff` against 128 ones in both oracles; the same text as `r = …`, `assign c = …` and through a port prints 128 ones. 165 of 264 cells (22 operator forms × {32,33,64,65,96,128} × {`logic`,`logic signed`}), 0 splits, all four binder copies. The band is a property of the operands, not the width: `logic [7:0] A = '1 >> 2` is `ff` against the oracles' `3f`, 82 more cells at widths 1..31 | the wide fold's fill arm plus node-local region sign | the 4-piece shape measures 778 FIXED / 0 new-silent / 0 new-loud over 1,622 cells (`fold_bits_at`'s fill arm folding at `ctx` when `ctx>0` and not x/z; `param_i64_fill_at_declared` ahead of the i64 walk in all four binders; a LOCAL predicate with a `Cast` arm; a `fill_width_survived_the_fold` guard) but it does not stand alone: §11.6.1 evaluates at `max(ctx, every self-determined operand's width)`, so freezing the fill at the LEAF is wrong (150 cells); the ROUTING predicate cannot serve as the guard (104 NEW-LOUD, since a decline at `param_bits_at_declared` is `E3009`); and of 504 loud→value cells 215 were silently wrong on the SIGN axis (`localparam logic [7:0] B = ($signed(4'hF)+1) \| 8'h00;` was `00` against the oracles' `10`, no fill anywhere) — that axis closed with §4.5.542 (the cell is `10` now, `region_sign_wide_fold.rs`): the wide fold sizes a fill at the REGION's width in its second pass and pushes the region's sign down, so the fill arm and the LOCAL predicate can be built on it. ACCEPT set = "correct a value, never create one"; re-measure the 1,622-cell census on HEAD before building |
| 🆕 H | OPEN | ⓐ closed by §4.5.543 (a definite operator over an x/z operand folds: `&4'b110x`, `\|4'b101x`, `~&`, `~\|`, `!`, `&&` / `\|\|` with a deciding operand, `===` / `!==`, an unambiguous `==` / `!=`, a ternary on a value with a known 1; 119 two-oracle cells across the range bound, the untyped and typed `localparam`, the override and the generate condition) · ⓑ the `\|P` bound of an ascending `parameter [0:3] P` or a lo≠0 `parameter [7:4] P` is loud in BOTH the module and the package lane (both oracles 4; 5 cells). §4.5.488's width-only twins do NOT reach it, measured: a reduction bound needs the operand's BITS through `wide_name_bits`, not only its width · ⓒ `localparam E = 4'hF \| 4'h0; wire [(&E)+2:0]` is loud (both oracles 4) · ⓓ `localparam R = ~(\|4'b1010);` is deliberately loud (both oracles `0`, `$bits` 1) · ⓔ a reduction after a local assignment in a constant-function body (`t = a[5:0]; return (\|t)+2;`) is loud (oracle 3) · ⓕ a bitwise operator over an x/z operand declines, so a definite operator above it stays loud: `~&(~4'b110x)` is 1 in both oracles (E3009); `(4'b110x & 4'b0000)` and `(4'b101x \| 4'b0100)` are the same class (the arms have no 4-state tables). A constant-function call with an x argument (`f(4'b101x)` returning `(!a) + 2`, both oracles 2) is the interpreter lane's twin | ⓑ `narrow_param_bits` / `pkg_const_narrow_bits` reject `lo != 0 \|\| ascending`, and that decline is the one §4.5.488 deliberately kept · ⓔ the body local's width is not in `envw` · ⓕ `const_wide.rs`'s bitwise and `~` arms decline on `bp_any_unknown` | ⓑ a reduction-only, layout-independent resolver · ⓒ the declared-width provenance lane (row 14 / 30) · ⓕ per-bit 4-state tables in the bitwise arms (`0 & x` = 0, `1 \| x` = 1, `~x` = x), carried like the placement arms; count the loud→value column at the binders as §4.5.543 did |
