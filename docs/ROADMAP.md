...
>
> **기준선(2026-07-25)**: format_version **23** · **4470 tests green** · 3-OS CI green · MsgCode **59** · **MSRV 1.85**. 최신 = **§4.5.221**(real-valued parameters — 미해결 3건으로 브랜치 `feat-real-params` 보류·§2 참조). **완료 슬라이스 55건의 상세·§번호별 원문 = [ROADMAP_ARCHIVE.md](ROADMAP_ARCHIVE.md)**(최신이 위) — 이 문서는 전방 전용이므로 완료 서사를 두지 않는다.
>
...
- **🔴🔴 §4.5.221 미해결 잔여 — real param 이 `lower_index_expr` 를 경유하지 않는 쓰기/계층/배열-원소 경로에서 SILENT-WRONG (5라운드 감사 발견·PRE loud 또는 PRE 정답 → POST 조용)**: 11형. `v[7:R] = 5'h1F`→`0`(쓰기 소실·`lvalue.rs:424-425`) · `u.v[R]` 계층 bit read→**틀린 비트**(`expr_main.rs:541`) · `u.v[R] = 1'b1`·`u.v[R +: 2]`→쓰기 소실(`lvalue.rs:359,457-459`) · `m[1][R]`·`m[1][R] = 1'b1`·`g[0][1][R]`·packed `pm[1][R]`→틀린 원소(`arrays.rs:174,265`). **근인**: `lower_expr` 의 Ident arm 이 `real_param_val` 에 있으면 **i64 쌍둥이 검사 없이** real `Const` 를 내는데, 이 11 site 는 `lower_index_expr` 를 안 거쳐 f64 비트패턴이 정수로 읽힌다. **fix 방향**: 이 11 site 를 `lower_index_expr` 로 라우팅(경계 변환이 이미 정확한 쌍둥이를 처리하므로 정수-값 real 은 correct-support 가 되고 비-정수만 loud). `lower_index_expr` 의 "모든 index site" 주석은 **여전히 거짓** — 라우팅 완료 시 테스트로 고정할 것.
- **🔴 replication count 가 `MinTypMax` 면 무한 루프(hang)**: `{(R:R:R){1'b1}}`(R=3) → 출력 없이 스핀(10s 에 733억 명령·RSS 16MB 라 메모리 캡으로 안 잡힘·iverilog 는 loud·PRE 는 `7`). `count_lowers_real_param` 의 `_` arm 이 const-fold resolver 로 폴백해 게이트를 못 문다. **AST arm 열거는 수렴하지 않는다**(4·5라운드 연속 실증) — count 를 IR 로 판정하는 게 옳으나, `lower_index_expr` 로 그냥 돌리면 `$clog2(R)` 가 SysFunc 라 `expr_is_real` 에 안 걸려 **조용한 0** 으로 퇴행(실측·revert 함). count 전용 IR 판정이 필요.
- **🔴 `$readmem*`/`$writemem*` 의 주소 인자에 real param 이 오면 f64 비트패턴을 정수로 읽음**(§4.5.221 4라운드 발견·5라운드가 **회귀임을 확정**: `$readmemh("mm.hex",m,R,R)` 가 iverilog·PRE `55 xx` → POST `xx xx`·`$writemem*` 은 파일을 조용히 안 씀): `$readmemh("f.hex", mem, R)` → `address 4607182418800017408 outside the load range` 후 `m1=0`(iverilog `m1=11`). **미수정 사유**: 해당 lowering site(`systask.rs`)가 **모든** systask 인자를 처리하는 공용 경로라 `lower_index_expr` 를 통째로 걸면 `$display("%f", R)` 가 false-loud 된다 — `$readmem*` 전용 인자-위치 게이트가 필요(같은 슬라이스의 나머지 6 site 는 인자가 전부 정수라 안전하게 전환됨).
