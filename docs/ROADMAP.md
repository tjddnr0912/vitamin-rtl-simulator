...
> | 16 | 🆕 **A parameter override that is an EXPRESSION with a context-determined top cannot be wider than 64 bits** (round-34 R5 residue · **2-oracle** · loud · **재측정 2026-09-01: 산다**) | `leaf #(.K(128'h1 << 100))` 은 loud 이고 두 오라클은 `00000010000000000000000000000000` 을 건다. 다른 wide override 철자는 전부 착지한다(`override_bits` 가 override 를 제 폭에서 접기 때문). 문맥결정 top 을 **일부러** 거절한다 — 자기 폭에서 접은 시프트는 문맥이 지켰을 비트를 잃는다. ⇒ 선행조건 = **타깃의 선언 폭에서 접기**, 표 21 과 **같은 항목**이다. ⚠️ **재측정 2026-09-01: 산다.** 표 14 의 ≤64비트 폭-인식 라우팅을 지어서 재 봤을 때도 이 줄은 **안 움직였다** — 선행조건이 **wide fold 가 문맥 폭에서 계산하는 것**이라 다른 기구다. 셋이 하나였다는 옛 문구는 **절반만 맞았다**(provenance 는 공유하고 도메인은 공유하지 않는다).
...
> | 21 | 🆕 **A `**` (and any context-determined top) cannot be folded at a width WIDER than its operands** (round-34 R3 residue · **2-oracle** · loud · **재측정 2026-09-01: 산다**) | `localparam bit [127:0] C = 3 ** 41;` 은 loud 이고 두 오라클은 `…1fa2a1cf67b5fb863` 를, `localparam longint D = 3 ** 40;` 도 같이 답한다. §11.6.1 상 `**` 는 폭을 **문맥**에서 받으므로 밑수를 128/64 비트에서 평가해야 하는데 wide fold 는 **피연산자 제 폭에서 접고 나서 확장**한다 — size cast 를 loud 로 유지하는 그 규칙과 같다. ⚠️ 이것과 표 16 은 **같은 선행조건**(대상의 선언 폭에서 override/피연산자를 접기)이고, 그 폭은 §2 표 14 가 멈춘 provenance 벽 위에 있다 ⇒ 세 줄이 아니라 **하나의 인프라 항목**이다. ⚠️ **재측정 2026-09-01: 산다.** 표 14 의 ≤64비트 폭-인식 라우팅을 지어서 재 봤을 때도 이 줄은 **안 움직였다** — 선행조건이 **wide fold 가 문맥 폭에서 계산하는 것**이라 다른 기구다. 셋이 하나였다는 옛 문구는 **절반만 맞았다**(provenance 는 공유하고 도메인은 공유하지 않는다).
> | 22 | 🆕 **A module-scope NAME can be declared twice and vita runs it** (row 8b/11 review residue · **2-oracle: both REJECT** · loud→silent-ACCEPT on one spelling, pre-existing on the rest) | `localparam A = 5; logic [7:0] A;` — either order — runs at exit 0 in vita and is *"'A' has already been declared in this scope"* in iverilog; verilator agrees. The same holds for an enum LABEL against a net or a genvar (`typedef enum { A = 5 } e_t; logic [7:0] A;`). ⚠️ **Row 11 is why this is filed now, and it did not cause it**: before that slice the label spelling was loud only INCIDENTALLY — the consumer could not fold — and the identical design with no consumer was already accepted by PRE. ⭐ So the missing thing is a module-scope NAME-SPACE duplicate check across params/localparams/labels/nets/genvars, not anything about enums. §4.5.377 built the type the check would need (`NAME SPACE` rather than a value map); this is the check. |
> | 23 | 🆕 **A clocking-input diagnostic drops the instance prefix, and its mangled key is not injective** (row 8b review · no oracle — hand-IEEE · **loud either way**, message quality only) | `clocking_hold_names` is keyed by the holding NET, so with two instances of the same module `u2.cb.s = 8'hAA;` reports ``cannot drive a clocking INPUT (`cb.s` is read-only)`` — naming neither the instance nor the spelling the source wrote, and with no `[in …]` suffix. ⚠️ Separately the mangled net name `__clk_{cb}_{sig}` is not injective: `clocking cb; input a_b;` and `clocking cb_a; input b;` collide, so the message can name a DIFFERENT clockvar than the one written. That collision always co-occurs with a pre-existing `redeclared` error (both binaries exit 1), which is why it is a message-quality row and not a correctness one. |
> | 24 | 🆕 **A clocking OUTPUT write lands as `x`** (row 8b review, out of scope for it · verilator is an oracle here — accept/reject + a 2-state value · **pre-existing, PRE == POST**) | `clocking cbo @(posedge clk); output s; endclocking` + `cbo.s <= 8'hAA;` prints `s=x` in vita where verilator prints `s=170`. iverilog cannot parse a clocking block at all. ⚠️ Row 8b's new loud (`output o = cb.s;` on a clocking INPUT) sits directly above this path, so the two read as one area and are not: the INPUT half is a legality question and this is a value. |
...
> | 27 | 🆕 **A shift's RIGHT operand is not self-determined in the width-aware constant walk** (§2 row 14 round-3 review · **2-oracle** · **pre-existing, reachable today**) | §11.4.10 makes a shift count self-determined and *"always treated as an unsigned number"*; `eval_const_assign` pushes the assignment context onto EVERY leaf including that one, so a narrow SIGNED count is sign-extended and the shift yields 0. Reachable today with no new routing, through a constant FUNCTION: `logic signed [2:0] C = -3'sd3; logic [15:0] B = 16'hFF01; f = B << C;` is **0000** in vita and **e020** in iverilog AND verilator. ⭐ **The count leaf is the only wrong one** — `B >> C+0`, `B >> C[1:0]`, `B >> $unsigned(C)` and every literal count are correct, and `**`, `&&` and a `?:` condition do not show it, so the target is `eval_const_env_at`'s leaf arm under a shift, not the sizing rules generally. ⚠️ It is also the FIRST prerequisite of §2 row 14: routing module-scope initializers through this walk turned 30 correct cells wrong and 36 loud cells wrong on this axis alone. `param_declared_width_provenance.rs` pins the const-function spelling. |
>
