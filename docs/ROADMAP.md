...
⚠️ **그런데 게이트를 다시 열자 여전히 발산했고, 그 발산이 다음 표적을 정확히 지목했다.**
`q0=7` 은 맞고 `q1`·`size` 만 x, 그리고 **native 만 exit 1** — E4002 범위 진단이고 그것을 세는
것은 **아레나뿐**이다. ⇒ 읽기가 복합 리더를 **거치지 않고** 아레나로 갔다. 범인은 §4.5.312 의
`wprog` **`LoadIdx`**(런타임 배열 인덱스 읽기): 슬롯을 직접 읽으며 **kind 가드가 없다.**
앞 항목이 *"가드는 공짜"* 라고 적은 것은 `Expr::Signal` arm 에만 참이었다(그 arm 은 화이트리스트를
갖고 있다).

**⇒ 2a 의 남은 목록** — 슬롯을 넷 id 로 **직접** 만지는 경로 전부에 heap 배제를 넣는 것:
`wprog::LoadIdx` · `wprog` 의 다른 슬롯 접근 arm · `eval_store_word`/융합 op · VCD 스냅샷 ·
dirty/엣지 스캔. **census 방법은 정해져 있다**: `arena.slots[` / `arena.read_net` 를 넷 id 로
인덱스하는 자리를 소스에서 세고, 각각에 가드가 있는지 본다. 게이트는 그때까지 **닫아 둔다**.
