...
| 꼬리 | ✅ ~~`probe`~~(§5.1-ah) · ✅ ~~concat heap chunk~~(§5.1-ai) · out-of-window write 5 · `$dumpall` 1 · X 단독 2 | 8 | ⚠️ out-of-window write 는 **과잉거부가 아니다**(실측: 목적지가 전부 모듈 넷이고 호출부가 이미 driven 프레임을 제외한다) — 위임 실행기의 쓰기 경로가 필요하다 |
