...
- ~~`$bits` 를 상수 인덱스 식에 쓰면 `x` 를 읽는다~~ — HEAD 재측정(2026-09-08): `m[$bits(m[0]) - 4'sd9]` 는 vita·iverilog 둘 다 `xx` 다(`4'sd9` 는 4비트 signed 로 −7 이므로 인덱스는 15, 범위 밖). 줄이 인용한 iverilog `10` 은 재현되지 않는다. 삭제.
...
- NEW(pre-existing · §4.5.458 soundness 렌즈 · 1건 · 다른 퍼널): **0-LSB 파라미터**의 비트/부분선택은 같은 결함이 남아 있다 — `localparam logic [7:0] K = 8'b1010_0101; K[-2'sd1]` vita `0`·`K[3'sd7]` `1` / iverilog 둘 다 `x`. 자리 = `norm_offset_if_net` 의 param arm(0-LSB 파라미터는 `param_range` 항목이 없어 `norm_offset_for_range` 에 닿지도 않는다) + `norm_offset_for_range` 의 `lo == 0` arm(넷 쪽과 같은 verbatim 반환). 파라미터 셀렉트는 elaborate 에서 접히므로 blast radius 가 넷 쪽보다 넓다 ⇒ 자체 슬라이스.
...
- 잔여 3종(전부 §4.5.458 에서 측정, 19칸): ⓐ **정밀도 == 단위**일 때만 사는 반올림 ORACLE-SPLIT 11칸 — iverilog 는 리터럴마다 잎에서 반올림(`2*2.5ns` 6 · `2.5ns+1.5ns` 5), verilator 는 완성값에서 한 번(5 · 4). vita = verilator(더 미세한 정밀도에선 두 오라클이 그 규칙에 합치). PRE 는 11칸 전부 무지연이었으니 wrong-on-both → right-on-one. 쫓지 말 것 · ⓑ 리터럴의 **단위**가 설계 정밀도보다 미세하지만 **값**은 아닌 것: `#(2500ps)`@`1ns/1ns` 두 오라클 3 ns / vita 무지연. bare 철자도 같다 ⇒ 뿌리는 §4.5.457 이 출하한 bare arm 의 `e < 0` decline 하나(`const_eval.rs::delay_ticks_in_scope`). pre-existing · 2-오라클 · 단일 뿌리 · 4칸 · format bump 없음 ⇒ 착수 가능 · ⓒ 음수 지연 `#(1ns - 5ns)`: 두 오라클 발화 안 함 / PRE·POST 둘 다 즉시 발화(`real_delay_ticks` 가 0 으로 clamp). 안 발화를 표현하려면 tick 필드가 필요 ⇒ frozen 타입.
...
|---|---|---|---|---|
| 1 | 1 | §3 ⑤ⓕ residue, in oracle-strength order: the dim-carrying OVERRIDE `m #(.T(b_t))` (**2-oracle, both `bits=64 size=4`**; needs a dim slot on the `T$w`/`T$s` channel, which is why §4.5.457 refused it at the parse and §4.5.458's sweep still shows E2002 on every `ovr_same` cell), then a `localparam` OF such a type (1-oracle, cheap — route `typedef_param_shape` into the existing `ConstArrayVar` channel). `$bits(T)` on the bare name SHIPPED §4.5.458; the DECLARATION subset and the tf-port FORMAL SHIPPED §4.5.457 / §4.5.456. The function RETURN type is do-not-start (1-oracle + frozen `FunctionDef`) | §3 ⑤ⓕ | ② |
| 2 | 2 | §2 "Delays / events" residue ⓑ, opened by §4.5.458's own differential sweep: a time literal whose UNIT is finer than the design's PRECISION but whose VALUE is not — `assign #(2500ps) y = a;` under `1ns/1ns` is 3 ns in BOTH oracles and no delay in vita, and the `#(2500ps + 1000ps)` expression twin is 4 ns / no delay. One root, one condition: the `e < 0` decline in `delay_ticks_in_scope`'s bare-`TimeLit` arm, which the expression lane inherits. 4 measured cells, 2-oracle, no format bump. ⚠️ It re-opens a rule §4.5.457 shipped and `delay_real_timelit_and_sized_negative.rs` pins ("sub-precision declines"), so measure `#(2.5ps)`@`1ns/1ns` on both oracles FIRST — that is the cell the decline was built for | §2 Delays | ① |
| 3 | 3 | §2 "Index sealing" residue, found by §4.5.458's soundness lens in the funnel NEXT to the one it fixed: a 0-LSB PARAMETER bit/part-select does not seal a narrow signed index — `localparam logic [7:0] K = 8'b1010_0101; K[-2'sd1]` is `0` and `K[3'sd7]` is `1` where iverilog reads `x` for both. Sites = `norm_offset_if_net`'s param arm (a 0-LSB param has no `param_range` entry, so it never reaches `norm_offset_for_range`) and `norm_offset_for_range`'s `lo == 0` arm (the same verbatim return the net side had). ⚠️ A param select folds at elaborate time, so the blast radius is wider than the net twin's — sweep the fold's consumers, not just the select | §2 Index sealing | ① |
