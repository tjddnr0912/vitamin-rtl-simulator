...
- **🔴 `resize_inline_assign` 은 봉인 안 된 쌍둥이다**(pre-existing·PRE==POST·오라클 ✓·§4.5.320 적대 리뷰 발굴). 같은 `Ordering::Equal => e` 와 같은 `expr_self_signed` 를 쓴다 — `function [7:0] fm(input [7:0] x); fm = x*x;` 에서 `r16 = fm(8'hFF)` 가 vita `fe01` / iverilog `0001`, `fa = x+x` 가 `01fe` / `00fe`. 곁: `inline_fn.rs:540` 의 actual 부호(`inl(fs(0))` = `000c` / `00fc`).
