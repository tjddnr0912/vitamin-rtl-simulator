...
| 🆕 I | OPEN | ⓐ another process's read in the same delta: `initial #1 v = 8'hA5;` declared before `initial #1 $display(c);` — oracles `a5`, vita `00` · ⓒ word-alias residue: a RUNTIME index `m[k]` (oracles `a5`, PRE == POST `xx`); `m[1][2]`, an `integer` array into an unsigned copy, `m[32'hFFFFFFFE]` / `m[64'd0 - 64'd2]` are splits (vita E4002 / W4029); not folded: a GENVAR index `assign cw[g] = m[g-2]` (iverilog `a5 5a`), an all-`z` driver beside a partial/delayed driver (E3001), a `force`d copy after `release` · ⓔ `bit [7:0] c; assign c = v;` excluded and unexercised (vita refuses `bit` copy destinations, `E-ELAB-LVALUE-KIND`) · ⓕ `wire signed [7:0] c; assign c = v;`: oracles read through with the COPY's sign (`r = c` → 4294967295); excluded · ⓖ a callee body is ONE set of expressions ⇒ marked only for roots EVERY calling process writes (3 of 4 cells); an `always_comb` whose only read is inside a called task never runs (`xx`, iverilog `a5`); an array-WORD and a sign-EXTENDING copy are not aliases | ⓐ a §5.4.1 race kept on the settle's value ON PURPOSE (the store-side forward broke picorv32 / UDP / keccak parity) · ⓕ interpreter/VM take the extension sign from the slot (255), the native path from the node · ⓖ callee reads are not in the sensitivity derivation | ⓕ re-stamp the copy's declared sign on the aliased read in `eval_core` AND `read_scalar_words`; a mismatch ANYWHERE in a chain disables the tail below it |
...
| ⑤ | CU-scope: unit-scope VARIABLE/net · class 본문의 unit enum label · unit 상수 사이 전방참조 · `$unit::t` · `typedef logic [7:0] a_t [0:3]` · `assign` 이 구동하는 enum-typed output port(E3018) | 파서 unit-scope 클론 밖 | 항목별 | 2-oracle (전방참조는 split, vita 는 iverilog 편) | — |
...
|---|---|---|---|---|
| 1 | 1 | §3 loud→correct, chosen by 3-tool re-measurement at start: ⑤ ⓓ/ⓔ residues · 🆕 L ⓦ package-function constant · CU-scope items in a class body · `typedef T a_t[N]` · class `#(type T)` header · ibex DPI export 12 + core:2481 (DEEP) | §3 ⑤ · §2 🆕 L | ② |
| 2 | 2 | §2 🆕 N residue: a class method's `%m` / diagnostic context prefixes the CALLING scope (`top.gi.C.show` · `top.t.C.show`; oracles `top.C.show`). Root = global class table. Fix = per-process inst_prefix sidecar, or the engine strips generate/frame segments. Elaborate-time `[in $class$C$m]` shares the root | §2 🆕 N | ① |
| 3 | 3 | §2 🆕 I ⓖ residue: copy shapes outside the alias set — array-word copy `assign c[0] = v[0]` · sign-extending copy `signed [7:0]`→`[15:0]` (iverilog measured; verilator at start) · an `always_comb` whose only read is inside a called task (different root) | §2 🆕 I | ① |
| 4 | next | mixed-caller callee (intersection rule, design decision) · `m #(8)` / `defparam u.T$w` (illegal input accepted) · 🆕 L ⓩ · VCD `$scope` `[0]` spelling · `genblk<N>` label collision · 🆕 I genvar index `m[g-2]` | §4.5.432/436/437 residues | ② |
