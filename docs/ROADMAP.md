...
- ~~**🔴 `resize_inline_assign` 은 봉인 안 된 쌍둥이다**~~ **RESOLVED**(§4.5.321·상세=ARCHIVE) — 스탬프가 무조건이 되어 선언 반환 폭이 봉인한다. **잔여 넷**(전부 pre-existing·PRE==POST·적대 2렌즈 실측):
  ...
  - **🔴 인자 바인드가 formal 이 아니라 actual 의 부호를 쓴다 — 그리고 그건 연산을 바꾼다**(오라클 ✓·같은 inline↔frame split). `function signed [7:0] fi(input signed [7:0] x); fi = x/3;` 에서 `fi(8'hf7)` 가 inline `82` / **frame `-3` = iverilog**. `>>>` 변종은 양쪽 다 다르게 틀린다(`fd`/`05`, iverilog `7d`).
