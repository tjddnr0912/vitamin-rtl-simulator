...
|---|---|
| Interfaces | nested instances, `generate`, `typedef`, `defparam` and `clocking` inside an interface body; interface instance arrays; non-ANSI or interface-typed header ports; a virtual interface that is unbound, bound to a non-interface, or re-bound |
...
|---|---|---|---|
| A wait armed in an Active batch after a write made earlier in the same batch: `initial #5 r = 1;` declared before `initial begin #5 @(posedge r); $display("late"); end`, or `reg clk; initial clk = 1;` before `initial begin @(posedge clk); … end` | the wait fires (`late 5`, `saw 0`) — the batch's changes are delivered after the whole batch, to every wait armed by then | both tools: nothing (the thread arms after the write has propagated; the reverse declaration order is a §4.7 race the tools answer differently) | Arm the wait before the writer runs (declare it first, or `#0` the writer), or wait on the value (`wait (r)`). The time-0 settle's own events are delivered before the first batch, so a wait armed at time 0 does not see them, and a net driven from a declaration-initialised variable takes its time-0 value after the initializer (IEEE §6.21): `reg r = 1; wire w = (r !== 1'b1); always @(posedge w)` no longer prints `P 0`; the `z → 0` of the settle itself is a negedge in vitamin (`N 0`), which Icarus Verilog fires for a concat and not for a compare and Verilator cannot see (2-state) |
...
| A port name repeated in a NON-ANSI header list (`module dut(a, a); input a;`) | runs; the body spelling `module dut(a); input a; input a;` is refused, as both tools refuse it | Verilator refuses the header spelling too; Icarus Verilog runs it, and on one shape prints a different value |
| A net declared in an unlabelled `generate … endgenerate` region beside a module `function` of that name | refused, with Icarus Verilog — the region's declarations belong to the enclosing module | Verilator scopes the unlabelled block and runs it |
