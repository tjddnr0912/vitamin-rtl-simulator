...
|---|---|---|---|
| An edge waiter on a net whose continuous driver reads a variable that a declaration initializer or a first-batch `initial` sets in time 0: `reg r = 1; wire w = (r !== 1'b1); always @(posedge w) … always @(negedge w) …` | `P 0` and `N 0` (the driver is settled from `x` before the initializer lands, then recomputed) | both tools print only the level line `W 0 w=0` | Initialise through the driver's own reset or a `#1`, or gate the body on `$time > 0`. A net whose drivers are literals alone (`wire w = 1'b1`, a copy of one) does not edge at time 0, as in both tools |
