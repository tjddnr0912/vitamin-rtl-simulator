...
| Type parameter `parameter type T = logic [7:0]` | Partial | Integral vector subset: `logic`/`reg`/`bit` with one or more packed dimensions, the 2-state atoms, `time`, an integral vector typedef (multi-dimensional included), another type parameter, or an unpacked-array typedef. `T` declares variables, ports, struct members, subroutine formals and returns, `typedef T u_t;`, `T'(e)` and `$bits(T)`, and is overridden by `.T(logic [15:0])`, positionally, by a typedef, by `pkg::t` or by `.T(T)`. An override may change the width and the packed / unpacked extents; one that changes the dimension count (packed or unpacked) is refused. A struct, enum, union, real, string or class type as the default or the override is Loud (`VITA-E2002`), as is a class `#(type T)`, a package `parameter type` used as `pkg::T`, and a multi-dimensional `T` as a packed struct/union member, an enum base, a function return type or a non-ANSI port. |
