...
| Unpacked-array typedef (`typedef logic [7:0] a_t [0:3];`, `[4]`, `[]`, `[$]`, `[string]`) | Yes | The dimensions reach the declaration: a variable, an ANSI port (input or output), a package / `$unit` type, a block-local or generate-scope declaration, `= '{…}`, a chained alias `typedef a_t b_t;`, an interface member. A non-ANSI port (`module m(i); input a_t i;`) and `$bits(a_t)` of the bare type name answer too (32 for `[0:3]` of a byte, 64 for a 2-D one). Loud: as a function return type, a subroutine formal, a `parameter`, a `parameter type`, an enum base, a packed-struct member, `a_t'(e)`, `$bits` of a `[]` / `[$]` / `[string]` typedef, `$bits` when a dimension names a parameter, and dimensions on BOTH the typedef and the declarator (the reference tools disagree about the dimension order). |
