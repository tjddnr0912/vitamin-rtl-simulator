...
| `defparam inst.param = const;` | Partial | Direct-child target only (exactly two path segments), last write wins per IEEE 1800 §23.10.1, and it beats a `#()` override. Its value binds through the same channels as a `#()` override (a literal wider than 64 bits, `$signed` / `$unsigned`, a string); one carrying an x or z bit is refused. A longer path, a non-constant value, or a `defparam` inside `generate` is Loud (`VITA-E3009`). A target matching no instance warns and is ignored. |
...
| Type parameter `parameter type T = logic [7:0]` | Partial | Integral vector subset: `logic`/`reg`/`bit` with one or more packed dimensions, the 2-state atoms, `time`, an integral vector typedef (multi-dimensional included), another type parameter, or an unpacked-array typedef. `T` declares variables, ports, struct members, subroutine formals and returns, `typedef T u_t;`, `T'(e)` and `$bits(T)`, and is overridden by `.T(logic [15:0])`, positionally, by a typedef, by `pkg::t` or by `.T(T)`. A package `parameter type PT` is usable as `pkg::PT` everywhere a package typedef is (the explicit `import p::PT;` is Loud). An override may change the width and the packed / unpacked extents; one that changes the dimension count (packed or unpacked) is refused. A struct, enum, union, real, string or class type as the default or the override is Loud (`VITA-E2002`), as is a class `#(type T)` and a multi-dimensional `T` as a packed struct/union member, an enum base, a function return type or a non-ANSI port. |
...
| `@e`, `@clk`, `@u.s`, `@a[2]` without parentheses | Supported | Parsed as a single level term, which then follows the rows below (a select of a net is Loud). |
...
| Level (non-edge) event control on a bit, part or element select of a net (`@(n[0])`, `@(n[1:0])`, `@(a[1])`, `@(n[i])`) | Loud | `VITA-E3009` — wait on the whole variable, or use `posedge`/`negedge` on a vector's bit 0. vitamin's level wait fires on any change of the whole variable, where both reference tools wake only when the selected bits change. The refusal fires even when a live term sits beside it in the same list. |
| Edge term on a name that cannot change (a parameter, `localparam`, genvar, enum label, package constant `p::C`, or a constant select such as `K[0]`) | Supported | The term is dropped: it can never fire. A process whose every term is dropped simply never wakes, which is what both reference tools do. In a mixed list the live terms still arm. |
| Level term on a constant beside a live term, in a process HEADER (`always @(K or clk)`, `@(K[0] or clk)`, `@(p::C or clk)`) | Supported | The process runs once at time 0 and then on every change of its live terms, as in both reference tools, when the block is an `always` written in the source, its list has no edge term and no `iff`, and its body cannot suspend. Any constant kind counts: a parameter (per-instance overrides included), `localparam`, genvar, enum label, `$unit` or imported constant, `p::C`, and a select of one whose index is constant. Otherwise the constant is dropped and the process has no time-0 run: with a suspending body (`#`, `@`, `wait fork`, intra-assignment timing, `fork … join`) the reference tools disagree and vitamin follows Verilator; the other bodies not taken (`wait (c)`, `fork … join_none`, an imported package task, a recursive task) are a known divergence (see Limitations). |
| Level term on a constant with NO live term, in a process HEADER (`always @(K)`, `@(K[0])`, `@(p::C)`, `@(K or K2)`) | Supported | The process runs once at time 0, after the `initial` statements of time 0, and never again, as in both reference tools — under the same conditions as the row above (an `always` written in the source, no edge term, no `iff`, a body that cannot suspend). A `$finish` reaching time 0 through any channel does not erase that run, since a `$finish` ends the run at the end of its time step. Otherwise it is Loud (`VITA-E3009`): the message prints the constant as written and names this block's reason. |
| Level term on a constant, IN-BODY (`@(V);`, `@(K[0]);`, `@(p::C);`) | Supported | Never wakes, matching both reference tools. |
| Event control on a select of a constant whose index changes (`@(K[i])`, `@(posedge K[i] or posedge clk)`) | Loud | `VITA-E3009` — the selected value changes when the index does, and vitamin's event waits track nets, not an index. An index that reaches a changing net only through a concatenation, a system function or a hierarchical name is not recognised yet: that term never wakes (see Limitations). |
...
| Construct | Status | Notes |
|---|---|---|
| `interface` / `endinterface` | Partial | An instance is flattened into plain nets plus symbol aliases. The accepted body items are: net and variable declarations, continuous assigns, procedural blocks (including block-locals inside them, which behave exactly as they do in a module), `function` and `task` declarations, `modport`, `parameter`, port declarations, `genvar` and `import` (constants, types, variables, functions, tasks and constant functions; an interface body's calls resolve in the interface's own scope, never the enclosing module's). A DECLARED routine behaves as the same text in a module does: its static locals are per interface instance, `%m` names that instance, it may be called from the parent hierarchically and folded in a `localparam` or a header parameter default, and it wins a wildcard `import` of the same name. An explicit `import pkg::f;` colliding with a routine declared in the same interface and a `modport` sharing a declared routine's name are Loud; a compilation-unit (`$unit`) import of that name is shadowed by the local declaration (IEEE 1800 §26.4) and binds silently. A name used before it is declared in an interface body is Loud (IEEE 1800 §6.10), as in a module. A nested instance, a `generate` block, a `typedef` and a `defparam` inside an interface are Loud. |
...
| `pkg::f(...)` scoped routine call | Supported | A function whose body names only its own formals/locals, same-package constants, variables and routines; its same-package callees are reached the same way as by `import pkg::f;`, including callees with initialized locals, control flow, loops and mutual recursion. Loud: a callee (or the routine itself) that is `static` and reads a local before writing it, or reads its own return variable before assigning it (`VITA-E3010`); a scoped TASK enable as a statement (`VITA-E2002`). |
