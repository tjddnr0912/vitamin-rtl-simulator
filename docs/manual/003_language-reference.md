...
| Type parameters — `parameter type T = logic [7:0]` / `type T = int` / `localparam type` on a module or interface header, in a body, at the compilation-unit scope (IEEE §6.20.3) | Yes — integral vector subset | `T` declares variables, ports, struct members, function formals / returns, `typedef T u_t;`, `T'(e)`, `$bits(T)`; overridden by a type (`.T(logic [15:0])`, positional, a typedef, `pkg::t`, `.T(T)` pass-through). An override may change the width; one that changes the signedness or the 2-state kind is refused loudly. An unpacked-array typedef may be the DEFAULT (`typedef logic [7:0] a_t [0:2]; #(parameter type T = a_t)`): its dimensions reach every declaration of `T` — a variable, a port, a subroutine formal, `foreach`, `$size(v,1)` — so `$bits(v)` is 24. Loud: a struct / enum / union / real / string / class or multi-dimensional type parameter, a class `#(type T)`, and — for a type parameter whose default carries unpacked dimensions — any instance OVERRIDE of it (the width channel an override travels on has no slot for a dimension, so an override that kept the default's would report a wrong `$bits`), a cast `T'(e)` and `$bits(T)` on the bare name. |
...
| `#delay` scaling by unit | Yes | Delays scale to the global precision-based 64-bit time axis. A structural delay written as a real time literal (`assign #(2.5ns) y = a;`) rounds at the module's **time unit**, as both reference tools do: under `10ns/1ns`, `#(25ns)` is 2.5 units and delays three of them. A negated sized literal is a self-determined value read as unsigned — `#(-4'd1)` is 15 ticks. Loud: a real time literal in a PROCEDURAL delay (`#(2.5ns) stmt;`). |
