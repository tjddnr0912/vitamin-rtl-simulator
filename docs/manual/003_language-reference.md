...
| Level term on a constant with NO live term, in a process HEADER (`always @(K)`, `@(K[0])`, `@(p::C)`, `@(K or K2)`) | Loud | `VITA-E3009`. Both reference tools run such a process once at time 0 and never again. vitamin does not admit the list yet (ROADMAP §2 "Delays / events"; the `$finish` prerequisite it waited on is closed). The message prints the constant as written and names this block's reason. |
