...
| Construct | Status | Notes |
|---|---|---|
| `interface` / `endinterface` | Partial | An instance is flattened into plain nets plus symbol aliases. The accepted body items are: net and variable declarations, continuous assigns, procedural blocks (including block-locals inside them, which behave exactly as they do in a module), `modport`, `parameter`, port declarations, `genvar` and `import` (constants, types, variables, functions, tasks and constant functions; an interface body's calls resolve in the interface's own scope, never the enclosing module's). A nested instance, a `generate` block, a function or task, a `typedef` and a `defparam` inside an interface are Loud. |
