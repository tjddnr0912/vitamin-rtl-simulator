...
| Unpacked-array typedef (`typedef logic [7:0] a_t [0:3];`, `[4]`, `[]`, `[$]`, `[string]`) | Yes | The dimensions reach the declaration: a variable, an ANSI port (input or output), a package / `$unit` type, a block-local or generate-scope declaration, `= '{…}`, a chained alias `typedef a_t b_t;`, an interface member. A non-ANSI port (`module m(i); input a_t i;`) and `$bits(a_t)` of the bare type name answer too (32 for `[0:3]` of a byte, 64 for a 2-D one), in the package-scoped spelling as well (`$bits(pkg::a_t)`, and `$bits(pkg::T)` of any struct / union / enum / atom alias). A dimension may name a constant, including an overridable `parameter`: `$bits` is then computed per instance from the OVERRIDDEN value (`typedef logic [7:0] a_t [0:N-1]` is 64 under `#(.N(8))`), and a package type's dimensions always read the package's own constants, never a same-named one in the module that uses it. A variable or subroutine formal that shares the type's name shadows it, as it does elsewhere. A subroutine formal (`function int f(input a_t v);`, both binders, every direction) and a type-parameter DEFAULT (`#(parameter type T = a_t)`) carry the dimensions too. Loud: as a function return type, a `parameter`, an enum base, a packed-struct member, `a_t'(e)`, `$bits` of a `[]` / `[$]` / `[string]` typedef, and dimensions on BOTH the typedef and the declarator (the reference tools disagree about the dimension order). |
