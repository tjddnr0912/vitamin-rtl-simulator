...
| `parameter` | Yes | Overridable at instantiation. An unsized fill inside a sized initializer (`localparam logic [39:0] X = '1 ^ 1'b0;`) is sized by the declared width (IEEE §5.7.1 / §11.6.1); an instance-override expression containing a fill is folded before the target's width is known (an axis the oracles split on). An UNTYPED parameter takes its type from its value (IEEE §6.20.2), and when that value is an OPERATOR the width is the operator's own (Table 11-21), not the value's magnitude: `~8'h5A` is 8 bits (`a5`), `8'hFF << 8'd9` is 8 bits (`00`), `2'd3 ** 4'd10` is 2 bits (`1`), a comparison is one bit, and a bitwise pair takes its wider operand. `+`, `-` and `*` follow the same rule, so an untyped sum WRAPS at its operand width — `localparam Q = 8'd200 + 8'd100` is 44 and `parameter A = 1 << 32` is 0. Declare a width (`localparam [31:0] Q = …`) when the wider value is wanted. A DECLARED width still wins, and so does an override's — except that an override of an untyped parameter is still sized from the default's initializer rather than its own expression (`#(.HE(~4'h5))` binds 32 bits where the reference tool binds 4). |
| `localparam` | Yes | Not overridable. |
| Type parameters — `parameter type T = logic [7:0]` / `type T = int` / `localparam type` on a module or interface header, in a body, at the compilation-unit scope (IEEE §6.20.3) | Yes — integral vector subset | `T` declares variables, ports, struct members, function formals / returns, `typedef T u_t;`, `T'(e)`, `$bits(T)`; overridden by a type (`.T(logic [15:0])`, positional, a typedef, `pkg::t`, `.T(T)` pass-through). An override may change the width; one that changes the signedness or the 2-state kind is refused loudly. An unpacked-array typedef may be the DEFAULT (`typedef logic [7:0] a_t [0:2]; #(parameter type T = a_t)`): its dimensions reach every declaration of `T` — a variable, a port, a subroutine formal, `foreach`, `$size(v,1)` — so `$bits(v)` is 24. Such a type parameter may also be OVERRIDDEN with another array type (`#(.T(b_t))`, positional, package-scoped, one or two dimensions): the override's element width and its array bounds both take effect, so `$bits`, `$size`, `$low` / `$high` and the element addresses all follow it — including an override with different bounds (`[1:4]`) or a different extent (`[0:7]`). What an override may not change is the NUMBER of dimensions, because the module's declarations of `T` are shaped once when the module is read; adding or dropping one is refused loudly rather than mis-shaped silently. Loud: a struct / enum / union / real / string / class type parameter, a class `#(type T)`, an override that changes the dimension count, a cast `T'(e)`. |
| Compilation-unit-scope declarations — `typedef`, `localparam` / `parameter`, `function`, `task` before the first `module` (IEEE §3.12.1) | Yes | Visible to every module, interface, package and class that follows in the unit; a module's own declaration of the same name shadows it; a unit `parameter` is a localparam. Loud: `$unit::t`, a declaration after its first use, a unit-scope variable or net. |
| Elaboration system tasks (`$info` / `$warning` / `$error` / `$fatal` as a module item, IEEE §20.11) | Yes | Run at elaboration with constant arguments (`"…"`, a parameter, `%m`, `%d %h %b %o %s %c`); `$error`/`$fatal` fail elaboration with no simulation (exit 1); also inside a generate branch. Loud: a non-constant argument, any other `$task` as a module item. |
...
| Unpacked-array typedef (`typedef logic [7:0] a_t [0:3];`, `[4]`, `[]`, `[$]`, `[string]`) | Yes | The dimensions reach the declaration: a variable, an ANSI port (input or output), a package / `$unit` type, a block-local or generate-scope declaration, `= '{…}`, a chained alias `typedef a_t b_t;`, an interface member. A non-ANSI port (`module m(i); input a_t i;`) and `$bits(a_t)` of the bare type name answer too (32 for `[0:3]` of a byte, 64 for a 2-D one), in the package-scoped spelling as well (`$bits(pkg::a_t)`, and `$bits(pkg::T)` of any struct / union / enum / atom alias). A dimension may name a constant, including an overridable `parameter`: `$bits` is then computed per instance from the OVERRIDDEN value (`typedef logic [7:0] a_t [0:N-1]` is 64 under `#(.N(8))`), and a package type's dimensions always read the package's own constants, never a same-named one in the module that uses it. A variable or subroutine formal that shares the type's name shadows it, as it does elsewhere. A subroutine formal (`function int f(input a_t v);`, both binders, every direction) and a type-parameter DEFAULT (`#(parameter type T = a_t)`) carry the dimensions too. A `localparam` or `parameter` may have the type (`localparam a_t P = '{…}`): the module body, a package, a package-scoped prefix (`pk::a_t`), an interface body and an ANSI module header all bind it, an interface or module header default takes an override (`#(.P('{7,8,9}))`), and dimensions written on the NAME come before the typedef's own (`localparam a_t P [0:1]` is `P[0:1][0:2]`). Loud: as a function return type, an enum base, a packed-struct member, `a_t'(e)`, `$bits` of a `[]` / `[$]` / `[string]` typedef, dimensions on BOTH the typedef and the declarator (the reference tools disagree about the dimension order), and — for a parameter of the type — a `string` element, a module-BODY overridable `parameter`, and an interface or program HEADER declaration; each of those is refused for the same reason its explicitly-dimensioned twin (`localparam string P [0:1]`, `parameter int P [0:2]`) is refused. |
...
| Module instantiation & hierarchy | Yes | Named (`.p(x)`) and positional port maps; arbitrary nesting. A hierarchical read of a parameter (`u.P`, `u.g.P`, `u.v.P`, an interface's) folds to its constant, including one wider than 64 bits; a bit / part / indexed-part select of one (`u.P[7:0]`, `u.P[0]`, `u.P[b+:w]`, a non-zero-LSB declaration) and `$bits(u.P)` / `$bits(u.net)` fold too. A hierarchical name may be the target of a WRITE — procedural (`u.x = v;`, `u.mem[i] = v;`, `u.x[3:0] = v;`) or CONTINUOUS (`assign u.x = v;`, and the same select forms) — in any direction: into a child, up into the parent, or the module's own net through its full path. A continuous assign may drive a hierarchical `wire`; a procedural write to one is refused (`E3018`), as Icarus refuses it. Loud: a select of an ascending (`[0:7]`) or value-sized (untyped, unranged) hierarchical parameter; `$bits` of a hierarchical string. |
| `interface` / `modport`, `package`, `program`, `class` | Yes | Interfaces bind as signal aliases (modport direction enforcement pending); packages with `import`; `program` blocks; classes with inheritance + virtual dispatch, parameterized classes, and constrained-random (`rand`/`constraint`/`randomize()`). A procedural **block-local** inside an interface body behaves exactly as it does in a module — including one that shares a name with an interface member, a named block, a `for` body, sibling blocks reusing a name, and several instances of one interface. Still outside the MVP inside an interface: `generate`, functions/tasks, nested instances, typedefs and `defparam`. |
...
| `time` | Yes | 64-bit, 4-state. As a `parameter` / `localparam` the width and the unsignedness come from the DECLARATION, never from the initializer: `localparam time A = 8'd5` is 64 bits, and `localparam time B = -8'sd2` reads `18446744073709551614`. |
| `real` / `realtime` | Yes | IEEE-754 f64, 2-state. `realtime` is a synonym. `$bits` of a real variable OR of a `real` / `realtime` parameter is 64 (IEEE §6.12.1), in every context — a range bound, a `localparam` initializer, a `generate if` condition, a constant function. A real parameter read through `import p::*` is a known exception and still answers 32; the `p::P` spelling is correct. |
...
| `typedef` | Yes |
| `struct packed` | Yes — a member may be another packed struct/union typedef (`perms_t perms;`, any depth: `s.a.b.c`, `arr[i].a.b`, sub-selects, `$bits`, ports, a `'{…}` recursed per member or a plain value for the nested slot, `s.perms = '{…}`, a `default:` fill, a struct-typed cast `cap_t'(e)` also in a constant) and its width may name a constant (`logic [W-1:0]`, `[0:W-1]`, a typedef of one, `$clog2(N)`, `W/2`, `p::W`: a `localparam`, a package `parameter`, a body `parameter` behind an ANSI header — IEEE §6.20.1 — directly or through `import`). A member width folds at its IEEE §11.6 width (`[C+D:0]` over 4-bit constants, `[$clog2(W+2):0]` over a 32-bit one wrap as in both oracles); a parameter whose value is one based literal (`32'd12`, `'hff`, `40'sd5`) is such a constant when it is 32 bits or wider. A member width naming an OVERRIDABLE `parameter` (an ANSI header's, a header-less module's body `parameter`, or a localparam derived from one — `logic [MemDataWidth-1:0]`, `[2*W-1:0]`, `[$clog2(W)-1:0]`) is laid out per instance: declarations, ports, packed and unpacked arrays of the type, member reads/writes, `arr[i].f`, `t'(e)`; loud on such a struct: a member sub-select, `$bits(T)`, a `'{…}` pattern, a union, a nested-member chain, a non-zero / non-literal LSB or ascending range, a tf-port or subroutine formal of the type. Loud: `1 << 3` / a truncated or x/z literal / a narrower-than-32-bit parameter with a based-literal value / a forward-referenced localparam as that width, a `default:` for a nested member that is not a fill or 0, an enum method through a chain, a packed array of a struct typedef as a member (`in_t [1:0] i`), an indexed member write |
...
| Array-parameter element in a constant — `localparam X = P[1].b` / `A[1][4:0]` / `A[i][7-:4]` as an override / `$size(A)` / `{A[0], A[1]}` | Yes — a select, member or bit of an element of a 0-based single-dimension array parameter (body, generate, header, package) folds in every constant position with the element's declared range and sign; the `$size` family folds over an array parameter, a parameter with a declared range, and a multi-packed parameter; an untyped parameter takes a select initializer's width. Loud: a multi-packed ELEMENT array (`logic [1:0][3:0] A[2]`), an ascending / non-zero-LSB element inside a concatenation or replication count, a select outside the element, a header struct-typed array override pattern, `p::S[1].b` |
| ANSI header `import` — `module m import p::*; #(parameter T X = Dflt) (…)` / `interface i import p::*; #(…)` | Yes — a compilation-unit import and the header's own `import pkg::*` / `import pkg::X` / comma list are visible to the header's parameter defaults and ranges (`= Dflt`, `[W-1:0]`, `$clog2(N)`, an enum label, a sibling) and to the port list; a body `import` applies after the header (a header default naming a body import's constant is loud). Loud: an explicit import of a name the module also declares (IEEE §26.3), a name two wildcards export when referenced, a `real`/string package parameter through an import. A package function (`import p::*`, `import p::f`, `p::f(…)`) or a module function folds in a header default, a `localparam` and a range bound (its body runs in the package's own constant scope); a call in a range bound that does not fold is loud |
...
|---|---|---|
| `function` with return value | Yes | ANSI or non-ANSI ports; range and `signed` qualifiers; a multi-dimensional packed formal (`logic [15:0][3:0] shifts`, inline or typedef, any direction incl. `ref`) with element/part reads, element writes and the `$size` family in the body. Loud: a multi-dim packed formal that is also an unpacked array. |
...
| `#delay` scaling by unit | Yes | Delays scale to the global precision-based 64-bit time axis. A structural delay written as a real time literal (`assign #(2.5ns) y = a;`) rounds at the module's **time precision**, as both reference tools do — under `10ns/1ns`, `#(25ns)` delays 25 ns, and `#(2.5ns)` under `1ns/1ns` delays 3 ns. A literal whose UNIT is finer than the design's precision is rounded where it is written rather than dropped: `#(2500ps)` under `1ns/1ns` is 3 ns, `#(1250fs + 1250fs)` under `1ns/1ps` is 2 ps, and one that really is smaller than a tick (`#(2.5ps)` under `1ns/1ns`) rounds to no delay, all matching both reference tools. A negated sized literal is a self-determined value read as unsigned — `#(-4'd1)` is 15 ticks. The PROCEDURAL lanes use the same rule as the structural ones — a statement delay, a statement prefix, an intra-assign `=` or `<=`, a task body, `always`, a `fork` arm, `repeat` and `for` — so `#(2500ps);` under `1ns/1ns` is 3 ns and `#(3ns / 2)` is 2 ns, not the 1 ns an integer division would give. A delay that evaluates NEGATIVE never fires in a procedural lane (`#(1ns - 5ns)`), as in both reference tools. Loud: a negative time LITERAL (`#(-2500ps)`), where both reference tools instead never fire. |
