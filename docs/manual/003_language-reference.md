...
| Level term on a constant with NO live term, in a process HEADER (`always @(K)`, `@(K[0])`, `@(p::C)`, `@(K or K2)`) | Loud | `VITA-E3009`. Both reference tools run such a process once at time 0 and never again. vitamin refuses it because its `$finish` ends a time step without running the processes already woken in it, so a `$finish` reaching time 0 would erase that one run at exit 0. The message prints the constant as written and names this block's reason. |
