...
| REQ | 내용 | 원문 | 분류 |
|---|---|---|---|
| R-L0 | run manifest(run.json — 출하 필드: schema_ver·tool·version·format_version·seed·plusargs·source{name,blake3}·finish_reason·exit_class·exit_code·sim_time·counts{…}·status·**backend**·**codegen{able,total,frame_bodies,reject_reasons{…}}**·**native{eligible,reject_reasons{…}}**·utc_unix_s·wall_s. `backend`/`codegen`/`native`=T0/S0 계기(doc-21 §7.3, 2026-08-03 additive — 기존 kind 위 필드 추가라 schema_ver 1 유지): `backend`=실제 실행기(`"vm"`\|`"interp"`, `--backend` 어휘 — `codegen` 이 정적 census 라 이 필드 없이는 interp 강제 런의 `able` 이 "VM 이 돌렸다"로 오독됨), `codegen`=②층 VM 이 이 설계에서 무엇을 거부했는가(엔진 compile gate 와 같은 walk = 단일 소스 · `sformatf` 키는 desugar 된 string concat 도 포함 — "소스가 $sformatf 를 썼다"가 아니라 "IR 이 $sformatf 노드에 닿는다"), `native`=③층 설계 수준 적격 판정((설계, 런 옵션) 별 정적 — `--probe`/stage 계측 런은 설계상 부적격, doc-21 §4.3). 리뷰어 원문의 `run_id`는 **미구현-연기**, wall-clock 키는 `utc`가 아니라 `utc_unix_s`/`wall_s`) | §2 L0 | 로그 rail |
