...
> | **S1d-4c-2d** | **in-body 웨이터**(`WaitCause::Edge`/`Expr`/`Level{arm=Some}`) — 코퍼스 커버리지 **0**(측정: 정지 터미네이터 138 이 전부 `Delay`)이라 전용 설계로 짓는다 | 웨이터 arm/consume/재무장 결정 차분 + 전용 설계 stdout 동일 |
