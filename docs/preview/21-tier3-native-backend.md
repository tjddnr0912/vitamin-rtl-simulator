...
> **슬라이스 1 ✅ (2026-08-05, §4.5.305) — 중단 판정 통과.** `native/wprog.rs` = admission-gated
> 폭 특수화 평가기(`W=(val,unk)` u64 두 평면·컴파일 시점 확정 슬롯 인덱스·(rhs,폭,부호) 캐시).
> **admission 이 정확성 논증**(균일 폭 ≤64·unsigned·상수 인덱스/시프트량 → 넓힘·부호확장·절단이
> 트리에 존재하지 않음), 나머지는 제네릭 경로 그대로. 4-state 비트 테이블은 **소진 배터리
> 256×256×11 + 코퍼스 admitted 2255** 로 제네릭 평가기와 0 발산.
> **native 4.49 → 2.79 s = 1.61×**(keccak_f_flat·N=5000·release·vm 2.55 s 와 10% 이내).
> And arm 실측 = 로드 2 + **분기·호출·할당 0 + ALU 9** — 문자 그대로의 "2 op" 는 2-state 전용이라
> 다음 표적이 **`two_state` 슬롯 컴파일 특수화**(슬라이스 2)이고, 재설계 사유는 없다.
> ⚠️ 프로파일 잔여(같은 실측): `mask_top`/`resize` 는 **쓰기 퍼널**(`write_chunk`)과 declined 트리에
> 남아 있다 — 슬라이스 2/3 의 표적.
> ⚠️ **측정 비교가능성**: bench/keccak 의 TB 가 `start` 를 설계가 샘플하는 바로 그 posedge 에 세워
> **Active 리전 순서 레이스**를 만든다(IEEE 1364 §11 미정의). iverilog 와 vita 가 다른(둘 다 적법한)
> 선택을 해 vita 쪽이 퍼뮤테이션당 1주기 더 돈다 — 값은 동일. **재측정 슬라이스가 TB 를 레이스-프리로
> 고치고 숫자를 다시 잰다**(지금 고치면 이 절의 수치가 무효가 된다).
