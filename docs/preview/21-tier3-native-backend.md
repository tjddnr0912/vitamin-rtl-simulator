...
> (커밋 `3ec7dc7`, 5051 tests green). 그 결과가 이 문서의 **§1.1 근거와 §7 우선순위를 바꾼다** —
> 아래 §0.5 를 먼저 읽을 것.
...
### 7.1 선결 조건 ①의 답: **③층이 고칠 수 있는 격차가 아니었다**

개정 1 은 "리뷰어 성능 테스트 결과 수령"을 착수 전 조건으로 걸었다. 받았고, §0.5 가 그 결과다.

| | |
|---|---|
| 관측된 격차 | Xcelium 31 s vs vita 시간 단위 — **매우 큼** |
| 그 격차의 위치 | **RTL 시뮬레이션이 아니라 `string` 원소 접근 한 곳** (TB 비용의 99.4%) |
| ③층이 고쳤을까 | **아니다.** 알고리즘 결함(O(len) 인덱싱)이지 표현 범용성의 비용이 아니다 |
| 실제로 고친 방법 | 워드 단위 추출 + 제자리 인덱싱 — **커밋 하나, 172×** |

**"격차가 크다"가 "③층이 답이다"를 뜻하지 않는다.** 이 리포트가 그 구분을 실측으로 보여줬다.

### 7.2 그래서 지금 우선순위는 ③층이 아니다

| 순위 | 무엇 | 근거 |
|---|---|---|
| **1** | **실사용 워크로드를 더 넓게 재고, 남은 알고리즘 결함을 찾는다** | 리포터가 벤치 하나 밖에서 **172× 짜리**를 찾았다. 같은 종류가 더 있을 가능성이 낮지 않다 — `substr`·문자열 비교·`$sformatf`·assoc 키·dyn 배열 성장 등 **len 에 비례하는 경로 전수 점검**이 미완이다 |
| **2** | ③층 **S0**(설계 수준 게이트 + 적격률 측정) | 값이 싸다(~200줄, 되돌리기 0). 1번의 결과와 무관하게 정보 가치가 있다 |
| 3 | ③층 S1 이후 | **1번이 끝나고, 그때도 격차가 표현 범용성에 있다고 측정될 때만** |

### 7.3 ③층 착수의 새 전제조건

개정 1 의 두 조건에 하나를 **추가**한다:

1. ~~리뷰어 성능 테스트 결과 수령~~ → ✅ 받았고, **③층 근거를 강화하지 않았다**
2. RHEL9/UBI 에서 최신 rustc 확보 가능성 확인 — **여전히 유효** (MSRV 정책이 상한 없음으로 개정됐으므로 이제 CI 확보 가능성만이 제약이다)
3. **⭐ 신규: 남은 `len` 비례 경로를 전수 점검하고, 그 뒤에도 격차가 남는지 재측정.**
   문자열 결함 하나가 TB 비용의 99.4% 였다. 그것을 고친 지금 **같은 워크로드를 다시 재기 전에는
   ③층이 무엇을 얼마나 고칠지 말할 수 없다.**

### 7.4 계약 개정은 그대로 유효하다 (2026-08-03 승인)
