...
- **하이브리드 전략 (인터프리터 MVP → IR 경계 너머 컴파일드 옵션)** — Phase 1은 정확성과 VCD 정밀도를 먼저 확보하는 인터프리터 방식. `sim-ir`를 언어 중립 경계로 설계해 재작성 없이 컴파일드/JIT 백엔드를 후속 단계에서 추가할 수 있다. *(2026-08-16 현재: 실행기가 **셋**이다 — ①`interp`(의미의 정본·오라클) ②`vm` 바이트코드(기본 백엔드) ③`native` 평면 아레나. ③층은 **전체 코퍼스의 100.00% 를 바이트 정확하게 실행**하고(6,470 중 거부 0 · flip 발산 0 · ROADMAP §5.1-ap), 다음 단계는 제품 표면을 native 하나로 좁히는 **빌드 분리**(Phase B)다. 기계어 생성은 아직 없다 — 그것은 Phase D 이고 본체는 cranelift 가 아니라 **2-state 좁히기**다. 해설 = [study/02](../study/02-v1-native-coverage.md))*
