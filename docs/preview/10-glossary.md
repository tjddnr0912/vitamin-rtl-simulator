...
| **`format_version`** | The monotonic integer stamped into every artifact header, currently `32`. It is bumped when the golden shape changes, and also when a staged trailer segment is added even though the golden frame is unchanged. | — |
