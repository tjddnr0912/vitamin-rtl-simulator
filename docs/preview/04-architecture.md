...
(§4.5.329~332). **실측**(picorv32 · release · 번갈아 best-of-5 · 2026-08-16):
...
방법으로 계산하고, 둘이 갈렸을 때 **누가 틀렸는지 판정할 기준**이 필요하다. Phase C 는 그것을
*지우는 것*이 아니라 **제품 표면에서 빼고 테스트 전용 오라클로 강등**하는 일이고, `oracle` feature 가
이미 그 경계다.
